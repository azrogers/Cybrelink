...
void GenericOptionsInterface::ChangeOptionValue(char* option, int newvalue)
{
	...
	if (strcmp(option, "sound_musicenabled") == 0) {
		...
		if (newvalue == 0) {
			...
			SgPlaylist_Play("main"); // SgPlaylist_Play ("action");
		}
	}
}

void GenericOptionsInterface::ExitGameClick(Button* button) { app->Close(); }

void GenericOptionsInterface::SetOptionTYPE(const char* newtype)
{
	...
	EclRegisterButton(
		screenw - 210, screenh - 40, 200, 15, title, "Close this options screen", "generic_title");
	...
	EclRegisterButton(
		screenw - 210, screenh - 40, 145, 15, "Apply Changes", "Apply all changes", "generic_apply");
...
void GenericOptionsInterface::Create()
{
	...
	if (!IsVisible()) {
		...
		EclRegisterButton(
			screenw - 60, screenh - 40, 50, 15, "Close", "Return to Options menu", "generic_return");
...
bool GenericOptionsInterface::IsVisible() { return (EclGetButton("generic_return") != NULL); }

int GenericOptionsInterface::ScreenID() { return MAINMENU_GENERICOPTIONS; }
