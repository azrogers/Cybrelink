...
void DisavowedInterface::Update()
{
	...
	if (IsVisible()) {
		...
		if (starttime != -1 && time(NULL) > starttime + 4) {
			...
			if (strcmp(EclGetButton("disavowed")->caption.c_str(), " ") == 0) {

				EclRegisterCaptionChange(
					"disavowed",
					"We have recently been informed by a large corporation that you have \n"
				...
				EclRegisterButton(
					screenw - 40, screenh - 40, 32, 32, "", "Return to Main Menu", "disavowed_mainmenu");
				EclRegisterButtonCallback("disavowed_mainmenu", ReturnToMainMenuClick);
				button_assignbitmaps("disavowed_mainmenu",
...
bool DisavowedInterface::IsVisible() { return (EclGetButton("disavowed") != NULL); }

int DisavowedInterface::ScreenID() { return MAINMENU_DISAVOWED; }
