...
void WarezGameOverInterface::Update()
{
	...
	if (IsVisible()) {
		...
		if (starttime != -1 && time(NULL) > starttime + 4) {
			...
			if (strcmp(EclGetButton("warezgameover")->caption.c_str(), " ") == 0) {

				EclRegisterCaptionChange(
					"warezgameover",
					"Uplink Corporation has detected that you are using\n"
				...
				EclRegisterButton(
					screenw - 40, screenh - 40, 32, 32, "", "Return to Main Menu", "warezgameover_mainmenu");
				EclRegisterButtonCallback("warezgameover_mainmenu", ReturnToMainMenuClick);
				button_assignbitmaps("warezgameover_mainmenu",
...
bool WarezGameOverInterface::IsVisible() { return (EclGetButton("warezgameover") != NULL); }

int WarezGameOverInterface::ScreenID() { return MAINMENU_WAREZGAMEOVER; }
