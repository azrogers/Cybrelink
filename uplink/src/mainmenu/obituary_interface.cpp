...
void ObituaryInterface::PiClick(Button* button) { app->GetMainMenu()->RunScreen(MAINMENU_THETEAM); }
...
void ObituaryInterface::Create()
{
	...
	if (!IsVisible()) {
		...
		EclRegisterButton(
			190, 320, 100, 15, Rating::GetUplinkRatingString(gob->uplinkrating), "", "obituary_urating");
		EclRegisterButton(190,
						  340,
						  100,
						  15,
						  Rating::GetNeuromancerRatingString(gob->neuromancerrating),
						  "",
						  "obituary_nrating");
		...
		for (int i = 0; i < 16; ++i) {
			if (gob->specialmissionscompleted & (1 << i)) {
				...
				UplinkSnprintf(tooltip,
							   ...
							   PlotGenerator::SpecialMissionTitle(i));
		...
		UplinkSnprintf(highsecurityhacks,
					   sizeof(highsecurityhacks),
					   "You compromised %d high security systems",
					   gob->score_highsecurityhacks);
		UplinkSnprintf(
			livesruined, sizeof(livesruined), "You ruined the lives of %d people", gob->score_peoplefucked);
		UplinkSnprintf(systemsdestroyed,
					   sizeof(systemsdestroyed),
					   "You destroyed %d computers",
					   gob->score_systemsfucked);
		...
		button_assignbitmaps("obituary_mainmenu",
...
bool ObituaryInterface::IsVisible() { return (EclGetButton("obituary_text") != 0); }

int ObituaryInterface::ScreenID() { return MAINMENU_OBITUARY; }
