...
std::vector<std::string> ThemeInterface::themes;
...
{
	std::vector<std::string> themes = ListAvailableThemes();
	if (currentSelect < themes.size()) {
		app->GetOptions()->SetThemeName(const_cast<char*>(themes[currentSelect].c_str()));
	}
...
std::vector<std::string> ThemeInterface::ListAvailableThemes()
{
	if (themes.empty()) {

		themes = ListSubdirs(std::string(app->path));

		std::vector<std::string> validThemes;
		for (int i = themes.size() - 1; i >= 0; --i) {
			// Is there a theme in here?

			char themeFilename[256];
			UplinkSnprintf(themeFilename, sizeof(themeFilename), "%s/theme.txt", themes[i].c_str());

			if (!RsArchiveFileLoaded(themeFilename)) {
				continue;
			}

			char* lowercaseThemename = LowerCaseString(themes[i].c_str());
			if (strcmp(lowercaseThemename, "graphics") != 0) {
				validThemes.push_back(themes[i]);
			}
...
void ThemeInterface::Create()
{
	...
	if (!IsVisible()) {
		...
		EclRegisterButton(screenw - 210,
						  screenh - 40,
						  200,
						  15,
						  "THEME options",
						  "Close this options screen",
						  "theme_title");
		...
		for (int i = 0; i < themes.size(); ++i) {
			char* themeName = const_cast<char*>(themes[i].c_str());
			char bname[64];
			UplinkSnprintf(bname, sizeof(bname), "theme %d", i);
		...
		EclRegisterButton(
			screenw - 210, screenh - 40, 200, 15, "Apply", "Set the current theme", "theme_apply");
		...
		EclRegisterButton(screenw - 210,
						  ...
						  15,
						  "Restore Default",
						  "Set the theme back to standard",
						  "theme_default");
		...
		EclRegisterButton(
			screenw - 60, screenh - 40, 50, 15, "Close", "Return to Options menu", "theme_return");
		...
		button_assignbitmaps(
			"theme_hardware", "hud/hardware.tif", "hud/hardware_h.tif", "hud/hardware_c.tif");
...
bool ThemeInterface::IsVisible() { return (EclGetButton("theme_return") != NULL); }

int ThemeInterface::ScreenID() { return MAINMENU_THEME; }
