...
void RevelationLostInterface::Update()
{
	...
	if (IsVisible()) {
		...
		if (starttime != -1 && time(NULL) > starttime + 4) {
			...
			if (strcmp(EclGetButton("revelationlost")->caption.c_str(), " ") == 0) {

				EclRegisterCaptionChange(
					"revelationlost",
					"We have terminated your connection to your Gateway.\n\n"
				...
				EclRegisterButton(
					screenw - 40, screenh - 40, 32, 32, "", "Return to Main Menu", "revelationlost_mainmenu");
				EclRegisterButtonCallback("revelationlost_mainmenu", ReturnToMainMenuClick);
				button_assignbitmaps("revelationlost_mainmenu",
...
bool RevelationLostInterface::IsVisible() { return (EclGetButton("revelationlost") != NULL); }

int RevelationLostInterface::ScreenID() { return MAINMENU_REVELATIONLOST; }
