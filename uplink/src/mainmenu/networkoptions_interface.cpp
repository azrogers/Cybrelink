...
void NetworkOptionsInterface::CloseClick(Button* button) { app->GetMainMenu()->RunScreen(MAINMENU_OPTIONS); }
...
void NetworkOptionsInterface::ClientButtonClick(Button* button)
{
	...
	if (app->GetNetwork()->STATUS == NETWORK_SERVER) {
		...
		printf("NetworkOptionsInterface::ClientButtonClick, cannot connect to a server when already running "
			   "as a server\n");
		return;
	}
	...
	if (app->GetNetwork()->STATUS == NETWORK_NONE) {
		...
		app->GetNetwork()->StartClient(EclGetButton("network_clienttarget")->caption.c_str());
...
bool NetworkOptionsInterface::ReturnKeyPressed() { return true; }
...
void NetworkOptionsInterface::Create()
{
	...
	if (!IsVisible()) {
		...
		EclRegisterButtonCallbacks(
			"network_client", ClientButtonDraw, ClientButtonClick, button_click, button_highlight);
		...
		EclRegisterButton(
			screenw - 60, screenh - 40, 50, 15, "Close", "Return to Options menu", "network_close");
...
void NetworkOptionsInterface::Update()
{
	...
	char details[512];
	UplinkSnprintf(details,
				   sizeof(details),
				   "Localhost : %s\n"
				   "Local IP  : %s",
				   app->GetNetwork()->GetLocalHost(),
...
bool NetworkOptionsInterface::IsVisible() { return (EclGetButton("network_title") != NULL); }

int NetworkOptionsInterface::ScreenID() { return MAINMENU_NETWORKOPTIONS; }
