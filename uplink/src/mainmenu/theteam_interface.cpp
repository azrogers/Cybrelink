...
static char *teamwords[] = {
							 ...
							 "Some might say he was the reason why this game is so self consciously
pretentious at times.  " "Times like this in fact.\n\n" "The creative vision behind Introversion Software.\n"
							 "Its a lonely world.",
...
static const char* teamwords[] = { "The last of the bedroom programmers\n"
...
void TheTeamInterface::TeamMemberHighlight(Button* button)
{
	...
	int index;
	sscanf(button->name.c_str(), "theteam_member %d", &index);
...
void TheTeamInterface::ExitClick(Button* button) { app->GetMainMenu()->RunScreen(MAINMENU_LOGIN); }
...
void TheTeamInterface::MergeCaption(const char* buttonName, const char* targetCaption)
{

	UplinkAssert(EclGetButton(buttonName));
	const char* currentCaption = EclGetButton(buttonName)->caption.c_str();
	...
	for (size_t i = 0; i < newCaptionSize; ++i) {
		...
		if (i < targetCaptionSize) {
			...
			if (newCaption[i] != targetCaption[i]) {
				...
				if (targetCaption[i] == ' ' || targetCaption[i] == '.' || targetCaption[i] == ','
					|| targetCaption[i] == '\n') {
...
bool TheTeamInterface::IsVisible() { return (EclGetButton("theteam_member 1") != 0); }

int TheTeamInterface::ScreenID() { return MAINMENU_THETEAM; }
