...
void DemoGameOverInterface::Update()
{
	...
	if (IsVisible()) {
		...
		if (starttime != -1 && time(NULL) > starttime + 4) {
			...
			if (strcmp(EclGetButton("demogameover")->caption.c_str(), " ") == 0) {
				...
				EclRegisterButton(
					screenw - 40, screenh - 40, 32, 32, "", "Return to Main Menu", "demogameover_mainmenu");
				EclRegisterButtonCallback("demogameover_mainmenu", ReturnToMainMenuClick);
				button_assignbitmaps("demogameover_mainmenu",
...
bool DemoGameOverInterface::IsVisible() { return (EclGetButton("demogameover") != NULL); }

int DemoGameOverInterface::ScreenID() { return MAINMENU_DEMOGAMEOVER; }
