...
void RevelationWonInterface::Update()
{
	...
	if (IsVisible()) {
		...
		if (starttime != -1 && time(NULL) > starttime + 4) {
			...
			if (strcmp(EclGetButton("revelationwon")->caption.c_str(), " ") == 0) {

				EclRegisterCaptionChange("revelationwon",
				...
				EclRegisterButton(
					screenw - 40, screenh - 40, 32, 32, "", "Return to Main Menu", "revelationwon_mainmenu");
				EclRegisterButtonCallback("revelationwon_mainmenu", ReturnToMainMenuClick);
				button_assignbitmaps("revelationwon_mainmenu",
...
bool RevelationWonInterface::IsVisible() { return (EclGetButton("revelationwon") != NULL); }

int RevelationWonInterface::ScreenID() { return MAINMENU_REVELATIONWON; }
