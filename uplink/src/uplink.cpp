...
int RunUplinkExceptionHandling()
{
	...
	if (file_stdout) {
		...
		if (app && strncmp(app->userpath, "c:/", sizeof(app->userpath)) != 0) {
			fprintf(
				file_stdout, "See the %sdebug.log file for more informations on the error\n", app->userpath);
		} else {
...
bool Init_Steam()
{
	...
	if (hinstLib != NULL) {
		...
		if (NULL != ProcInitSteam) {
			...
			if (((retSeed ^ seed) & 0xFF) == 'G' && (((retSeed ^ seed) >> 8) & 0xFF) == 'O'
				&& (((retSeed ^ seed) >> 16) & 0xFF) == 'O' && (((retSeed ^ seed) >> 24) & 0xFF) == 'D') {
				...
				if (errorMsg[0] == '\0') {
					UplinkIntSnprintf(
						errorMsg, sizeof(errorMsg), "Could not initialize Steam: Unknown Error.");
				}
				errorSteam = true;
			}
			...
			if (errorSteam) {
				...
				char message[1024];
				UplinkIntSnprintf(message,
								  sizeof(message),
								  "An error occured in Uplink.\n\n"
								  "%s\n",
								  errorMsg);

				MessageBox(NULL, message, "Uplink Error", MB_ICONEXCLAMATION | MB_OK);
			}
			return !errorSteam;
		#else
			if (errorSteam) {
...
void Init_Game()
{
	...
	if (debugging) {
		printf("Init_Game called...creating game object\n");
	}
	...
	if (debugging) {
		printf("Finished with Init_Game\n");
	}
}
...
void Init_Sound()
{
	...
	if (debugging) {
		printf("Init_Sound called...setting up sound system\n");
	}

	SgInitialise();

	if (debugging) {
		printf("Finished with Init_Sound\n");
	}
}
...
void Init_Music()
{
	...
	if (debugging) {
		printf("Init_Music called...loading modules\n");
	}
	...
	*/
	...
	if (debugging) {
		printf("Finished with Init_Music\n");
	}
}
...
void Init_Fonts()
{
	...
	if (debugging) {
		printf("Init_Fonts called...setting up system fonts\n");
	}
	...
	if (!dungeonFontFilename) {
		...
		if (debugging) {
			printf("Loading system fonts into memory...\n");
		}

		success = (GciLoadTrueTypeFont(HELVETICA_10, "Dungeon", dungeonFontFilename, 9)
		...
		if (dungeonFontAdded) {
			GciUnregisterTrueTypeFont(dungeonFontFilename);
		}
		if (debugging) {
			printf("done\n ");
		}
		...
		if (crashInit != 0) {
			if (opengl_setSoftwareRendering(true)) {
				if (app->GetOptions()->GetOption("graphics_softwarerendering")) {
					app->GetOptions()->SetOptionValue("graphics_softwarerendering",
