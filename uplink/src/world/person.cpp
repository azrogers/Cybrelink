...
void Person::SetPhotoIndex(int newindex) { photoindex = newindex; }

void Person::SetVoiceIndex(int newindex) { voiceindex = newindex; }

void Person::SetLocalHost(const char* newip)
{
...
void Person::SetRemoteHost(const char* newip)
{
...
void Person::GiveMessage(Message* message) { messages.PutData(message); }

bool Person::IsConnected() { return (strcmp(localhost, remotehost) != 0); }

Connection* Person::GetConnection() { return &connection; }
...
void Person::SetCurrentAccount(int index) { currentaccount = index; }

void Person::ChangeBalance(int amount, const char* description)
{
	...
	if (accounts.Size() > 0) {
		...
		if (amount < 0 && ba->balance < 0) {
			if (this == game->GetWorld()->GetPlayer()) {
				...
				date->SetDate(
					0, 0, 0, date->GetDay(), date->GetMonth(), date->GetYear()); // Round to midnight
...
void Person::SetIsTargetable(bool newvalue) { istargetable = newvalue; }
...
bool Person::Load(FILE* file)
{
	...
	if (!LoadDynamicStringStatic(name, SIZE_PERSON_NAME, file)) {
		return false;
	}
	if (!LoadDynamicStringStatic(localhost, SIZE_VLOCATION_IP, file)) {
		return false;
	}
	if (!LoadDynamicStringStatic(remotehost, SIZE_VLOCATION_IP, file)) {
		return false;
	}
	if (!LoadDynamicStringStatic(phonenumber, SIZE_VLOCATION_IP, file)) {
		return false;
	}
	...
	if (!FileReadData(&age, sizeof(age), 1, file)) {
		return false;
	}
	if (!FileReadData(&photoindex, sizeof(photoindex), 1, file)) {
		return false;
	}
	if (!FileReadData(&voiceindex, sizeof(voiceindex), 1, file)) {
		return false;
	}
	if (!FileReadData(&currentaccount, sizeof(currentaccount), 1, file)) {
		return false;
	}
	if (!FileReadData(&istargetable, sizeof(istargetable), 1, file)) {
		return false;
	}
	if (!FileReadData(&STATUS, sizeof(STATUS), 1, file)) {
		return false;
	}

	if (!LoadLList((LList<UplinkObject*>*)&messages, file)) {
		return false;
	}
	if (!LoadLList(&accounts, file)) {
		return false;
	}
				...
				if (!handled) {
					...
					for (int i = 0; i < game->GetWorld()->GetPlayer()->missions.Size(); ++i) {
						...
						if (strcmp(mis->contact, name) == 0
							&& strstr(msg->GetBody(), mis->description) != NULL
							&& // ie the name of the mission appears in your email
...
std::string Person::GetID() { return "PERSON"; }

int Person::GetOBJECTID() { return OID_PERSON; }
