...
Computer* LanGenerator::LoadLAN(const char* filename)
{
	...
	Company* company =
		WorldGenerator::GenerateCompany(companyName, size, COMPANYTYPE_COMMERCIAL, growth, alignment);
	...
	while (thefile) {
		...
		if (strstr(systemName, "ROUTER")) {
			indexUsed = GenerateRouter(comp, cluster, x, y, security);
		} else if (strstr(systemName, "HUB")) {
			indexUsed = GenerateHUB(comp, cluster, x, y, security);
		} else if (strstr(systemName, "TERMINAL")) {
			indexUsed = GenerateTerminal(comp, cluster, x, y, security);
		} else if (strstr(systemName, "MAINSERVER")) {
			indexUsed = GenerateMainServer(comp, cluster, x, y, security);
		} else if (strstr(systemName, "AUTHENTICATION")) {
			indexUsed = GenerateAuthentication(comp, cluster, x, y, security);
		} else if (strstr(systemName, "LOCK")) {
			indexUsed = GenerateLock(comp, cluster, x, y, security);
		} else if (strstr(systemName, "ISOLATIONBRIDGE")) {
			indexUsed = GenerateIsolationBridge(comp, cluster, x, y, security);
		} else if (strstr(systemName, "SESSIONKEYSERVER")) {
			indexUsed = GenerateSessionKeyServer(comp, cluster, x, y, security);
		} else if (strstr(systemName, "RADIOTRANSMITTER")) {
			indexUsed = GenerateRadioTransmitter(comp, cluster, x, y, security);
		} else if (strstr(systemName, "RADIORECEIVER")) {
			indexUsed = GenerateRadioReceiver(comp, cluster, x, y, security);
		} else if (strstr(systemName, "FAXPRINTER")) {
			indexUsed = GenerateFaxPrinter(comp, cluster, x, y, security);
		...
		} else if (strstr(systemName, "LOGSERVER")) {
			indexUsed = GenerateLogServer(comp, cluster, x, y, security);
		}
	...
	while (thefile) {
		...
		if (!cluster->VerifyLanLink(from, fromX, fromY, to, toX, toY, security)) {
			printf("LanGenerator::LoadLAN WARNING: Invalid link, linkName(%s), from(%d), fromX(%f), "
				   "fromY(%f), to(%d), toX(%f), toY(%f), security(%d).(%s)\n",
				   linkName,
				   from,
				   fromX,
				   fromY,
...
Computer* LanGenerator::GenerateLAN(const char* companyname, int difficulty)
{
	...
	strncpy(computername, NameGenerator::GenerateLANName(companyname), MAX_COMPUTERNAME);
					...
					if (hss->systems.ValidIndex(j)) {
						...
						if (comp->screens.ValidIndex(mso->nextpage)) {
							...
							if (nextPage->GetOBJECTID() == OID_MENUSCREEN) {
								...
								printf("ERROR Screen %d Points to %d at %s",
									   i,
									   mso->nextpage,
									   comp->companyname);
							}
...
void LanGenerator::GenerateLANCluster(LanComputer* comp, int difficulty)
{
	...
	case 0:
		GenerateLAN_Level0Cluster(comp, cluster, &usedRadioFrenquencies, 10, 100, 200, 200);
		break;
	case 1:
		GenerateLAN_Level1Cluster(comp, cluster, &usedRadioFrenquencies, 10, 100, 275, 275);
		break;
	case 2:
		GenerateLAN_Level2Cluster(comp, cluster, &usedRadioFrenquencies);
		break;
	case 3:
		GenerateLAN_Level3Cluster(comp, cluster, &usedRadioFrenquencies);
		break;
	case 4:
...
void LanGenerator::GenerateLAN_Level0Cluster(LanComputer* comp,
											 LanCluster* cluster,
											 LList<int>* usedRadioFrenquencies,
											 int x,
											 int y,
											 ...
											 int height)
{
	...
	if (type == 1) {
		...
		case 0:
			cluster->SetOutput(t1Index);
			break;
		case 1:
			cluster->SetOutput(t2Index);
			break;
		case 2:
			cluster->SetOutput(t3Index);
			break;
		case 3:
			cluster->SetOutput(t4Index);
			break;
		};
...
void LanGenerator::GenerateLAN_Level1Cluster(LanComputer* comp,
											 LanCluster* cluster,
											 LList<int>* usedRadioFrenquencies,
											 int x,
											 int y,
											 ...
											 int height)
{
	...
	if (type == 1) {
		...
		char freqData[128];
		UplinkSnprintf(
			freqData, sizeof(freqData), "Radio Receiver Frequency : %d.%d Ghz", recei->data1, recei->data2);
		...
		case 0:
			...
			term = cluster->systems.GetData(t1Index);
			break;
		case 1:
			UplinkAssert(cluster->systems.ValidIndex(t2Index));
			term = cluster->systems.GetData(t2Index);
			break;
		case 2:
			UplinkAssert(cluster->systems.ValidIndex(t3Index));
			term = cluster->systems.GetData(t3Index);
			break;
		default:
			UplinkAssert(cluster->systems.ValidIndex(t3Index));
			term = cluster->systems.GetData(t3Index);
...
void LanGenerator::GenerateLAN_Level2Cluster(LanComputer* comp,
											 LanCluster* cluster,
											 LList<int>* usedRadioFrenquencies)
{
...
void LanGenerator::GenerateLAN_Level3Cluster(LanComputer* comp,
											 LanCluster* cluster,
											 LList<int>* usedRadioFrenquencies)
{
	// TODO : Fill this in
...
void LanGenerator::GenerateLAN_Level4Cluster(LanComputer* comp,
											 LanCluster* cluster,
											 LList<int>* usedRadioFrenquencies)
{
	...
	if (type == 3) {
		...
		UplinkSnprintf(data,
					   sizeof(data),
					   "Known Radio Receiver Frequencies :\n\n"
					   "Receiver A : %d.%d Ghz\n"
					   "Receiver B : %d.%d Ghz\n"
					   "Receiver C : %d.%d Ghz",
					   recei1->data1,
...
void LanGenerator::GenerateLAN_BoundingBox(
	LanComputer* comp, LanCluster* cluster, int x, int y, int width, int height)
{
...
int LanGenerator::GenerateTerminal(LanComputer* comp, LanCluster* cluster, int x, int y, int security)
{
	...
	if (security == 1) {
		ps->SetPassword(NameGenerator::GenerateEasyPassword());
	} else if (security == 2) {
		ps->SetPassword(NameGenerator::GeneratePassword());
	} else {
		ps->SetPassword(NameGenerator::GenerateComplexPassword());
	}
...
int LanGenerator::GenerateAuthentication(LanComputer* comp, LanCluster* cluster, int x, int y, int security)
{
	...
	ds->AddWidget("enable",
				  ...
				  "Enable the locks on this Local Area Network",
				  80,
				  dsIndex,
				  NULL,
				  NULL);
	ds->AddWidget("disable",
				  WIDGET_SCRIPTBUTTON,
				  50,
				  240,
				  150,
				  20,
				  "Disable security locks",
				  "Disable the locks on this Local Area Network",
				  81,
				  dsIndex,
				  NULL,
				  NULL);
...
int LanGenerator::GenerateIsolationBridge(LanComputer* comp, LanCluster* cluster, int x, int y, int security)
{
	...
	ds->AddWidget("enable",
				  ...
				  150,
				  20,
				  "Enable Isolation Bridge",
				  "Enable the bridge (disconnecting Router)",
				  82,
				  -1,
				  NULL,
				  NULL);
	ds->AddWidget("disable",
				  ...
				  150,
				  20,
				  "Disable Isolation Bridge",
				  "Disable the bridge (enable Router)",
				  83,
				  -1,
				  NULL,
				  NULL);
...
int LanGenerator::GenerateModem(
	LanComputer* comp, LanCluster* cluster, int x, int y, int security, char* phoneNum, size_t phoneNumsize)
{
	...
	if (phoneNum) {
		UplinkStrncpy(phoneNum, ip, phoneNumsize);
	}
	...
	char textMsg[512];
	UplinkSnprintf(textMsg,
				   sizeof(textMsg),
				   "LAN DIAL-UP ACCESSED\n\n"
...
int LanGenerator::GetRadioFrequency(LList<int>* usedRadioFrenquencies)
{
	...
	int newFrequency = -1;
	do {
		newFrequency = RADIOTRANSMITTER_MINRANGE
			+ NumberGenerator::RandomNumber(RADIOTRANSMITTER_MAXRANGE - RADIOTRANSMITTER_MINRANGE);
...
LanCluster::~LanCluster() { DeleteDArrayData((DArray<UplinkObject*>*)&links); }

int LanCluster::AddLanSystem(LanComputerSystem* system) { return systems.PutData(system); }
...
int LanCluster::AddLanSystem(int TYPE, int x, int y, int security, int screenIndex, int dataScreenIndex)
{
	...
	if (TYPE == LANSYSTEM_ROUTER) {
		system->visible = LANSYSTEMVISIBLE_TYPE;
	}
	return AddLanSystem(system);
}

int LanCluster::AddLanLink(int from, float fromX, float fromY, int to, float toX, float toY, int security)
{
	LanComputerLink* link = new LanComputerLink();
	link->from = from;
	link->to = to;
...
bool LanCluster::VerifyLanLink(int from, float fromX, float fromY, int to, float toX, float toY, int security)
{
	if (!systems.ValidIndex(from) || !systems.ValidIndex(to)) {
		return false;
	}

	if (0.0f > fromX || fromX > 1.0f || 0.0f > fromY || fromY > 1.0f || 0.0f > toX || toX > 1.0f || 0.0f > toY
		|| toY > 1.0f) {
		return false;
	}
...
void LanCluster::Merge(LanCluster* target, LanComputer* comp)
{
			...
			if (system->TYPE == LANSYSTEM_MODEM) {
				...
				if (comp->screens.ValidIndex(system->screenIndex)) {
					int targetScreenIndex =
						((UserIDScreen*)comp->screens.GetData(system->screenIndex))->nextpage;
	...
	for (int j = 0; j < links.Size(); ++j) {
		if (links.ValidIndex(j)) {
			...
			target->AddLanLink(newIndexes[link->from],
							   link->fromX,
							   link->fromY,
							   newIndexes[link->to],
							   link->toX,
	...
	for (int l = 0; l < systems.Size(); ++l) {
		if (systems.ValidIndex(l)) {
			...
			if (system->TYPE == LANSYSTEM_AUTHENTICATION || system->TYPE == LANSYSTEM_ISOLATIONBRIDGE
				|| system->TYPE == LANSYSTEM_MAINSERVER) {

				if (system->data1 != -1) {
					system->data1 = newIndexes[system->data1];
				}
				if (system->data2 != -1) {
					system->data2 = newIndexes[system->data2];
				}
				if (system->data3 != -1) {
					system->data3 = newIndexes[system->data3];
				}
			}
	...
	if (input != -1) {
		input = newIndexes[input];
	}
	if (output != -1) {
		output = newIndexes[output];
	}
		...
		if (systems.ValidIndex(i)) {
			...
			if (system->TYPE == LANSYSTEM_MODEM) {
				...
				if (target->screens.ValidIndex(system->screenIndex)) {
					int targetScreenIndex =
						((UserIDScreen*)target->screens.GetData(system->screenIndex))->nextpage;
					DArray<VLocation*>* locations = game->GetWorld()->locations.ConvertToDArray();
					for (int i = 0; i < locations->Size(); ++i) {
						if (locations->ValidIndex(i)) {
							if (locations->GetData(i)->GetOBJECTID() == OID_VLOCATIONSPECIAL) {
