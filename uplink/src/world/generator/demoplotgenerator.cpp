...
DemoPlotGenerator::DemoPlotGenerator() { scene = 0; }
...
Mission* DemoPlotGenerator::Generate_ChangeSocialRecordARC()
{
	...
	fulldetails
		<< "Below you will find the personal details of our target. We have selected this man "
		...
		<< newnumber << ".  We will take care of the rest.\n";
...
void DemoPlotGenerator::PlayerCompletedMission(Mission* mission)
{
	...
	if (mission->TYPE == MISSION_CHANGEDATA && strcmp(mission->completionA, IP_ACADEMICDATABASE) == 0
		&& scene == 0) {
...
void DemoPlotGenerator::GenerateNewMission(bool randomdate)
{
	...
	if (playerrating == 2) {
		...
		if (numm1 == 0) {
			...
			if (randomdate) {
				MoveNewMissionToRandomLocation(m);
			}
		}
...
void DemoPlotGenerator::RunScene(int newscene)
{
	...
	if (scene == 1) {
		...
		newsdetails << "An Uplink Agent going by the handle of " << agenthandle
					...
					<< arcmissiontarget
					<< " and he had been working for the controversial "
					   "Andromeda Research Corporation, otherwise known as ARC.  ARC have been in the news "
					   "in previous months, "
					   "responding to allegations that they have been developing some kind of weapon using "
					   "the expertise of new and naive Uplink Agents.\n"
					   ...
					   "ARC released a statement earlier today, stating 'He was the best programmer we had "
					   "ever seen and "
					   "was a lead player in the project.  For something like this happen is a tradgedy of "
					   "the greatest order.  "
					   "Our thoughts go out to this mans family and friends.'\n\n"
					   "Due to the seriousness of these claims, Uplink Corporation has asked all agents not "
					   "to accept work from ARC while this issue is resolved."
					<< '\x0';
...
std::string DemoPlotGenerator::GetID() { return "DEMOPGEN"; }
