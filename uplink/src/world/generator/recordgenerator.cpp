...
void RecordGenerator::GenerateRecord_SocialSecurity(char* personname, int age)
{
	...
	UplinkSnprintf(dob,
				   sizeof(dob),
				   "%d - %d - %d",
				   NumberGenerator::RandomNumber(28) + 1,
				   NumberGenerator::RandomNumber(12) + 1,
				   2010 - age);

	char socialsecurity[16];
	UplinkSnprintf(
		socialsecurity, sizeof(socialsecurity), "%d", 10000000 + NumberGenerator::RandomNumber(99999999));

	char maritalstatus[64];

	switch (NumberGenerator::RandomNumber(5)) {
	case 0:
		UplinkStrncpy(maritalstatus, "Single", sizeof(maritalstatus));
		break;
	case 1:
		UplinkStrncpy(maritalstatus, "Married", sizeof(maritalstatus));
		break;
	case 2:
		UplinkStrncpy(maritalstatus, "Seperated", sizeof(maritalstatus));
		break;
	case 3:
		UplinkStrncpy(maritalstatus, "Divorced", sizeof(maritalstatus));
		break;
	case 4:
		UplinkStrncpy(maritalstatus, "Widowed", sizeof(maritalstatus));
		break;
	}
	...
	switch (NumberGenerator::RandomNumber(5)) {
	case 0:
		UplinkStrncpy(personalstatus, "Employed", sizeof(personalstatus));
		break;
	case 1:
		UplinkStrncpy(personalstatus, "Un-Employed", sizeof(personalstatus));
		break;
	case 2:
		UplinkStrncpy(personalstatus, "Student", sizeof(personalstatus));
		break;
	case 3:
		UplinkStrncpy(personalstatus, "Self-Employed", sizeof(personalstatus));
		break;
	case 4:
		UplinkStrncpy(personalstatus, "Deceased", sizeof(personalstatus));
		break;
	}
	...
	if (person->rating.uplinkrating == 0) {
		...
		if (NumberGenerator::RandomNumber(100) < PERCENTAGE_PEOPLEWITHCONVICTIONS) {
			...
			for (int i = 0; i < numconvictions; ++i) {
				...
				case 7:
					break;
				case 8:
					convictions << "Disturbing the peace\n";
					numconv++;
					break;
				case 9:
					convictions << "Reckless driving\n";
					numconv++;
					break;
				case 10:
					convictions << "Man-slaughter\n";
					numconv++;
					break;
				case 11:
					convictions << "G-B-H\n";
		...
		if (NumberGenerator::RandomNumber(100) < PERCENTAGE_AGENTSWITHCONVICTIONS) {
			...
			for (int i = 0; i < numconvictions; ++i) {
				...
				case 0:
					convictions << "High tech fraud\n";
					numconv++;
					break;
				case 1:
					convictions << "Willful destruction of data\n";
					numconv++;
					break;
				case 2:
					convictions << "Theft of sensitive data\n";
					numconv++;
					break;
				case 3:
					convictions << "Unlawful access of classified data\n";
					numconv++;
					break;
				case 4:
					break;
				...
				case 7:
					break;
				case 8:
					convictions << "Falsifying academic results\n";
					numconv++;
					break;
				case 9:
					convictions << "Illegal modification of credit rating\n";
					numconv++;
					break;
				case 10:
					convictions << "Illegally transfering funds\n";
					numconv++;
					break;
				case 11:
					...
					break;
				case 12:
					convictions << "Destruction of financial data\n";
					numconv++;
					break;
...
void RecordGenerator::GenerateRecord_Academic(char* personname, int age)
{
	...
	int brains =
		(int)NumberGenerator::RandomNormalNumber(50, 50); // 0 = thick, 50 = average, 100 = rocket scientist
	...
	if (age >= 16) {
		...
		std::vector<std::string> possibleequals;
		possibleequals.push_back("Science");
		possibleequals.push_back("Maths");
		possibleequals.push_back("Physics");
		possibleequals.push_back("Biology");
		possibleequals.push_back("Art");
		possibleequals.push_back("History ");
		possibleequals.push_back("Psychology");
		possibleequals.push_back("Electronics");
		possibleequals.push_back("Computing");
		possibleequals.push_back("Media studies");
		possibleequals.push_back("Sociology");
		...
		if (numquals > 0) {
			...
			for (int i = 0; i < numquals; ++i) {
				...
				int thisqualindex = NumberGenerator::RandomNumber(possibleequals.size());
				...
				collegequals << possibleequals[thisqualindex] << "      " << grades[grade] << "\n";

				possibleequals.erase(possibleequals.begin() + thisqualindex);
			}
	...
	if (age > 18 && brains > 50) {
		...
		case 3:
			...
			break;
		case 4:
			UplinkStrncpy(qualification, "Systems Engineering diploma", sizeof(qualification));
			break;
...
void RecordGenerator::GenerateRecord_Financial(char* personname, int age)
{
	...
	int financial_situation =
		(int)NumberGenerator::RandomNormalNumber(10, 10); // 0 = fucked, 10 = average, 20 = Gates

	int balance = (int)((financial_situation - 6) * NumberGenerator::RandomNormalNumber(1000, 500));
	int loan = financial_situation < 10
		? (int)(NumberGenerator::RandomNormalNumber(10.0f - financial_situation, 5.0f) * 1000)
		: 0;
	loan = (loan / 100) * 100;
	if (loan < 0) {
		loan = 0;
	...
	UplinkAssert(person);
	int accno =
		person->CreateNewAccount(bank->ip, personname, NameGenerator::GeneratePassword(), balance, loan);
	...
	if (myaccount->balance > 0) {
		...
		while (transferdate.Before(&nomoredate)) {
			...
			if (target_acc) {
				...
				if (strcmp(target_acc->name, game->GetWorld()->GetPlayer()->handle)
					!= 0) { // Prevent transfers to player

					if (target_acc != myaccount) { // Prevent transfers to my own account

						int amount = (int)NumberGenerator::RandomNormalNumber(myaccount->balance * 0.3f,
																			  myaccount->balance * 0.2f);
						...
						if (amount > 0) {
							...
							char s_amount[16];
							UplinkSnprintf(source_data,
										   sizeof(source_data),
										   "%s %d",
										   target_bank->ip,
										   target_acc->accountnumber);
							UplinkSnprintf(target_data,
										   sizeof(target_data),
										   "%s %d",
										   bank->ip,
							...
							source_log->SetProperties(&transferdate,
													  bank->ip,
													  person->name,
													  LOG_NOTSUSPICIOUS,
													  LOG_TYPE_TRANSFERTO);
							...
							target_log->SetProperties(&transferdate,
													  bank->ip,
													  person->name,
													  LOG_NOTSUSPICIOUS,
													  LOG_TYPE_TRANSFERFROM);
...
Record* RecordGenerator::GetSocialSecurity(char* personname)
{
	...
	char* name = (strcmp(personname, "PLAYER") == 0 ? game->GetWorld()->GetPlayer()->handle : personname);
...
Record* RecordGenerator::GetCriminal(const char* personname)
{
	...
	const char* name =
		(strcmp(personname, "PLAYER") == 0 ? game->GetWorld()->GetPlayer()->handle : personname);
