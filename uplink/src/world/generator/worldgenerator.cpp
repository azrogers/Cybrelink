...
void WorldGenerator::GenerateRandomWorld()
{
	...
	for (i = 0; i < companies->Size(); ++i) {
		...
		if (companies->ValidIndex(i)) {
			...
			if (strcmp(company->name, "Government") != 0 && strcmp(company->name, "Uplink") != 0
...
void WorldGenerator::GenerateSimpleStartingMissionA()
{
	...
	while (!employer1 || !target1 || strcmp(employer1->name, target1->companyname) == 0) {
...
void WorldGenerator::GenerateSimpleStartingMissionB()
{
	...
	while (!employer2 || !target2 || strcmp(employer2->name, target2->companyname) == 0) {
	...
	if (mission2) {
		mission2->SetMinRating(2);
	}
}

Player* WorldGenerator::GeneratePlayer(const char* handle)
{
...
void WorldGenerator::GenerateLocalMachine()
{
	...
	dlg3->AddWidget(
		"okbutton", WIDGET_NEXTPAGE, 300, 350, 50, 20, "OK", "Click to continue", 0, 0, NULL, NULL);
	...
	dlg6->AddWidget("text2",
					WIDGET_CAPTION,
					100,
					210,
					340,
					15,
					"UPLINK Operating System successfully started on Gateway",
					"");
	dlg6->AddWidget(
		"okbutton", WIDGET_NEXTPAGE, 300, 350, 50, 20, "OK", "Click to continue", 7, 0, NULL, NULL);
	...
	dlg7->AddWidget(
		...
		"in the future.  When you next start Uplink on your home computer, you will be connected "
		"automatically "
		"to this Gateway.\n\n"
		...
		"");
	dlg7->AddWidget(
		"runtutorial", WIDGET_SCRIPTBUTTON, 300, 350, 50, 20, "Yes", "Run the tutorial", 43, 0, NULL, NULL);
	dlg7->AddWidget("dontruntutorial",
					WIDGET_NEXTPAGE,
					200,
					350,
					50,
					20,
					"No",
					"Skip the tutorial and return to the main screen",
					0,
					0,
					NULL,
					NULL);
	...
	ms10->SetTextMessage(
		"Congratulations agent - your are now using your new gateway and your old one is already "
		"being dismantled.\n\n"
...
void WorldGenerator::GenerateUplinkPublicAccessServer()
{
	...
	strncpy(computername, NameGenerator::GeneratePublicAccessServerName("Uplink"), MAX_COMPUTERNAME);
	...
	body2 << "Your Uplink membership package includes : \n\n"
			 ...
			 "computer when you are working for Uplink.  You can have it upgraded at a later stage if "
			 "necessary.\n\n"
			 "- A low interest loan of "
		  << PLAYER_START_BALANCE
		  << " credits with Uplink International Bank, "
			 "to get you started.\n\n"
	...
	// delete [] body2.str ();
	dlg2->AddWidget(
		"cancel", WIDGET_NEXTPAGE, 160, 380, 100, 20, "Cancel", "Cancel registration", 1, 0, NULL, NULL);
	dlg2->AddWidget("continue",
					WIDGET_NEXTPAGE,
					270,
					380,
					100,
	...
	dlg3->AddWidget("password",
					...
					20,
					"Enter your password",
					"You will need this to access the Uplink Services Machine");
	dlg3->AddWidget(
	...
	dlg3->AddWidget("moretext",
					WIDGET_CAPTION,
					80,
					270,
					360,
					100,
					"Uplink Corporation will not ask for any more personal details.  "
	...
	body3
		<< "Registration is now taking place.\n\n"
		   ...
		   "knowledge of your actions and will destroy your account.  Your Gateway machine will also be "
		   "destroyed.\n\n"
		   "Rental of your Gateway computer will cost "
		<< COST_UPLINK_PERMONTH << " credits a month." << '\x0';
	dlg5->AddWidget("caption1", WIDGET_CAPTION, 100, 130, 380, 270, body3.str(), "");
	body3.rdbuf()->freeze(0);
	// delete [] body3.str ();

#ifdef TESTGAME
	dlg5->AddWidget("continue",
					WIDGET_SCRIPTBUTTON,
					270,
					380,
					100,
	...
	dlg10->AddWidget("yes",
					 ...
					 20,
					 "YES",
					 "Click here to rent a new Gateway",
					 50,
					 4,
					 NULL,
					 NULL);
	dlg10->AddWidget(
		"no", WIDGET_SCRIPTBUTTON, 300, 380, 100, 20, "NO", "Click here to log out", 51, -1, NULL, NULL);
...
void WorldGenerator::GenerateUplinkInternalServicesSystem()
{
	...
	comp->SetTraceAction(COMPUTER_TRACEACTION_DISCONNECT | COMPUTER_TRACEACTION_LEGAL);
	...
	ms22->SetTextMessage(
		"When you connect to a computer from your links screen, a direct connection is established between  "
		"your Gateway and the computer.  This type of connection is very easy to trace back to your Gateway. "
		" "
		"If a connection is traced back to your Gateway, you will receive blame for the crime and risk legal "
		"action.  "
		"\n\nThis section of the help files will teach you to establish safe connections that cannot be "
		"traced back to you.");
	...
	ms23->SetTextMessage(
		"The first step in establishing a secure untracable connection is to route your connection through "
		...
		"\n\nCall bouncing can be set up from the main map screen - simply click on each location to route "
		"the "
		"call through, clicking finally on the target as the final node in the chain.  Then click on the "
		"connect "
	...
	ms24->SetTextMessage(
		"Whenever you begin to act suspiciously, the target machine will begin to trace you.  Many computers "
		...
		"\n\nWhilst you are connected you will be traced with an Active Trace - the agents on the target "
		"machine "
		"trace the physical lines that you are using.  This method can result in a trace back to your "
		"Gateway in "
		"as little as one minute.  You can see an active trace as it happens on your mini-map, which should "
		"provide "
		"ample warning before the trace completes.  You are strongly advised to disconnect before an active "
		"trace is completed.");
	...
	ms25->SetTextMessage("Whenever you use call bouncing to connect to a computer, you will leave logs "
						 "behind on each computer "
						 "used.  Normally this is not a problem, but this information can be used against "
						 "you.  If you disconnect "
						 "before an active trace has completed, the company that owned that computer may "
						 "begin a Passive Trace. "
						 "\n\nThey will dig up any logs left by you and trace you that way.  It can take "
						 "longer - usually several hours, "
						 "but the results can be just as severe.  Because of this, you should make every "
						 "effort to delete the logs you leave."
						 "\n\nYou never need to delete all the logs on every system you bounce through.  "
						 "Deleting the logs on one system "
	...
	ms26->SetTextMessage("Whilst you are hacking, it is inevitable that people will attempt to trace you.  "
						 "It is very important "
						 "that you avoid being traced - this of course requires that you know how close you "
						 "are to being traced "
						 "at any point."
						 "\n\nIf you purchase the relevent HUD upgrade (from the software sales screen), the "
						 "mini-map will show you "
						 "the progress of any Active Traces visually.  You can also purchase software "
						 "such as the Trace Tracker which will sit in the corner of your screen and will "
						 "report on the progress "
						 "of any active traces.  Some later versions even report the time you have remaining "
	...
	ms27->SetTextMessage(
		"There are several methods of preventing Passive traces from coming back to you.  Here are some "
		"suggestions. \n"
		"First, it is best to delete any logs left behind on the target machine.  This is not always "
		"possible though, "
		"(you may be forced to disconnect quickly) and it is not always successful.\n\n"
		"If you can establish one link in the chain on a computer that you can access easily, you can erase "
		"the logs "
		"on that machine after every call bounce.  This requires admin access on a low security system.  "
		"Academic "
		"systems are ideal for this due to the low consequence if caught.\n\n"
		"Another common trick is to route through a financial or government system.  These companies are "
		"typically very "
	...
	ms29->SetTextMessage("When an Uplink agent logs on to his local machine, the Uplink software on his "
						 "computer establishes a direct "
						 "connection to his Gateway computer and runs the Uplink Operating System on that "
						 "Gateway.  This allows each "
						 "agent to retain a safe distance from his jumping off point onto the Web.  This "
						 "gives Uplink Agents virtual "
						 "anonymity in the world - there is no way to trace an agent from a Gateway.\n\n"
						 "A gateway also provides the processor power, storage and high bandwidth connection "
						 "necessary for an Uplink "
						 "agent to complete his tasks.\n"
						 "Gateway computers are rented to Uplink Agents as part of the contract and are "
						 "stored in secure server rooms "
						 "around the world.");
	...
	ms30->SetTextMessage("A gateway computer gives total physical anonymity to all Uplink Agents who use "
						 "them.  A gateway computer is "
						 "programmed to automatically disconnect any connection that has been traced back to "
						 "the Gateway, thus ensuring "
						 "that an agents physical location can never be determined.  Gateway computers keep "
						 "no logs of access.  Further more, "
						 "there is no record of which real person uses which username.  This means that, in "
						 "a worse case scenario when a connection "
						 "is traced back to a Gateway, any federal agents inspecting the Gateway will be "
						 "able to determine the username but "
						 "not the real person who uses that username.\n"
						 "Unfortunately, when this happens it is often necessary for Uplink Corporation to "
						 "disavow the agent in question, "
						 "in order to avoid any legal consequences.  That user's account and rating will be "
						 "deleted, and his gateway will be "
	...
	ms31->SetTextMessage("Uplink Corporation owns several large offices around the world, which are used to "
						 "house hundreds of Gateway computers.  "
						 "These computers are openly connected to the phone network, allowing Agents to "
						 "connect to them from any local computer.  "
						 "The buildings are under 24 hour guard.  Engineering teams from Uplink Corporation "
						 "routinely service the computers inside, "
						 "installing new hardware when requested.\n\n"
						 "As such, the physical specifications of each machine can vary drastically.  With "
						 "enough money, an Uplink Agent could upgrade "
						 "his Gateway to enormous power.  Typically a Gateway contains an inbound modem (to "
						 "allow the agent to dial in), a high speed "
						 "processor and large quantities of memory storage, and a high speed broadband "
	...
	ms32->SetTextMessage(
		"You can order new hardware upgrades for your Gateway from the Hardware Menu on the Uplink Internal "
		"Services Machine.  "
		"All hardware will be installed within 24 hours by a certified Uplink technician.  Available "
		"upgrades include faster "
		"processors, more memory capacity, higher speed connections to the Net, and many more.\n\n"
		"You can also add extra security to your Gateway by buying security camera's, laser trip wires etc, "
		"which will inform you "
		"if your Gateway is about to be seized by Federal Agents.  This can give you warning, allowing you "
		"to destroy any incriminating "
		"evidence.  You can even fit your Gateway to self-destruct on your command.  In this case, Federal "
		"Agents have no way of "
		"finding out who was using the Gateway - allowing you to keep your username and established "
		"reputation.  ");
	...
	ms36->SetTextMessage(
		"As you accept more work from Uplink Corporation you will encounter more and more security systems "
		"of different types and strengths.  "
		"All security systems are designed with one primary goal - to keep you out, and as an Agent you must "
		"learn to bypass them.\n\n"
		"Most systems can be simply disabled once Administrative access has been gained, but this method is "
		"considered unprofessional and is "
	...
	ms37->SetTextMessage(
		"The primary purpose of the Proxy server is to stop unauthorised users from modifying records or "
		"logs.  A system with a good Proxy server "
		"will permit you to examine records and logs but will not allow you to change them.  As such, you "
		"will not be able to remove any logs of "
		"your actions, nor will you be able to make any alterations to any records on the target system.\n\n"
		"Proxy servers are extremely popular on government data systems (such as the social security "
		"database) and other systems were the data "
		"itself is sensitive.  They can be bypassed simply by disabling them on the target computer (which "
		"will typically require admin access) "
		"or by running a Proxy_Disable tool.  Both of these methods will cause you to get noticed.  You can "
		"also bypass them silently with more "
	...
	ms38->SetTextMessage("The primary purpose of the firewall is to prevent unauthorised usage of files on "
						 "the target system.  A good Firewall will stop you from "
						 "copying, deleting or downloading any files in existence on the target computer.\n\n"
						 "Firewalls are common on File Servers or any system where the contents of the files "
						 "are sensitive or valuable. They can be bypassed "
						 "simply by disabling them on the target computer (which will typically require "
						 "admin access) or by running a Firewall_Disable tool.  "
						 "Both of these methods will cause you to get noticed.  You can also bypass them "
						 "silently with more advanced software tools and a "
						 "Connection Analyser.");
	...
	ms41->SetTextMessage("This section of the help documents is designed to help new Uplink Agents with "
						 "their first few hours online.\n\n"
						 "We will start with the Uplink Test Mission.  You will notice this mission has "
						 "already been given to you (it is given to all new agents) "
						 "and you can read the details by clicking on its icon at the bottom right of the "
						 "screen.  This mission is the perfect opportunity to "
						 "test yourself against real computer defenses, without risking jail time.\n\n"
						 "We will go over the basic equipment needed, how to do the mission, and how to "
						 "aquire more missions afterwards.");
	...
	ms42->SetTextMessage(
		"The basic tools that you will require for the Uplink Test mission will also be required for almost "
		"every other type of mission, so you "
		"will not be wasting your money when you buy these.  We recommend you purchase these tools "
		"immediately as they will be essential.\n\n"
		...
		"Trace Tracker (V2.0) - Will allow you to monitor your communication lines, so you will know when "
		"you are about to be traced.  "
		"We recommend version 2.0 for beginners as it will show you the percentage of your connection that "
		"has "
		"been traced.\n\n"
		"You can purchase all of these items with your starting loan, from the 'Software Sales' section of "
		"this site.");
	...
	ms43->SetTextMessage("The Uplink Test Mission requires you to connect to the Uplink Test Machine and "
						 "steal a file.  You should not connect directly to it - "
						 "they would be able to trace you very quickly.  Instead, you should bounce our call "
						 "around the world.\n"
						 "Open the Communications map by clicking on the world map, top right.  Click on a "
						 "few other nodes to bounce through them.  Bounce through "
						 "at least 3 other nodes before the Uplink Test Machine.  Click on the connect "
						 "button to connect to the target computer.\n\n"
						 "Now would be a good time to start up your TraceTracker  (click the software menu "
						 "bottom left).  This tool will tell you if you are being traced.  "
						 "Once you start hacking, you'll have a couple of minutes before they trace you and "
						 "cut you off.  In the real world, this would get you in trouble.");
	...
	ms44->SetTextMessage("Once connected to the Uplink Test Machine, you will see the main log in screen.  "
						 "Since you do not have security clearence to use this machine, "
						 "you will need to hack through this screen.  Passwords can be broken in one of two "
						 "ways - either using a dictionary hacker (which tries every english "
						 "word from a large dictionary) or a password cracker, which takes longer but can "
						 "break non-english-word passwords.  \n\n"
						 "Run your Password Breaker and target the password box.  It will take around 10 "
						 "seconds to break this password (security is low) and they "
	...
	ms45->SetTextMessage(
		"Once you are in the system, you should have no trouble accessing the file server and downloading "
		"the file you need.  (Use the file copier and download "
		"the file 'uplink test data' into your memory banks.)  In many cases this might not be so easy - you "
		"may find that Firewall or Proxy systems are running, "
		...
		"You can now send it to the employer (by clicking on 'reply' in the mission screen) and receive your "
		"payment, which in this case "
	...
	ms46->SetTextMessage("The final step in any hack is to cover your tracks.  "
						 "Access the log banks and run your log deleter.  Remember to keep an eye on the "
						 "trace tracker.  Locate the logs that you have created from "
						 "your actions (especially the ones that detail where your connection came from) and "
						 "target your log deleter on them.  The logs will be "
						 "deleted but will leave a clear marker behind.  More advanced versions of the log "
						 "deleter can cover your tracks better, but this will be "
						 "sufficient for now.\n\n"
						 "In the real world of hacking, you would probably not delete the logs on the target "
						 "machine - you would not have time.  Instead, you would "
						 "bounce your connection through a system on which you have admin access.  After the "
						 "hack, you would connect to that system and remove those "
	...
	ms47->SetTextMessage("The Uplink test mission is the easiest mission you will ever attempt.  Once you "
						 "feel good enough, connect to the Uplink Internal Services "
						 "Machine and go to the Missions section.  You will see some missions that are "
						 "already available.  You may be able to auto-accept some of them, "
						 "and you will need to contact the employer to accept others.  As your rating "
						 "increases, more and more missions will appear here.  You will find "
						 "that you will need to upgrade your hardware and software before the more complex "
	...
	ms48->SetTextMessage(
		"There are several important points to keep in mind when setting up connections. If done correctly, "
		"you can give yourself the extra few "
		"seconds need to get the job done without being caught.\n\n"
		"Firstly, bouncing through a random computer will not work against any reasonably secure system. "
		"They will be able to trace through that link in seconds.\n"
		"If you bounce through a system that you have already compromised, and have an access account "
		"available, the trace will take much longer.\n"
		"If you bounce through a system on which you have Admin access, the trace will take longer still.\n"
		"In general, the higher security the system you bounce through, the longer it will take to trace "
		"through that link.\n\n"
		"It is worth keeping in mind that you can lose your username and password login on a system if you "
		"route through it too many times.");
	...
	ms49->SetTextMessage(
		"Monitor systems are designed to detect unauthorised activity. They are usually triggered when a "
		"user attempts to hack a password, "
		"or makes use of the Admin account, or starts deleting files. Once they have detected illegal "
		"activity they will sound the alarm and "
		"start an Active Trace on your connection. They will also flag your logs as suspicious.\n\n"
		"Monitor systems will also periodically check the other security systems running locally, to see if "
		"they have been disabled.\n\n"
		"You can bypass a monitor system with a connection Analyser HUD upgrade and a Monitor Bypass.  This "
		"will allow you to hack without being noticed.");
	...
	ms50->SetTextMessage("Many systems that require a specific person to log in use Voice Print technology.  "
						 "The users voice is first sampled and analysed "
						 "by the company, then digitised and encrypted onto their system.  When that person "
						 "logs in, he reproduces a certain phrase or set "
						 "of words.  These are then analysed and compared.\n\n"
						 "Recent advances in this area have made it very difficult to crack.  For many years "
						 "Digital Approximation software was capable of "
						 "defeating these systems, but this is no longer the case.  In most modern systems, "
						 "the only real way is to obtain a sample of the users "
						 "voice directly.  Agents can then use Voice Analysis software to reproduce the "
						 "phrase digitally from that sample.");
	...
	ms51->SetTextMessage(
		"Faced with the growing threat of hackers, many companies have begun using Elliptic Curve "
		"Cryptography.  A user on a system must "
		"input a large block of data in sequence, synchronised with the system.  This cannot be done by hand "
		"- the timings must be perfect. "
		"Elliptic Curve Cryptography is popular due to the large amount of CPU power needed to force crack "
		"it.\n\n"
		"However, as with all cryptography, even this system can be broken given enough time and CPU cycles. "
		" Agents can use the Decypher "
...
void WorldGenerator::GenerateUplinkTestMachine()
{
	...
	ms->SetTextMessage(
		"This will be your first opportunity to hack into a real computer system with real defenses. "
		"There is no point in trying to hack a real computer system until you can do the Uplink Test in your "
		"sleep.\n"
		"\nYou will be traced during your attempts, so remember to monitor the communications lines, however "
		"no action will be taken against you should you be traced.\n"
		...
		"Remember that you will leave logs on the computers you use as relays (which can be traced back to "
		"you) - ideally your connection should use a computer which allows you easy and risk free log "
		"access, "
...
void WorldGenerator::GenerateGlobalCriminalDatabase()
{
	...
	ds7->AddWidget("name",
				   ...
				   15,
				   "Enter name here",
				   "Type the name of the person to search for here");
	ds7->AddWidget(
		"OK", WIDGET_SCRIPTBUTTON, 150, 240, 80, 15, "OK", "Click here when done", 15, -1, NULL, NULL);
	ds7->AddWidget("Cancel",
				   WIDGET_NEXTPAGE,
				   270,
				   240,
				   80,
...
void WorldGenerator::GenerateInternationalSocialSecurityDatabase()
{
	...
	comp->SetTraceAction(COMPUTER_TRACEACTION_DISCONNECT | COMPUTER_TRACEACTION_FINE);
	comp->SetIsTargetable(false);
	comp->security.AddSystem(SECURITY_TYPE_PROXY, 1);
	comp->security.AddSystem(SECURITY_TYPE_MONITOR, 1);
	game->GetWorld()->CreateComputer(comp);
	...
	ds5->AddWidget("name",
				   ...
				   "Type the name of the person to search for here");
	ds5->AddWidget(
		"OK", WIDGET_SCRIPTBUTTON, 150, 240, 80, 15, "OK", "Click here when done", 17, -1, NULL, NULL);
	ds5->AddWidget("Cancel",
				   WIDGET_NEXTPAGE,
				   270,
				   240,
				   80,
...
void WorldGenerator::GenerateCentralMedicalDatabase()
{
	...
	comp->SetTraceAction(COMPUTER_TRACEACTION_DISCONNECT | COMPUTER_TRACEACTION_WARNINGMAIL);
	comp->SetIsTargetable(false);
	comp->security.AddSystem(SECURITY_TYPE_PROXY, 1);
	comp->security.AddSystem(SECURITY_TYPE_MONITOR, 1);
	game->GetWorld()->CreateComputer(comp);
...
void WorldGenerator::GenerateInternationalAcademicDatabase()
{
	...
	ds5->AddWidget("name",
				   ...
				   "Type the name of the person to search for here");
	ds5->AddWidget(
		"OK", WIDGET_SCRIPTBUTTON, 150, 240, 80, 15, "OK", "Click here when done", 16, -1, NULL, NULL);
	ds5->AddWidget("Cancel",
				   WIDGET_NEXTPAGE,
				   270,
				   240,
...
void WorldGenerator::GenerateStockMarket()
{
	...
	ds2->AddWidget(
		"create", WIDGET_SCRIPTBUTTON, 250, 330, 80, 20, "Create", "Create this account", 12, -1, "", "");
...
void WorldGenerator::GenerateIntroversion()
{
	...
	ms5->SetTextMessage(
		"One possible direction for Uplink to go is shown in the film Mission:Impossible, "
	...
	ms6->SetTextMessage(
		"Around 15 years ago a film was released in what was then called Hollywood, called Johnny Mnemonic. "
		...
		"Nevertheless, Uplink2 would look fantastic with a similar look. The Internet would be rendered in "
		"3D, "
		"with major corporate computer systems appearing as huge 3D sky-scrapers.  Every tool and action "
		"would "
		"be extremely visual.\n\n"
		"It is thought that this feel would give Uplink2 a broader appeal, but would alienate the hard-core "
		"Uplink "
		"player who values its (semi) realism. In addition, it would require a team of artists to produce "
		"the 3d models.");
	...
	ms7->SetTextMessage(
		"Assasination mission. The target is flying on a chartered flight. Agent has to hack into airport "
	...
	ms8->SetTextMessage("The founder of Introversion Software believes the definitive version of Uplink "
						"could only be achieved "
						"as an online massively-muliplayer universe.  When a player launches Uplink2, it "
						"would connect to a global "
						"network of Uplink2 clients.  Each agent would be able to attack other Agent "
						"computer systems and steal "
						"things from them.  A number of 'core' systems would be run by Introversion "
						"Software, including most of the "
						"Government systems.  Agents would need to work in teams to properly hack the well "
						"guarded systems.\n\n"
						"Agents would also need to defend their own systems from hackers.  In effect, it "
						"would be similar in design "
						"to Napster - everyone can see everyone else on the Uplink Network.\n\n"
						"It is not clear what the legal situation would be - could anyone actually be "
						"prosecuted under this system?");
...
VLocation* WorldGenerator::GenerateLocation()
{
	...
	UplinkSnprintf(ip,
				   sizeof(ip),
				   "%d.%d.%d.%d",
				   NumberGenerator::RandomNumber(1000),
				   NumberGenerator::RandomNumber(1000),
				   NumberGenerator::RandomNumber(1000),
				   NumberGenerator::RandomNumber(1000));
...
Company*
WorldGenerator::GenerateCompany(const char* companyname, int size, int TYPE, int growth, int alignment)
{
...
Company* WorldGenerator::GenerateCompany()
{
	...
	int alignment =
		(int)NumberGenerator::RandomNormalNumber(COMPANYALIGNMENT_AVERAGE, COMPANYALIGNMENT_RANGE);
...
Computer* WorldGenerator::GenerateComputer(const char* companyname)
{

	// char *computername = strdup( NameGenerator::GenerateAccessTerminalName( companyname ) );
	char computername[MAX_COMPUTERNAME];
	strncpy(computername, NameGenerator::GenerateAccessTerminalName(companyname), MAX_COMPUTERNAME);
	if (computername[MAX_COMPUTERNAME - 1] != '\0') {
		computername[MAX_COMPUTERNAME - 1] = '\0';
	}
...
Computer* WorldGenerator::GenerateComputer(const char* companyName, int TYPE)
{

	if (TYPE & COMPUTER_TYPE_PUBLICACCESSSERVER) {
		return GeneratePublicAccessServer(companyName);
	} else if (TYPE & COMPUTER_TYPE_INTERNALSERVICESMACHINE) {
		return GenerateInternalServicesMachine(companyName);
	} else if (TYPE & COMPUTER_TYPE_CENTRALMAINFRAME) {
		return GenerateCentralMainframe(companyName);
	} else if (TYPE & COMPUTER_TYPE_PUBLICBANKSERVER) {
		return GeneratePublicBankServer(companyName);
	} else if (TYPE & COMPUTER_TYPE_PERSONALCOMPUTER) {
		return GeneratePersonalComputer(companyName);
	} else if (TYPE & COMPUTER_TYPE_VOICEPHONESYSTEM) {
		return GenerateVoicePhoneSystem(companyName);
	} else if (TYPE & COMPUTER_TYPE_LAN) {
		return GenerateLAN(companyName);
	} else {
		return NULL;
	}
}

Computer* WorldGenerator::GeneratePublicAccessServer(const char* companyname)
{

	// char *computername = strdup( NameGenerator::GeneratePublicAccessServerName( companyname ) );
	char computername[MAX_COMPUTERNAME];
	strncpy(computername, NameGenerator::GeneratePublicAccessServerName(companyname), MAX_COMPUTERNAME);
	if (computername[MAX_COMPUTERNAME - 1] != '\0') {
		computername[MAX_COMPUTERNAME - 1] = '\0';
	}
	...
	comp->SetName(computername);
	comp->SetCompanyName(companyname);
	comp->SetTraceSpeed((int)NumberGenerator::RandomNormalNumber(
		TRACESPEED_PUBLICACCESSSERVER, (float)(TRACESPEED_PUBLICACCESSSERVER * TRACESPEED_VARIANCE)));
	comp->SetIP(vl->ip);
...
Computer* WorldGenerator::GenerateInternalServicesMachine(const char* companyname)
{
	...
	char computername[MAX_COMPUTERNAME];
	strncpy(computername, NameGenerator::GenerateInternalServicesServerName(companyname), MAX_COMPUTERNAME);
	if (computername[MAX_COMPUTERNAME - 1] != '\0') {
		computername[MAX_COMPUTERNAME - 1] = '\0';
	}
	...
	comp->SetCompanyName(companyname);
	comp->SetTraceSpeed((int)NumberGenerator::RandomNormalNumber(
		TRACESPEED_INTERNALSERVICESMACHINE,
	...
	if (company->size > MINCOMPANYSIZE_MONITOR) {
		...
		if (monitorstrength > 5) {
			monitorstrength = 5;
		}
		if (monitorstrength < 1) {
			monitorstrength = 1;
		}
	...
	if (company->size > MINCOMPANYSIZE_FIREWALL) {
		...
		if (firewallstrength > 5) {
			firewallstrength = 5;
		}
		if (firewallstrength < 1) {
			firewallstrength = 1;
		}
	...
	uid->SetSubTitle("Log in");
	uid->SetDifficulty((int)NumberGenerator::RandomNormalNumber(
		HACKDIFFICULTY_INTERNALSERVICESMACHINE,
	...
	for (i = 0; i < numfiles; ++i) {
		...
		data->SetDetails(TYPE,
						 size,
						 (encrypted ? NumberGenerator::RandomNumber(5) : 0),
...
Computer* WorldGenerator::GenerateCentralMainframe(const char* companyname)
{
	...
	char computername[MAX_COMPUTERNAME];
	strncpy(computername, NameGenerator::GenerateCentralMainframeName(companyname), MAX_COMPUTERNAME);
	if (computername[MAX_COMPUTERNAME - 1] != '\0') {
		computername[MAX_COMPUTERNAME - 1] = '\0';
	}
	...
	comp->SetTraceSpeed((int)NumberGenerator::RandomNormalNumber(
		TRACESPEED_CENTRALMAINFRAME, (float)(TRACESPEED_CENTRALMAINFRAME * TRACESPEED_VARIANCE)));
	comp->SetTraceAction(COMPUTER_TRACEACTION_DISCONNECT | COMPUTER_TRACEACTION_LEGAL);
	...
	uid->SetSubTitle("Log in");
	uid->SetDifficulty((int)NumberGenerator::RandomNormalNumber(
		HACKDIFFICULTY_CENTRALMAINFRAME, (float)(HACKDIFFICULTY_CENTRALMAINFRAME * HACKDIFFICULTY_VARIANCE)));
	uid->SetNextPage(0);
	comp->AddComputerScreen(uid, 1);
...
Computer* WorldGenerator::GeneratePublicBankServer(const char* companyname)
{

	// char *computername = strdup( NameGenerator::GenerateInternationalBankName( companyname ) );
	char computername[MAX_COMPUTERNAME];
	strncpy(computername, NameGenerator::GenerateInternationalBankName(companyname), MAX_COMPUTERNAME);
	if (computername[MAX_COMPUTERNAME - 1] != '\0') {
		computername[MAX_COMPUTERNAME - 1] = '\0';
	}
	...
	ds4->AddWidget(
		"create", WIDGET_SCRIPTBUTTON, 250, 330, 80, 20, "Create", "Create this account", 11, -1, "", "");
	...
	ds7->AddWidget(
		"s_accno", WIDGET_BASIC, 100, 150, 100, 15, "Acc No. (From)", "Shows the source account number");
	ds7->AddWidget(
		"t_ip", WIDGET_BASIC, 100, 200, 100, 15, "Bank IP (To)", "Enter the IP of the target bank here");
	ds7->AddWidget(
		"t_accno", WIDGET_BASIC, 100, 230, 100, 15, "Acc No. (To)", "Enter the target account number here");
	ds7->AddWidget(
		"amount", WIDGET_BASIC, 100, 280, 100, 15, "Amount", "Enter the value (in credits) to transfer");
	...
	ds7->AddWidget("transfer",
				   ...
				   "");
	ds7->AddWidget(
		"cancel", WIDGET_NEXTPAGE, 350, 330, 80, 15, "Cancel", "Cancel the transfer", 3, -1, "", "");
	...
	ds19->AddWidget("text",
					...
					200,
					50,
					"Are you sure you wish to close your account?",
					"Shows the source account number");
	ds19->AddWidget(
		"close", WIDGET_SCRIPTBUTTON, 250, 330, 80, 15, "Close", "Close your account", 13, -1, "", "");
	ds19->AddWidget(
		"cancel", WIDGET_NEXTPAGE, 350, 330, 80, 15, "Cancel", "Do not close the account", 3, -1, "", "");
...
Computer* WorldGenerator::GenerateEmptyFileServer(const char* companyname)
{

	// char *computername = strdup( NameGenerator::GenerateFileServerName( companyname ) );
	char computername[MAX_COMPUTERNAME];
	strncpy(computername, NameGenerator::GenerateFileServerName(companyname), MAX_COMPUTERNAME);
	if (computername[MAX_COMPUTERNAME - 1] != '\0') {
		computername[MAX_COMPUTERNAME - 1] = '\0';
	}
...
Computer* WorldGenerator::GeneratePersonalComputer(const char* personname)
{

	// char *computername = strdup( NameGenerator::GeneratePersonalComputerName( personname ) );
	char computername[MAX_COMPUTERNAME];
	strncpy(computername, NameGenerator::GeneratePersonalComputerName(personname), MAX_COMPUTERNAME);
	if (computername[MAX_COMPUTERNAME - 1] != '\0') {
		computername[MAX_COMPUTERNAME - 1] = '\0';
	}
...
Computer* WorldGenerator::GenerateVoicePhoneSystem(const char* personname)
{

	// char *computername = strdup( NameGenerator::GenerateVoicePhoneSystemName( personname ) );
	char computername[MAX_COMPUTERNAME];
	strncpy(computername, NameGenerator::GenerateVoicePhoneSystemName(personname), MAX_COMPUTERNAME);
	if (computername[MAX_COMPUTERNAME - 1] != '\0') {
		computername[MAX_COMPUTERNAME - 1] = '\0';
	}
	...
	UplinkSnprintf(ip,
				   ...
				   NumberGenerator::RandomNumber(10),
				   NumberGenerator::RandomNumber(10),
				   NumberGenerator::RandomNumber(10),
				   NumberGenerator::RandomNumber(10),
				   NumberGenerator::RandomNumber(10),
...
Person* WorldGenerator::GeneratePerson()
{
	...
	strncpy(name, NameGenerator::GeneratePersonName(), MAX_PERSONNAME);
...
Company* WorldGenerator::GetRandomCompany()
{
	...
	while (1) {
		...
		if (strcmp(comp->name, "Player") != 0 && strcmp(comp->name, "Government") != 0
			&& strcmp(comp->name, "Uplink") != 0 && strcmp(comp->name, "Sample Company") != 0
...
Computer* WorldGenerator::GetRandomLowSecurityComputer(int TYPE)
{
	...
	while (!result || result->security.IsRunning_Proxy() || result->security.IsRunning_Firewall()
...
Person* WorldGenerator::GetRandomPerson()
{
	...
	*/
	...
	for (int i = 0; i < people->Size(); ++i) {
		...
		if (person->istargetable && person->GetStatus() == PERSON_STATUS_NONE) {
...
Agent* WorldGenerator::GetRandomAgent()
{
	...
	*/
	...
	for (int i = 0; i < people->Size(); ++i) {
		...
		if (person->istargetable && person->rating.uplinkrating > 0
...
bool WorldGenerator::VerifyPlayerGateway ()
{
	...
	if ( !realgatewaydef ) {
		...
		char message [ 256 ];
		UplinkSnprintf ( message, sizeof ( message ), "This agent is using the following custom gateway '%s',
but Uplink cannot find a definition for it.", plgatewaydef->name ); printf ( "WARNING :
WorldGenerator::VerifyPlayerGateway : %s\n", message );
