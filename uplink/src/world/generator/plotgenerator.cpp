...
const char* MISSION_TITLE[] = { "Backfire",
								"Tracer",
								"TakeMeToYourLeader",
								"ARCInfiltration",
								"CounterAttack",
...
const char* MISSION_DESCRIPTION[] = { "Counter attack!",
...
void PlotGenerator::Initialise_ARUNMOR()
{
	...
	const char* companyname = "Arunmor";
...
void PlotGenerator::Initialise_DARWIN()
{
	...
	const char* companyname = "Darwin Research Associates";
...
void PlotGenerator::Run_Scene(int newact, int newscene)
{
	...
	case 1:
		...
		if (scene == 1) {
			...
			printf("Invalid Scene number: Act1 Scene%d\n", newscene);
		}
	...
	case 2:
		...
		if (scene == 1) {
			...
			printf("Invalid Scene number: Act2 Scene%d\n", newscene);
		}
		break;
	...
	case 3:
		...
		if (scene == 1) {
			...
			printf("Invalid Scene number: Act3 Scene%d\n", newscene);
		}
		break;
	...
	case 4:
		...
		if (scene == 1) {
			...
			printf("Invalid scene number: Act4 scene%d\n", newscene);
		}
		break;
	...
	case 5:
		...
		if (scene == 1) {
			...
			printf("Invalid Scene number: Act5 Scene%d\n", newscene);
		}
		break;
	...
	case 6:
		...
		if (scene == 1) {
			...
			printf("Invalid Scene number: Act6 Scene%d\n", newscene);
		}
		break;
...
void PlotGenerator::PlayerCancelsMail() { playercancelsmail = true; }
...
bool PlotGenerator::PlayerContactsARUNMOR(Message* msg)
{
	...
	if (act == 2 && scene == 3) {
		...
		if (attached && attached->TYPE == DATATYPE_PROGRAM && strcmp(attached->title, "Revelation") == 0) {

			version_faith = attached->version;

			game->GetWorld()->GetPlayer()->ChangeBalance(PAYMENT_SPECIALMISSION_BACKFIRE,
														 "Arunmor [Faith project]");
		...
		if (attached && attached->TYPE == DATATYPE_PROGRAM && strcmp(attached->title, "Revelation") == 0) {
			...
			if (playerloyalty != 1 && attached->version > version_faith) {
				...
				msg->SetBody("Let us be the first to congratulate you Agent - we have been "
							 ...
							 "Of course, we won't ask where you got it, nor will we let it become known who "
							 "sent it too us.\n\n"
							 "Thanks to your submission, we will now be able to pull several weeks ahead of "
							 "Revelation.  We have already deposited "
				...
				msg->SetFrom("Arunmor");
				msg->SetSubject("Your recent submission");
				msg->SetBody("Thank you for your recent submission of the Revelation virus, and "
							 "congratulations on finding "
							 "such a rare item.  Unfortunately, we already have a copy of this version of "
							 "the virus, so what "
							 "you have sent is will not prove useful.\n\n"
							 "As such, we are unable to pay you any reward.  If you find any newer versions, "
							 "please send them "
							 "to us.");
				msg->Send();
			}
...
void PlotGenerator::RunRevelation(char* ip, float version, bool playerresponsible)
{
	...
	if (numrunningsystems == numsystems) {
		success = 1;
	} else if (numrunningsystems == 0) {
		success = 3;
	} else {
		success = 2;
	}
...
void PlotGenerator::RunFaith(char* ip, float version, bool playerresponsible)
{
	...
	if (comp->isinfected_revelation && version >= comp->isinfected_revelation) {
...
void PlotGenerator::RunRevelation(char* ip, float version, bool playerresponsible, int success)
{
	...
	/*
		...
		if ( success == 0 ) {												// They manage to stop the virus
	   from running

		}
		else if ( success == 1 ) {											// Infect this computer only
	   before it is stopped

			comp->InfectWithRevelation ();

		}
		else if ( success == 2 ) {											// Infect a lot of related
	   computers
		...
		else if ( success == 3 ) {											// Infect all computers owned by
	   this company
...
float PlotGenerator::GetVersion_Faith() { return version_faith; }

float PlotGenerator::GetVersion_Revelation() { return version_revelation; }
...
void PlotGenerator::Run_Act1Scene2()
{
	...
	news->SetDetails(
		"Andromeda Research Corporation, otherwise known as ARC, have been extremely busy lately.\n\n"
	...
	for (int ip = 0; ip < allpeople->Size(); ++ip) {
		if (allpeople->ValidIndex(ip)) {
			...
			if (p->GetOBJECTID() == OID_AGENT && p->GetStatus() == PERSON_STATUS_NONE) {
...
void PlotGenerator::Run_Act1Scene4()
{
	...
	std::ostrstream details;
	details
		<< "In a suprising development in the mystery of Andromeda Research Corporation (ARC), "
		...
		<< ", says in his statement "
		   ...
		   "in the dark to protect their trade secrets.  Uplink Agents should not be afraid of accepting "
		   "jobs "
		   "from this company.'\n"
		   "'Some people have said they think ARC are building some kind of military weapon - I can say that "
		   "I "
		   "have had full access to the ARC Central Mainframe for the past week and there is nothing of the "
		   "sort "
		   "on the system.  What they are doing is being kept under wraps but it is definately not a "
		   "weapon.'\n\n"
		   "Agent "
		<< agent->handle
		<< " is a well respected hacker with years of experience and his comments "
...
void PlotGenerator::Run_Act1Scene5()
{
	...
	ds->AddWidget("cancel1",
				  ...
				  14,
				  "Cancel",
				  "Click to stop this mail from being sent",
				  60,
				  -1,
				  NULL,
				  NULL);
	ds->AddWidget("cancel2",
				  WIDGET_SCRIPTBUTTON,
				  280,
				  235,
				  50,
	...
	details
		...
		<< " has been found dead in his house after an apparent suicide.\n"
		   ...
		   "was a lead player in the project.  For something like this happen is a tradgedy of the greatest "
		   "order.  "
...
void PlotGenerator::Run_Act1Scene8()
{
	...
	if (!playercancelsmail) {
		...
		details << "Uplink Corporation has issued a formal warning to all of its agents, warning them "
				   ...
				   "Uplink Agents refuse any work being offered to them from ARC or anyone working for that "
				   "company.  "
				   "Anyone caught working for this company will lose their Uplink status immediately.\n\n";
#ifdef DEMOGAME
		details
			<< "'In addition, all news and data related to ARC will be classified and restricted to those "
			   "agents "
			   "with fully registered accounts.  Those agents using the Demo Uplink client will not be able "
			   "to "
	...
	if (game->GetWorld()->GetPlayer()->gateway.databank.GetData("RevelationCoreV0.7")) {
		...
		body << "Agent " << game->GetWorld()->GetPlayer()->handle << ",\n"
			 << "We couldn't help but notice that you have been poking into our "
				...
				"Reply to this email (or send one to internal@ARC.net) if you are interested in discussing "
				"this further."
			 << '\x0';
		...
		body << "Agent " << game->GetWorld()->GetPlayer()->handle << ",\n"
			 << "We have been tidying up some of our loose ends recently and we "
				...
				"large favour.  It appears that Agent "
			 << act1scene4->handle
...
void PlotGenerator::Run_Act2Scene2()
{
	...
	if (playerloyalty == 0) {
		...
		Computer* comp =
			game->GetWorld()->GetComputer(NameGenerator::GenerateInternalServicesServerName("Arunmor"));
...
void PlotGenerator::Run_Act2Scene3()
{
	...
	body << "Agent " << game->GetWorld()->GetPlayer()->handle << "\n\n"
		 << "Firstly let us congratulate you on scoring such a lucrative deal with "
			...
			"a copy of whatever ARC sent you, and we'll reward you with "
		 << PAYMENT_SPECIALMISSION_BACKFIRE
		 << "c.  You can "
...
void PlotGenerator::Run_Act3Scene1()
{
	...
	if (playerloyalty == 1) {
		...
		msg->SetBody("So it seems that you were working for the enemy behind our backs all this time.  You "
					 "cannot imagine "
					 "how suprised we were when we heard that Arunmor had received a complete copy of "
					 "Revelation, and that "
					 "it was an Uplink Agent who was the source.  Unfortunately for you, you are the only "
					 "Uplink Agent "
					 "who could have sent them this copy.  This makes you our new enemy.\n\n"
					 "Take this message however you want - but Andromeda Research Corporation will not let "
					 "this betrayal go "
					 "unpunished.  We'll be watching you, and when we finally come after you, you're going "
					 "to wish you had "
		...
		news->SetDetails(
			"Earlier today Arunmor Corporation shut down all of its primary computer systems and "
			"disconnected "
			"their Internal Network from the web. The disconnection lasted just under one hour, and caused a "
			"great "
			...
			"working for the controversial Andromeda Research Corporation. So far Arunmor has not said why "
			"they suspect "
		...
		news->SetDetails(
			"Arunmor Corporation recently suffered an attack from a new computer virus, dubbed by "
			...
			"Recently a high ranking Uplink Agent contacted us and suggested that Andromeda Research "
			"Corporation "
			"might be connected to this viral outbreak.  Agents have been assigned the task of "
			"investigating.");
...
void PlotGenerator::Run_Act3Scene2()
{
	...
	news->SetDetails(
		"Following the recent attempted attack on the Arunmor computer network, "
		...
		"attempted attack on our network highlighted how weak Internet security is.  We intend to put a stop "
		"to this.'\n\n"
	...
	news2->SetDetails(
		"In light of the recent developments related to Revelation and Faith, Uplink Corporation "
...
void PlotGenerator::Run_Act3Scene3()
{
	...
	std::ostrstream details;
	details
		<< "Arunmor Corporation have been working hard on researching the recently discovered "
		   ...
		   "It has now become clear that the aim of Revelation is to totally destroy the Internet, from the "
		   "inside.\n\n"
		   "'Once released, it will spead like wildfire to all the main systems, then eventually to the "
		   "entire Internet, "
		   "destroying everything in its path.  Its payload is extremely well designed and will take down "
		   "any system "
		   "it comes up against.'\n\n";
	...
	if (playerloyalty == 1) {
		...
		details << "We will be posting missions on the Uplink BBS mission board.  Any agents that can help "
				   "us will be "
				   "well rewarded, and will be fighting for a good cause.\n\n"
				   "'The offer of 25000c for any data on related to Revelation is still open, and will "
				   "remain open "
				   "for the next 48 hours.'\n\n";
	}

	details << "'Our Faith project is now aiming to provide a counter-virus that will stop Revelation in its "
			   "tracks.'\n";
	details << "Federal Agents today began an investigation into the activities of Andromeda Research "
			   "Corporation.";
	details << '\x0';
...
void PlotGenerator::Run_Act3Scene4()
{
	...
	std::ostrstream details;
	details << "Federal Agents today made their first public announcement on their investigation into "
			   "Andromeda Research Corporation (ARC).\n\n"
			   "'Our investigation so far has uncovered a number of disturbing facts, however no concrete "
			   "evidence has yet been discovered' "
			   "said a Federal Spokesman. 'It would appear that ARC are developing a software tool in an "
			   "attempt to damage the operation of the "
			   "Internet. So far no evidence has been found and no witnesses have come forward to back this "
			   "up.'\n\n"
			   "'It is our belief that ARC were responsible for the murder of "
			<< act1scene4agent
			<< " and possibly one other, but until "
			   "we aquire evidence we will be unable to act in any legal way.'\n\n"
			   "Most importantly, we have determined that Andromeda Research Corporation is privately owned "
			   "and controlled by the secretive group 'Andromeda', "
			   "and that the CEO and the Managing Director of ARC are both core members of this group.  Our "
			   "profile of Andromeda lists them as "
			   "well funded Anti-Capitalist Techno-Anarchists, which would seem to fit the charges against "
			   "them.  So far, we have not had any success in finding this group.'\n\n"
			   "'Their only real-world presence that we are aware of is their Public Access Terminal, but "
			   "the registered contact details of that site are all fake.'\n\n"
			   "'We intend to continue our investigation and appeal for anyone with any information to come "
			   "forward.'"
			<< '\x0';
...
void PlotGenerator::Run_Act4Scene4()
{
	...
	news->SetDetails(
		"The Internal Services System run by Andromeda Research Corporation (ARC) has been "
		...
		"developing some doomsday weapon, we have had serious problems attracting new business.  Now "
		"this.'\n\n"
...
void PlotGenerator::Run_Act4Scene5()
{
	...
	news->SetDetails(
		"Following an outbreak of the Revelation computer virus on Arunmor Corporations Internal Services "
...
void PlotGenerator::Run_Act4Scene6()
{
	...
	news->SetDetails(
		"Industry experts have expressed suprise at the discovery that the ARC central Mainframe was "
		...
		"a press release this morning. 'We have found equipment which suggests somebody temporarily "
		"connected "
...
void PlotGenerator::Run_Act4Scene10()
{
	...
	news->SetDetails("Earlier today a hacker penetrated the Local Area Network of Darwin Research Associates "
					 ...
					 "known exactly what the hackers wanted the data for, since its use is described as "
					 "purely theoretical.\n\n"
					 "'The idea behind Darwin Digital Life was to create a life form that could live and "
					 "reproduce on the Internet' "
					 "said the CEO of Darwin Research Associates recently.  'Obviously we are gutted that "
					 "somebody has stolen what "
					 "amounts to years of research into this area.'\n\n"
					 "A suspect has been named to Federal Agents, but so far no arrests have been made.  "
					 "Darwin Research Associates "
					 "earlier today stated that an Uplink Agent was responsible, and that the Agent in "
					 "question was working for "
					 "Andromeda Research Corporation (ARC) at the time.\n\n"
					 "Once again, ARC was unwilling to comment on the issue, but this latest allegation has "
					 "fueled speculation as to "
...
void PlotGenerator::Run_Act4Scene11()
{
	...
	news->SetDetails(
		"The lead Technical director of Arunmor Corporation has been arrested by Federal Agents, "
		"following allegations that he has been caught attempting bank fraud.  This latest development "
		"in the Arunmor Versus ARC story has called into question the true motives behind Arunmor "
		"Corporation.\n\n"
		"Federal Agents arrested the technical director in a dawn raid, after an anonymous tip led them to "
		"discover "
		"evidence which immediately incriminated him.\n\n"
		"So far he has protested his innocence, claiming he has been framed.\n\n"
		"This arrest will come as a severe blow to Arunmor Corporation, who relied on this man for most of "
		"their "
...
void PlotGenerator::Run_Act4Scene12()
{
	...
	news->SetDetails(
		"Arunmor Corporation today released a press statememt that said their primary computer system "
...
void PlotGenerator::Run_Act5Scene1()
{
	...
	news->SetDetails(
		"Industry experts are predicting that a launch of the destructive Revelation virus "
...
void PlotGenerator::Run_Act5Scene3()
{
	...
	news->SetDetails(
		"Within the last couple of hours the Revelation virus was released onto the Internet "
		"by operatives at the controversial Andromeda Research Corporation. Experts within the "
		"industry had predicted this was about to happen, but were unprepared for the scale of the "
		"attack.\n\n"
...
void PlotGenerator::Run_Act5Scene4()
{
	...
	news->SetDetails(
		"The Revelation computer virus has been released into the wild and has spread rapidly through "
...
void PlotGenerator::Run_Act6Scene1()
{
	...
	news->SetDetails(
		"The Leader of the Andromeda group today issued a statement through his lawyer, regarding his recent "
		"arrest "
		"for criminal damage to the Web. In it he outlines why he tried to destroy the Internet that we all "
		"rely on.\n\n"
		"'Firstly, I make no apologies for what I have done. My only regret is that I did not succeed' says "
		"the man, who "
		"has not yet been identified by Federal Agents.\n"
		"'The Internet used to be a place of amazing opportunity and promise - the last remnant of free "
		"speech and "
		"true anonymity.  Now it is little more than an extension of the evil of Western Capitalism.'\n\n"
		"'Think of this.  Do you have any idea how much money Arunmor will make from recent events?  This is "
		"why I tried "
		"to destroy the Internet.  It only serves to line the pockets of those who would try to control "
		"us.'\n\n"
		"'While the Web exists, you will never be safe. Your entire life is on file, waiting for somebody to "
		"tamper with it.  Your lives "
...
void PlotGenerator::Run_Act6Scene2()
{
	...
	news->SetDetails("The Federal courts today found the CEO of ARC and the leader of Andromeda Guilty of "
					 "criminal destruction of data. \n"
					 "The judge called the Leader of Andromeda 'an evil and vindictive man, who truly "
					 "believes the world would be a better place in a state of Anarchy.'\n\n"
					 "He went on to say that he had little sympathy for both the defendants.\n\n"
					 "As punishment for their crimes, both members will serve eight years imprisonment and "
					 "will be banned from the use of computers for life.\n\n");
...
void PlotGenerator::Run_Act6Scene3()
{
	...
	news->SetDetails("It would appear that the recent panic over Revelation was (as usual) totally "
					 "unnecessary.  The Internet has suffered a number of "
					 "high profile scares since its foundation, and it has always survived.  This incident "
					 "was no different, and only serves to highlight the "
					 "ease with which the public can be scared into thinking their lives are in danger.\n\n"
					 "A special help line was recently set up by Federal Agents for worried members of the "
					 "public, and they have so far received an average of "
					 "ten-thousand calls a day from people who are terrified their life records are now wide "
					 "open due to this recent security scare.  'They just "
					 "keep calling' said a Federal Spokesman today.  'These people honestly believe that "
					 "somebody could just open up their records and change "
					 "them as if they were in the public library.  Its total hysteria.'\n\n"
					 "Since the collapse of the Andromeda Group, ARC has been exposed as little more than a "
					 "front for their activities, and has shut down all operations. "
					 "It is a wonder that nobody noticed ARC never released any products.\n\n"
					 "Arunmor Corporation yesterday launched their new Active-Virus-Guard software, titled "
					 "'Faith'.  This is an extension of the tool used to defeat "
					 "Revelation, and the makers claim it actively seeks out viruses and eradicates them.  "
					 "Sales have apparently been 'very promising'.\n\n"
...
Mission* PlotGenerator::GenerateMission_Tracer()
{
	...
	const char* password = "tracer";
	char username[12];
	UplinkSnprintf(username,
				   sizeof(username),
				   "temp%c%c%c%c",
	...
	fulldetails
		<< "One of our sources within Andromeda Research Corporation has discovered that ARC "
		   ...
		   "systems.  This includes Proxies, Monitors, Firewalls etc.  You have to stop them from seeing "
		   "what "
		   "you are doing.\n\n"
		   "Copy the Tracer into place, then drop into a console.  You can run the tracer from there.  Once "
		   "it is "
		   "running it will immediately start hiding itself. Check you can't see it from the File Server "
		   "screen - this "
		   "will ensure nobody else can see it.\n\n"
		   "Cover your tracks and get out.  Reply to this email when you are done.\n\n"
		   "Codename: Tracer\n\n"
		   "Payment will be "
		<< PAYMENT_SPECIALMISSION_TRACER << "\n\n"
		<< "Our FileServer: " << ourcomp->ip << "\n"
		<< "Target system: " << comp->name << "\n"
		<< "Target IP: " << comp->ip << '\x0';
	...
	details << "Payment for this job is " << PAYMENT_SPECIALMISSION_TRACER << " credits.\n"
			...
			<< "This mission cannot be auto-accepted." << '\x0';
...
Mission* PlotGenerator::GenerateMission_TakeMeToYourLeader()
{
	...
	const char* password = "takemetoyourleader";
	char username[12];
	UplinkSnprintf(username,
				   sizeof(username),
				   "temp%c%c%c%c",
	...
	std::ostrstream fulldetails;
	fulldetails
		<< "We have recently been co-operating with Federal Agents in an attempt to bring the CEO "
		   ...
		   "unfortunately, we have a copy of an early version of Revelation, thanks to an earlier "
		   "mission.\n\n"
		   "Your first task is to connect to our File Server and retrieve Revelation.\n\n"
		   "You must then break into OUR Internal Services System (shown below), disable the security and "
		   "run "
		   "Revelation on that system.  Finally, cover your tracks to make sure the source can't be "
		   "traced.\n\n"
		   "This attack will give the Federal Agents a good enough reason to arrest the CEO of ARC, and we "
		   "can "
		   "start questioning him.\n\n"
		   "It is important to note that nobody else knows of this attack - even the system administrators "
		   "at our "
		   "Internal Services System.  They will still try to trace you, and if you are caught we will not "
		   "be "
	...
	m->SetEmployer("Arunmor");
	m->SetContact("internal@Arunmor.net");
	m->SetPayment(PAYMENT_SPECIALMISSION_TAKEMETOYOURLEADER,
				  (int)(PAYMENT_SPECIALMISSION_TAKEMETOYOURLEADER * 1.1));
	m->SetDifficulty(10);
	m->SetMinRating(10);
	m->SetAcceptRating(99);
...
Mission* PlotGenerator::GenerateMission_ARCInfiltration()
{
	...
	const char* password = "infiltrate";
	char username[12];
	UplinkSnprintf(username,
				   sizeof(username),
				   "temp%c%c%c%c",
	...
	UplinkStrncpy(
		description, SpecialMissionDescription(SPECIALMISSION_ARCINFILTRATION), sizeof(description));

	fulldetails
		<< "As you may be aware, ARC are storing most of their Research on their Local Area Network system. "
		   ...
		   "disconnected, meaning their data is always totally isolated from public access.  This has made "
		   "it "
		   "impossible for us to steal their research until now.\n\n"
		   "A member of ARCs team who wishes to remain anonymous has come forward and offered to help us. "
		   "You "
		   "will need to break in to the system first.  Once inside, you should be able to find the "
		   "restricted "
		   "file area.  Our man on the inside will then install a remote radio modem on the file server, "
		   "which "
		   ...
		   "You may also have noticed that the CEO of ARC was recently arrested and questioned by Federal "
		   "Agents "
		   "in connection with Revelation. We believe ARC are planning to launch Revelation early due to "
		   "this.  "
		   "This makes the success of this mission Super critical.\n\n"
		   "You will be paid "
		<< PAYMENT_SPECIALMISSION_ARCINFILTRATION
		<< "c for this mission.\n\n"
	...
	mission->SetPayment(PAYMENT_SPECIALMISSION_ARCINFILTRATION,
						(int)(PAYMENT_SPECIALMISSION_ARCINFILTRATION * 1.1));
...
Mission* PlotGenerator::GenerateMission_CounterAttack()
{
	...
	const char* password = "faithinchaos";
	char username[12];
	UplinkSnprintf(username,
				   sizeof(username),
				   "temp%c%c%c%c",
	...
	details << "Our tracer programs have just informed us that Revelation has been released "
			   ...
			   "are currently infected but more will certainly follow.  This is what we have been planning "
			   "for.\n\n"
			   "Revelation will infect a system, wait for a few minutes, then spread to two other systems. "
			   "The origional "
			   "infected system will then be shut down. If two many systems get infected, we won't be able "
			   "to stop it.\n\n"
			   "Connect to our File Server and download the Revelation Tracker program. This will keep you "
			   "updated when "
			   "systems become infected with Revelation, so you can connect and disinfect them.\n\n"
			   "You will also need to download Faith - our countervirus.  This is run by copying it into the "
			   "databanks of "
			   "the infected systems, dropping into a command prompt, and running it.\n\n"
			   "There isn't much time.  You have only minutes until Revelation spreads beyond control. Get "
			   "moving.\n\n"
...
Mission* PlotGenerator::GenerateMission_MaidenFlight()
{
	...
	std::ostrstream fulldetails;
	fulldetails
		<< "Our corporation has been developing a software product known as Revelation for some time now, "
		   ...
		   "The target system is the Arunmor Internal Services System.  You will need to first locate this "
		   "computer. "
		   ...
		   "systems.  The more security you disable, the better this product will operate.  In order to "
		   "activate "
...
Mission* PlotGenerator::GenerateMission_Darwin()
{
	...
	//    );
	Computer* target =
		game->GetWorld()->GetComputer(NameGenerator::GenerateLANName("Darwin Research Associates"));
	if (UplinkIncompatibleSaveGameAssert(target, __FILE__, __LINE__)) {
		return NULL;
	}
	...
	for (int i = 0; i < numfiles; ++i) {
		...
		char datatitle[64];
		UplinkSnprintf(datatitle,
					   sizeof(datatitle),
					   "%c%c%c%c-DARWIN %d.dat",
					   target->companyname[0],
					   target->companyname[1],
					   target->companyname[2],
					   target->companyname[3],
					   i);
	...
	fulldetails
		<< "The results of our recent test have been extremely encouraging.  Revelation is easily "
		   "destructive enough to take out most of the Internet when it is finally released.  Unfortunately, "
		   "it is not particularly virulant.  In other words, it doesn't spread very fast.  We need to fix "
		   "this.\n\n"
		   "We've looked around and we have found a company called Darwin Research Associates.  They are "
		   "currently "
		   "working on research into digital life forms that exist and breed on the Internet.  Since they "
		   "have refused "
		   "to sell any of their technology to us, we have decided instead to steal it.\n\n"
		   "Break into their Local Area Network system shown below, and steal any files relating to the "
		   "Darwin project.  "
		   "Try to copy as much data as possible.  You will need to decrypt the data before we can make any "
		   "use of it.  "
		   "We have determined that over 80Gqs of their research is stored on this system.\n\n"
		   "Once you have stolen the research, copy it onto the File Server that we have set up for you, "
		   "detailed below.\n\n"
		   "We are confident we will be able to use their artificial life code to make Revelation spread "
		   "like wildfire."
		   "You will be paid "
		<< PAYMENT_SPECIALMISSION_DARWIN
		<< "c for this mission.\n\n"
		   "Codename: Darwin\n\n";
...
Mission* PlotGenerator::GenerateMission_SaveItForTheJury()
{
	...
	strncpy(
		personalcomputername, NameGenerator::GeneratePersonalComputerName(poorsod->name), MAX_COMPUTERNAME);
	...
	UplinkStrncpy(
		description, SpecialMissionDescription(SPECIALMISSION_SAVEITFORTHEJURY), sizeof(description));

	fulldetails
		<< "We have decided that it is time to get personal.  The chief technical director of "
		   "Arunmor Corporation, Mr "
		<< poorsod->name
		<< ", has recently excelled in his line of work.\n"
		   "He has managed to make a number of breakthrough's in their research into Revelation, and no "
		   "doubt "
		   "his presence at Arunmor has increased the power of their rival virus Faith.  We have decided "
		   "that "
		   "his life should now come to an effective end.\n\n"
		   "In addition, we'd like to strike a blow to Arunmor's public image as well.  As such, we'd like "
		   "you "
		   ...
		   "to one of your relay systems and modify the logs so that "
		<< poorsod->name
		<< " is incriminated.\n\n"
...
Mission* PlotGenerator::GenerateMission_GrandTour()
{
	...
	std::ostrstream details;
	details
		<< "In your email inbox you will see two emails from us - one containing the final version "
		...
		<< " - we have "
		   ...
		   "Arunmor have Faith ready - they will be right behind you.  If you are quick enough, you will be "
		   "able "
		   "to spread the virus faster than they can stop it. Revelation will infect a system, scan for "
		   "nearby systems "
		   "on the same subnet, then migrate to the nearest two of those systems. At the same time it will "
		   "destroy "
		   "the host it originated from.  Once a certain number of systems have been disabled, the growth of "
		   "the virus "
		   ...
		   "Uplink Corporation, no more ARC and no more Arunmor.  The entire Internet will come down with "
		   "us.  "
		   "Think about it - we can destroy all records of ownership, reset everyone back to square one, "
		   "crash "
...
bool PlotGenerator::IsMissionComplete_Tracer()
{
	...
	if (strcmp(tracer_lastknownip, comp->ip) == 0) {
		...
		msg->SetBody(
			"We haven't received any ping signals from our Tracer yet.  You can't have installed it right.");
		msg->Send();
...
bool PlotGenerator::IsMissionComplete_TakeMeToYourLeader()
{
	...
	if (comp->isinfected_revelation >= 0.0) {
		...
		m->SetBody(
			"That looks like a total success to us. We'll have to keep quiet for a while, but it looks like "
			...
			"of the Revelation project - since ARC is actually owned by the Andromeda group, and their "
			"leader "
			...
			"getting quite desperate at the moment - they think their entire infrastructure may be "
			"destroyed, "
...
bool PlotGenerator::IsMissionComplete_ARCInfiltration()
{
	...
	for (int i = 0; i < ourcomp->databank.GetDataSize(); ++i) {
		if (ourcomp->databank.GetDataFile(i)) {
			...
			if (!thisfile->encrypted) {
				...
				if (strstr(thisfile->title, "REVELATION") != NULL && thisfile->encrypted == 0) {
					...
					if (thisfileindex >= 0 && thisfileindex < numfiles && filefound[thisfileindex] == false) {
	...
	if (foundsize == 0) {
		...
		std::ostrstream body;
		body << "\n\nWe can see " << foundsize
			 << " Gigaquads of useful data which will help us "
				"perfect Faith.\n"
			 << "Congratulations on your success Agent " << game->GetWorld()->GetPlayer()->handle
...
bool PlotGenerator::IsMissionComplete_Darwin()
{
	...
	for (int i = 0; i < ourcomp->databank.GetDataSize(); ++i) {
		if (ourcomp->databank.GetDataFile(i)) {
			...
			if (!thisfile->encrypted) {
				...
				if (strstr(thisfile->title, "DARWIN") != NULL && thisfile->encrypted == 0) {
	...
	if (foundsize == 0) {
		...
		std::ostrstream body;
		body << "Congratulations on your success Agent " << game->GetWorld()->GetPlayer()->handle
			 << "\n\nWe can see " << foundsize
			 << " Gigaquads of useful data which will help us "
...
bool PlotGenerator::IsMissionComplete_SaveItForTheJury()
{
	...
	for (int il = 0; il < comp->logbank.logs.Size(); ++il) {
		if (comp->logbank.logs.ValidIndex(il)) {
			...
			if (al->SUSPICIOUS != LOG_NOTSUSPICIOUS && al->TYPE == LOG_TYPE_CONNECTIONOPENED) {
	...
	if (success != 2) {
		...
		for (int i = 0; i < uplink->news.Size(); ++i) {
			...
			if (news->NEWSTYPE == NEWS_TYPE_ARREST && news->data1
				&& strcmp(news->data1, poorsod->name) == 0) {
	...
	if (success == 2) {
		...
		msg->SetSubject("Congratulations");
		msg->SetBody(
			"Well done Agent - we have checked the systems ourselves and have determined that "
...
bool PlotGenerator::IsMissionComplete_ShinyHammer()
{
	...
	if (!computer->isrunning && computer->databank.formatted) {
		...
		msg->SetBody(
			"Well done for attempting the Arunmor crash mission.  It appears you shut down the system, "
		...
		msg->SetBody(
			"Congratulations on deleting the data on Arunmor's Local Area Network.  It appears you didn't "
...
void PlotGenerator::NewsRevelationUsed(char* ip, int success)
{
	...
	part1 << comp->companyname
		  << " has been attacked by a computer virus.  "
			 "The virus first infected the "
		  << comp->name << ".";

	switch (success) {

	case 0:
		part2 << "System administrators were able to contain the virus before it managed any serious damage. "
				 " It appears the virus was totally contained.";
		break;
	case 1:
		part2 << "The virus destroyed most of the data and logs on the computer but the company was able to "
				 "stop the virus from spreading to other systems.";
		break;
	case 2:
		part2 << "The " << comp->name
			  << " was totally destroyed and the virus spread to many neighbouring systems, causing a great "
				 "deal of damage.";
		break;
	case 3:
		part2 << "It appears that the entire corporate computer network owned by " << comp->companyname
			  << " was severely damaged by the virus.";
		break;
	};
	...
	case 0:
	case 1:
		part3 << "This is the first time that this virus has been seen in the wild, but experts who analysed "
				 "it say it has incredible power.  "
				 "After analysing the virus in detail, it has been determined that the name of the virus is "
				 "Revelation.";
		headline << "Unknown virus outbreak at " << comp->companyname;
		break;

	case 2:
		part3 << "Upon closer inspection, industry experts have been able to determine that the Revelation "
	...
	default:
		part3 << "Once again Revelation is responsible for this destruction.";
...
void PlotGenerator::Disinfected(char* ip)
{
	...
	if (infected.Size() == 0 && revelation_arcbusted == true && playerloyalty == 1
...
void PlotGenerator::PlayerVisitsPlotSites() { playervisitsplotsites = true; }
...
int PlotGenerator::GetSpecialMissionsCompleted() { return specialmissionscompleted; }

const char* PlotGenerator::SpecialMissionTitle(int missionID)
{
...
const char* PlotGenerator::SpecialMissionDescription(int missionID)
{
...
bool PlotGenerator::IsPlotCompany(const char* companyname)
{
	...
	if (strcmp(companyname, "ARC") != 0 && strcmp(companyname, "Arunmor") != 0
...
bool PlotGenerator::UplinkIncompatibleSaveGameAssert(const bool& condition,
													 const char* fileassert,
													 int lineassert)
{
...
bool PlotGenerator::Load(FILE* file)
{
	...
	if (!FileReadData(&act, sizeof(act), 1, file)) {
		return false;
	}
	if (!FileReadData(&scene, sizeof(scene), 1, file)) {
		return false;
	}

	if (strcmp(game->GetLoadedSavefileVer(), "SAV59") >= 0) {
		if (!LoadDynamicStringStatic(act1scene3agent, sizeof(act1scene3agent), file)) {
			return false;
		}
		if (!LoadDynamicStringStatic(act1scene4agent, sizeof(act1scene4agent), file)) {
			return false;
		}
		if (!LoadDynamicStringStatic(act2scene1agent, sizeof(act2scene1agent), file)) {
			return false;
		}
	} else {
	...
	if (!FileReadData(&playervisitsplotsites, sizeof(playervisitsplotsites), 1, file)) {
		return false;
	}
	if (!FileReadData(&playercancelsmail, sizeof(playercancelsmail), 1, file)) {
		return false;
	}
	if (!FileReadData(&playerloyalty, sizeof(playerloyalty), 1, file)) {
		return false;
	}

	if (!FileReadData(&completed_tracer, sizeof(completed_tracer), 1, file)) {
		return false;
	}
	if (!FileReadData(&completed_takemetoyourleader, sizeof(completed_takemetoyourleader), 1, file)) {
		return false;
	}
	if (!FileReadData(&completed_arcinfiltration, sizeof(completed_arcinfiltration), 1, file)) {
		return false;
	}
	if (!FileReadData(&completed_darwin, sizeof(completed_darwin), 1, file)) {
		return false;
	}
	if (!FileReadData(&completed_saveitforthejury, sizeof(completed_saveitforthejury), 1, file)) {
		return false;
	}
	if (!FileReadData(&completed_shinyhammer, sizeof(completed_shinyhammer), 1, file)) {
		return false;
	}

	if (!FileReadData(&specialmissionscompleted, sizeof(specialmissionscompleted), 1, file)) {
		return false;
	}

	if (strcmp(game->GetLoadedSavefileVer(), "SAV59") >= 0) {
		if (!LoadDynamicStringStatic(
				saveitforthejury_guytobeframed, sizeof(saveitforthejury_guytobeframed), file)) {
			return false;
		}
		if (!LoadDynamicStringStatic(
				saveitforthejury_targetbankip, sizeof(saveitforthejury_targetbankip), file)) {
			return false;
		}
		if (!LoadDynamicStringStatic(tracer_lastknownip, sizeof(tracer_lastknownip), file)) {
			return false;
		}
	} else {
	...
	if (!FileReadData(&numuses_revelation, sizeof(numuses_revelation), 1, file)) {
		return false;
	}
	if (!FileReadData(&revelation_releaseuncontrolled, sizeof(revelation_releaseuncontrolled), 1, file)) {
		return false;
	}
	if (!FileReadData(&revelation_releasefailed, sizeof(revelation_releasefailed), 1, file)) {
		return false;
	}
	if (!FileReadData(&revelation_arcbusted, sizeof(revelation_arcbusted), 1, file)) {
		return false;
	}

	if (!FileReadData(&version_revelation, sizeof(version_revelation), 1, file)) {
		return false;
	}
	if (!FileReadData(&version_faith, sizeof(version_faith), 1, file)) {
		return false;
	}

	if (!LoadLList(&infected, file)) {
		return false;
	}
...
std::string PlotGenerator::GetID() { return "PLOTGEN"; }
