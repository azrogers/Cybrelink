	...
	*/
	...
	if (comp->traceaction & COMPUTER_TRACEACTION_FINE) {
		...
		if (strcmp(person->name, "PLAYER") == 0) {
			...
			UplinkSnprintf(reason,
						   sizeof(reason),
						   "Our system security agents have caught you making unauthorised access "
						   "to our computer system %s.",
						   comp->name);
	...
	if (comp->traceaction & COMPUTER_TRACEACTION_LEGAL) {
		...
		if (strcmp(person->name, "PLAYER") != 0) {
			...
			reason << "hacking into the " << comp->name << ".\n" << '\x0';
	...
	if (comp->traceaction >= COMPUTER_TRACEACTION_LEGAL) {
		...
		if (hacker == game->GetWorld()->GetPlayer()
			...
			for (int i = 0; i < uplink->missions.Size(); ++i) {
				...
				if (m->TYPE == MISSION_TRACEUSER && strcmp(m->completionA, hacker->name) == 0
			...
			if (!alreadynoticed) {
				MissionGenerator::Generate_TraceHacker(comp, hacker);
			}
		}
	}
}
...
void ConsequenceGenerator::MissionCompleted(Mission* mission, Person* person)
{
	...
	case MISSION_STEALFILE:
		MissionCompleted_StealFile(mission, person);
		break;
	case MISSION_DESTROYFILE:
		MissionCompleted_DestroyFile(mission, person);
		break;
	case MISSION_CHANGEACCOUNT:
		MissionCompleted_ChangeAccount(mission, person);
		break;
	case MISSION_TRACEUSER:
		MissionCompleted_TraceUser(mission, person);
		break;
...
void ConsequenceGenerator::MissionCompleted_ChangeAccount(Mission* mission, Person* person)
{
	...
	if (person == game->GetWorld()->GetPlayer()
		...
		MissionGenerator::Generate_TraceUser_MoneyTransfer(
			employer, source, target, source_acc, target_acc, amount, person);
	}
}
...
void ConsequenceGenerator::MissionCompleted_TraceUser(Mission* mission, Person* person)
{
	...
	if (guiltyperson->GetStatus() == PERSON_STATUS_NONE) {
		CaughtHacking(guiltyperson, comp);
	}
}
