...
Mission* MissionGenerator::Generate_StealFile(Company* employer)
{
	...
	case 1: {
		Computer* target =
			WorldGenerator::GetRandomLowSecurityComputer(COMPUTER_TYPE_INTERNALSERVICESMACHINE);
		UplinkAssert(target);
		if (strcmp(employer->name, target->companyname) == 0) {
			return NULL;
		}
	...
	case 2: {
		Computer* target = WorldGenerator::GetRandomComputer(
			COMPUTER_TYPE_INTERNALSERVICESMACHINE | COMPUTER_TYPE_CENTRALMAINFRAME | COMPUTER_TYPE_LAN);
		UplinkAssert(target);
		if (strcmp(employer->name, target->companyname) == 0) {
			return NULL;
		}
...
Mission* MissionGenerator::Generate_StealSingleFile(Company* employer, Computer* target)
{
	...
	int difficulty = (int)NumberGenerator::RandomNormalNumber(MINDIFFICULTY_MISSION_STEALFILE,
															  DIFFICULTY_MISSION_VARIANCE);
	if (difficulty < MINDIFFICULTY_MISSION_STEALFILE) {
		difficulty = MINDIFFICULTY_MISSION_STEALFILE;
	}

	// Set up the basic variables of the mission

	int payment = (int)NumberGenerator::RandomNormalNumber(
		(float)(difficulty * PAYMENT_MISSION_STEALFILE),
		(float)(difficulty * PAYMENT_MISSION_STEALFILE * PAYMENT_MISSION_VARIANCE));
	...
	int encrypted = difficulty < 3
		? 0
		: (int)NumberGenerator::RandomNormalNumber((float)difficulty, (float)(difficulty / 2));
	if (encrypted > 7) {
		encrypted = 7;
	}
	...
	case 1:
		UplinkStrncpy(
			description, "Steal important data from a rival company's file server", sizeof(description));
		break;
	case 2:
		UplinkStrncpy(
			description, "Access a remote system and copy important data files", sizeof(description));
		break;
	case 3:
		UplinkStrncpy(
			description, "Gain access to a rival system and steal research files", sizeof(description));
		break;
	}
	...
	if (encrypted > 0 && encrypted < 5) {
		fulldetails << "\nThe file will be encrypted.\n";
	} else if (encrypted >= 5) {
		fulldetails << "\nThe file will be heavily encrypted.\n";
	}

	if (compressed > 0 && compressed < 5) {
		fulldetails << "\nThe file will be compressed.\n";
	} else if (compressed >= 5) {
	...
	if (difficulty > 3) {
		mission->SetWhySoMuchMoney("The files contain sensitive data.");
	}
	if (!game->IsRunning()) {
		mission->SetCreateDate(&postdate);
	}
...
Mission* MissionGenerator::Generate_StealAllFiles(Company* employer, Computer* target)
{
	...
	int difficulty = (int)NumberGenerator::RandomNormalNumber(MINDIFFICULTY_MISSION_STEALALLFILES,
															  DIFFICULTY_MISSION_VARIANCE);
	if (difficulty < MINDIFFICULTY_MISSION_STEALALLFILES) {
		difficulty = MINDIFFICULTY_MISSION_STEALALLFILES;
	}

	if (target->TYPE == COMPUTER_TYPE_CENTRALMAINFRAME) {
		difficulty += 1;
	}
	...
	int payment = (int)NumberGenerator::RandomNormalNumber(
		...
		(float)(difficulty * PAYMENT_MISSION_STEALALLFILES * PAYMENT_MISSION_VARIANCE));
	...
	const char* missiontypestring[4] = { "rsrch", "corp", "custmr", "softw" };
	...
	for (int i = 0; i < numfiles; ++i) {
		...
		UplinkSnprintf(datatitle,
					   ...
					   missiontypestring[missiontype - 1],
					   i);
	...
	if (missiontype == 1) {
		...
		fulldetails
			<< "Our establishment is very anxious to find out more about the inner workings "
		...
		UplinkStrncpy(
			description, "Break into High Security System and steal customer records", sizeof(description));

		fulldetails
			<< "It has come to our attention that one of our competitors has amassed an extensive "
		...
		fulldetails
			<< "As you may be aware we are currently in direct competition with another rival corporation "
			   ...
			   "Unfortunately, due to circumstances beyond our control, they have managed to pull several "
			   "months "
			   "ahead.  We believe the best course of action is for us to copy their program source code and "
			   "use "
			   "it to help our product provide adequate competition.\n"
			   "Break into the computer system below, copy anything that looks like source code and break "
			   "any "
			   "security systems in place on the data files.  Then copy those files onto our own file "
			   "server.\n\n";
	}
...
Mission* MissionGenerator::Generate_DestroySingleFile(Company* employer, Computer* target)
{
	...
	int difficulty = (int)NumberGenerator::RandomNormalNumber(MINDIFFICULTY_MISSION_DESTROYFILE,
															  DIFFICULTY_MISSION_VARIANCE);
	if (difficulty < MINDIFFICULTY_MISSION_DESTROYFILE) {
		difficulty = MINDIFFICULTY_MISSION_DESTROYFILE;
	}
	...
	int payment = (int)NumberGenerator::RandomNormalNumber(
		...
		(float)(difficulty * PAYMENT_MISSION_DESTROYFILE * PAYMENT_MISSION_VARIANCE));
	...
	case 1:
		UplinkStrncpy(description, "Find and destroy crucial data on a mainframe", sizeof(description));
		break;
	case 2:
		UplinkStrncpy(
			description, "Break into a rival computer system and sabotage files", sizeof(description));
		break;
	case 3:
		UplinkStrncpy(description, "Hack into a computer and delete key files", sizeof(description));
		break;
	}
...
Mission* MissionGenerator::Generate_DestroyAllFiles(Company* employer, Computer* target)
{
	...
	int difficulty = (int)NumberGenerator::RandomNormalNumber(MINDIFFICULTY_MISSION_DESTROYALLFILES,
															  DIFFICULTY_MISSION_VARIANCE);
	if (difficulty < MINDIFFICULTY_MISSION_DESTROYALLFILES) {
		difficulty = MINDIFFICULTY_MISSION_DESTROYALLFILES;
	}
	...
	int payment = (int)NumberGenerator::RandomNormalNumber(
		...
		(float)(difficulty * PAYMENT_MISSION_DESTROYALLFILES * PAYMENT_MISSION_VARIANCE));
...
Mission* MissionGenerator::Generate_FindData_FinancialRecord(Company* employer, Computer* target)
{
	...
	int difficulty = (int)NumberGenerator::RandomNormalNumber(MINDIFFICULTY_MISSION_FINDDATA_FINANCIAL,
															  DIFFICULTY_MISSION_VARIANCE);
	if (difficulty < MINDIFFICULTY_MISSION_FINDDATA_FINANCIAL) {
		difficulty = MINDIFFICULTY_MISSION_FINDDATA_FINANCIAL;
	}
	...
	int payment = (int)NumberGenerator::RandomNormalNumber(
		...
		(float)(difficulty * PAYMENT_MISSION_FINDDATA_FINANCIAL * PAYMENT_MISSION_VARIANCE));
	...
	if (strcmp(taccount->name, game->GetWorld()->GetPlayer()->handle) == 0) {
		return NULL;
	}
	...
	if (missiontype == 1) {
		...
		BankComputer* receiver =
			(BankComputer*)WorldGenerator::GetRandomComputer(COMPUTER_TYPE_PUBLICBANKSERVER);
		UplinkAssert(receiver);
		BankAccount* raccount = receiver->GetRandomAccount();
		UplinkAssert(raccount);

		if (raccount == taccount) {
			return NULL; // That would be silly ;)
		}
		...
		if (game->GetWorld()->GetPlayer()->IsPlayerAccount(receiver->ip, raccountno)) {
			return NULL;
		}
		if (game->GetWorld()->GetPlayer()->IsPlayerAccount(target->ip, taccountno)) {
			return NULL;
		}
		...
		bool success =
			taccount->TransferMoney(target->ip, taccountno, receiver->ip, raccountno, amount, transferer);
	...
	mission->SetWhoIsTheTarget(whoisthetarget);
	mission->GiveLink(target->ip);
	mission->AttachedBankAccounts.push_back(BankAccountId(target->ip, taccount->accountnumber));
...
Mission* MissionGenerator::Generate_ChangeData(Company* employer)
{
	...
	switch (missiontype) {
	case 0:
		mission = Generate_ChangeData_AcademicRecord(employer);
		break;
	case 1:
		mission = Generate_ChangeData_SocialSecurity(employer);
		break;
	case 2:
		mission = Generate_ChangeData_CriminalRecord(employer);
		break;
	}
...
Mission* MissionGenerator::Generate_ChangeData_AcademicRecord(Company* employer)
{
	...
	int difficulty = (int)NumberGenerator::RandomNormalNumber(MINDIFFICULTY_MISSION_CHANGEDATA_ACADEMIC,
															  DIFFICULTY_MISSION_VARIANCE);
	if (difficulty < MINDIFFICULTY_MISSION_CHANGEDATA_ACADEMIC) {
		difficulty = MINDIFFICULTY_MISSION_CHANGEDATA_ACADEMIC;
	}
	...
	int payment = (int)NumberGenerator::RandomNormalNumber(
		...
		(float)(difficulty * PAYMENT_MISSION_CHANGEDATA_ACADEMIC * PAYMENT_MISSION_VARIANCE));
	...
	if (missiontype == 1) { // Add a degree =====================================================
		...
		const char* personname = record->GetField(RECORDBANK_NAME);
		...
		case 0:
			UplinkStrncpy(degreesubject, "Computing", sizeof(degreesubject));
			break;
		case 1:
			UplinkStrncpy(degreesubject, "Physics", sizeof(degreesubject));
			break;
		case 2:
			UplinkStrncpy(degreesubject, "Electrical Engineering", sizeof(degreesubject));
			break;
		case 3:
			UplinkStrncpy(degreesubject, "Mechanical Engineering", sizeof(degreesubject));
			break;
		case 4:
			UplinkStrncpy(degreesubject, "Software Engineering", sizeof(degreesubject));
			break;
		case 5:
			UplinkStrncpy(degreesubject, "Systems Engineering", sizeof(degreesubject));
			break;
		case 6:
			UplinkStrncpy(degreesubject, "Management", sizeof(degreesubject));
			break;
		case 7:
			UplinkStrncpy(degreesubject, "Artificial Intelligence", sizeof(degreesubject));
			break;
		case 8:
			UplinkStrncpy(degreesubject, "Philosophy", sizeof(degreesubject));
			break;
		case 9:
			UplinkStrncpy(degreesubject, "Media studies", sizeof(degreesubject));
			break;
		...
		case 0:
			UplinkStrncpy(qualification, "Diploma in Computing", sizeof(qualification));
			break;
		case 1:
			UplinkStrncpy(qualification, "IEEE Accreditation", sizeof(qualification));
			break;
		case 2:
			UplinkStrncpy(qualification, "Certified systems engineer", sizeof(qualification));
			break;
		case 3:
			UplinkStrncpy(qualification, "Btec in IT", sizeof(qualification));
			break;
		case 4:
			UplinkStrncpy(qualification, "Registered Uplink Agent", sizeof(qualification));
			break;

		default:
			UplinkAbort(
				"MissionGenerator::Generate_ChangeData_EmploymentRecord, error in Qualification Type");
		}
...
Mission* MissionGenerator::Generate_ChangeData_SocialSecurity(Company* employer)
{
	...
	int difficulty = (int)NumberGenerator::RandomNormalNumber(MINDIFFICULTY_MISSION_CHANGEDATA_SOCIAL,
															  DIFFICULTY_MISSION_VARIANCE);
	if (difficulty < MINDIFFICULTY_MISSION_CHANGEDATA_SOCIAL) {
		difficulty = MINDIFFICULTY_MISSION_CHANGEDATA_SOCIAL;
	}
	...
	int payment = (int)NumberGenerator::RandomNormalNumber(
		...
		(float)(difficulty * PAYMENT_MISSION_CHANGEDATA_SOCIAL * PAYMENT_MISSION_VARIANCE));
	...
	if (missiontype == 1) {
		...
		fulldetails
			<< "The man below is living too easily.  Change his Personal Status to read 'Deceased'.\n";
...
Mission* MissionGenerator::Generate_ChangeData_CriminalRecord(Company* employer)
{
	...
	int difficulty = (int)NumberGenerator::RandomNormalNumber(MINDIFFICULTY_MISSION_CHANGEDATA_CRIMINAL,
															  DIFFICULTY_MISSION_VARIANCE);
	if (difficulty < MINDIFFICULTY_MISSION_CHANGEDATA_CRIMINAL) {
		difficulty = MINDIFFICULTY_MISSION_CHANGEDATA_CRIMINAL;
	}
	...
	int payment = (int)NumberGenerator::RandomNormalNumber(
		...
		(float)(difficulty * PAYMENT_MISSION_CHANGEDATA_CRIMINAL * PAYMENT_MISSION_VARIANCE));
	...
	if (missiontype == 1) {
		...
		UplinkStrncpy(
			whoisthetarget, "We wouldn't want to give his name out just yet.", sizeof(whoisthetarget));
	...
	if (missiontype == 1 || missiontype == 2) {
		mission->SetWhySoMuchMoney("The target may have friends.");
	}
	if (missiontype == 3) {
		mission->SetWhySoMuchMoney("The police may come after you.");
	}
...
Mission* MissionGenerator::Generate_TraceUser_MoneyTransfer(Company* employer,
															BankComputer* source,
															BankComputer* target,
															BankAccount* sacc,
															BankAccount* tacc,
															...
															Person* hacker)
{
	...
	int difficulty = (int)NumberGenerator::RandomNormalNumber(MINDIFFICULTY_MISSION_TRACEUSER_BANKFRAUD,
															  DIFFICULTY_MISSION_VARIANCE);
	if (difficulty < MINDIFFICULTY_MISSION_TRACEUSER_BANKFRAUD) {
		difficulty = MINDIFFICULTY_MISSION_TRACEUSER_BANKFRAUD;
	}

	int payment = (int)NumberGenerator::RandomNormalNumber(
		(float)(difficulty * PAYMENT_MISSION_TRACEUSER),
		(float)(difficulty * PAYMENT_MISSION_TRACEUSER * PAYMENT_MISSION_VARIANCE));
	...
	fulldetails
		<< "One of our bank accounts was recently hacked, and funds were transfered to an unknown account."
	...
	mission->SetWhoIsTheTarget(whoisthetarget);
	mission->AttachedBankAccounts.push_back(BankAccountId(source->ip, sacc->accountnumber));

	if (hacker == game->GetWorld()->GetPlayer()) {
		mission->SetNpcPriority(true);
	}
...
Mission*
MissionGenerator::Generate_PayFine(Person* person, Company* company, int amount, Date* duedate, char* reason)
{
	...
	if (strcmp(person->name, "PLAYER") == 0) {
		UplinkStrncpy(completionA, game->GetWorld()->GetPlayer()->handle, sizeof(completionA));
	}
	...
	fulldetails
		...
		<< "\n\n"
		<< "To pay this fine, access your financial screen and click on the account you wish to transfer "
		   "from.\n"
		<< "This will connect you to that account.\n"
		<< "Then select 'transfer' from the menu and fill in the details as above.\n\n"
		<< "Once you have paid this fine, send a mail to " << personname << " to confirm receipt.\n"
		<< "END." << '\x0';
	...
	mission->SetFullDetails(fulldetails.str());
	mission->AttachedBankAccounts.push_back(BankAccountId(bank->ip, account->accountnumber));
	if (!game->IsRunning()) {
		mission->SetCreateDate(&postdate);
	}
...
Mission* MissionGenerator::Generate_FrameUser(Company* employer, Person* person)
{
	...
	int difficulty = (int)NumberGenerator::RandomNormalNumber(MINDIFFICULTY_MISSION_FRAMEUSER,
															  DIFFICULTY_MISSION_VARIANCE);
	...
	int payment = (int)NumberGenerator::RandomNormalNumber(
		...
		(float)(difficulty * PAYMENT_MISSION_FRAMEUSER * PAYMENT_MISSION_VARIANCE));
	...
	if (missiontype == 1) { // Frame a user for hacking
		...
		fulldetails
			<< "We need this man to be arrested and charged for breaking into a bank and "
		...
		fulldetails
			<< "Our corporation has recently found a way to kill two birds with one "
	...
	if (missiontype == 1) {
		m->SetHowSecure("That depends on what you frame him for.");
	} else if (missiontype == 2) {
		m->SetHowSecure("Banks always have very high security.");
	}

	m->GiveLink(person->localhost);
	if (missiontype == 2 || missiontype == 3) {
		m->GiveLink(completionC);
	}
...
Mission* MissionGenerator::Generate_TraceHacker(Computer* hacked, Person* hacker)
{
	...
	int difficulty = (int)NumberGenerator::RandomNormalNumber(MINDIFFICULTY_MISSION_TRACEUSER,
															  DIFFICULTY_MISSION_VARIANCE);
	if (difficulty < MINDIFFICULTY_MISSION_TRACEUSER) {
		difficulty = MINDIFFICULTY_MISSION_TRACEUSER;
	}
	...
	fulldetails << "Our computer systems were recently compromised by a computer hacker.\n"
				...
				<< game->GetWorld()->date.GetShortString() << ".\n";
	...
	fulldetails << "Once completed, send the name of the hacker to " << contact
				<< " and payment will be delivered.\n"
				   "END"
				<< '\x0';
...
Mission* MissionGenerator::Generate_ChangeAccount(Company* employer, Computer* source, Computer* target)
{
	...
	int difficulty = (int)NumberGenerator::RandomNormalNumber(MINDIFFICULTY_MISSION_CHANGEACCOUNT,
															  DIFFICULTY_MISSION_VARIANCE);
	if (difficulty < MINDIFFICULTY_MISSION_CHANGEACCOUNT) {
		difficulty = MINDIFFICULTY_MISSION_CHANGEACCOUNT;
	}
	...
	int payment = (int)NumberGenerator::RandomNormalNumber(
		...
		(float)(difficulty * PAYMENT_MISSION_CHANGEACCOUNT * PAYMENT_MISSION_VARIANCE));
	...
	if (strcmp(sourceaccount->name, game->GetWorld()->GetPlayer()->handle) == 0) {
		return NULL;
	}
	if (strcmp(targetaccount->name, game->GetWorld()->GetPlayer()->handle) == 0) {
		return NULL;
	}

	int amount_to_transfer = (int)NumberGenerator::RandomNormalNumber(sourceaccount->balance / 2.0f,
																	  sourceaccount->balance * 0.4f);
	amount_to_transfer = int(amount_to_transfer / 500) * 500;

	if (amount_to_transfer <= 0) {
		return NULL;
	}
	if (amount_to_transfer > sourceaccount->balance) {
		amount_to_transfer = (int)(sourceaccount->balance * 0.9);
	}
	...
	mission->GiveLink(source->ip);
	mission->SetWhoIsTheTarget(whoisthetarget);
	mission->AttachedBankAccounts.push_back(BankAccountId(target->ip, targetaccount->accountnumber));
	mission->AttachedBankAccounts.push_back(BankAccountId(source->ip, sourceaccount->accountnumber));
...
Mission* MissionGenerator::Generate_RemoveComputer(Company* employer, Computer* target)
{
	...
	int difficulty = (int)NumberGenerator::RandomNormalNumber(MINDIFFICULTY_MISSION_REMOVECOMPUTER,
															  DIFFICULTY_MISSION_VARIANCE);
	if (difficulty < MINDIFFICULTY_MISSION_REMOVECOMPUTER) {
		difficulty = MINDIFFICULTY_MISSION_REMOVECOMPUTER;
	}
	...
	int payment = (int)NumberGenerator::RandomNormalNumber(
		...
		(float)(difficulty * PAYMENT_MISSION_REMOVECOMPUTER * PAYMENT_MISSION_VARIANCE));
	...
	UplinkStrncpy(
		description, "Elite agent required for destruction of computer system", sizeof(description));
	...
	fulldetails
		<< "One of our competitors has made the critical mistake of becoming over dependent "
		...
		<< personname
		<< " and we will determine if you have "
		   "been successful.\n"
		   "END"
		<< '\x0';
...
Mission* MissionGenerator::Generate_RemoveCompany(Company* employer) { return NULL; }

Mission* MissionGenerator::Generate_RemoveUser(Company* employer)
{

	int difficulty = (int)NumberGenerator::RandomNormalNumber(MINDIFFICULTY_MISSION_REMOVEUSER,
															  DIFFICULTY_MISSION_VARIANCE);
	if (difficulty < MINDIFFICULTY_MISSION_REMOVEUSER) {
		difficulty = MINDIFFICULTY_MISSION_REMOVEUSER;
	}
	...
	int payment = (int)NumberGenerator::RandomNormalNumber(
		...
		(float)(difficulty * PAYMENT_MISSION_REMOVEUSER * PAYMENT_MISSION_VARIANCE));
...
bool MissionGenerator::IsMissionComplete(Mission* mission, Person* person, Message* message)
{
	...
	case MISSION_STEALFILE:
		return IsMissionComplete_StealFile(mission, person, message);
		break;
	case MISSION_DESTROYFILE:
		return IsMissionComplete_DestroyFile(mission, person, message);
		break;
	case MISSION_FINDDATA:
		return IsMissionComplete_FindFinancial(mission, person, message);
		break;
	case MISSION_CHANGEDATA:
		return IsMissionComplete_ChangeData(mission, person, message);
		break;
	case MISSION_FRAMEUSER:
		return IsMissionComplete_FrameUser(mission, person, message);
		break;
	case MISSION_TRACEUSER:
		return IsMissionComplete_TraceUser(mission, person, message);
		break;
	case MISSION_CHANGEACCOUNT:
		return IsMissionComplete_ChangeAccount(mission, person, message);
		break;
	case MISSION_REMOVECOMPUTER:
		return IsMissionComplete_RemoveComputer(mission, person, message);
		break;
	case MISSION_REMOVEUSER:
		return IsMissionComplete_RemoveUser(mission, person, message);
		break;
	case MISSION_PAYFINE:
		return IsMissionComplete_PayFine(mission, person, message);
		break;

	case MISSION_SPECIAL:
		return IsMissionComplete_Special(mission, person, message);
		break;
...
bool MissionGenerator::IsMissionComplete_StealFile(Mission* mission, Person* person, Message* message)
{
	...
	if (msgdata && strcmp(msgdata->title, datatitle) == 0 && msgdata->encrypted == 0) {

		MissionCompleted(mission, person, message);
		return true;

	} else if (msgdata && strcmp(msgdata->title, datatitle) == 0 && msgdata->encrypted > 0) {
		...
		reason << "Your mission is not yet complete.\n"
			   ...
			   << "You will not be paid until this has been accomplished." << '\x0';
...
bool MissionGenerator::IsMissionComplete_StealAllFiles(Mission* mission, Person* person, Message* message)
{
	...
	if (strcmp(datatype, "SCIENTIFIC") == 0) {
		UplinkStrncpy(missiontypestring, "rsrch", sizeof(missiontypestring));
	} else if (strcmp(datatype, "RESEARCH") == 0) {
		UplinkStrncpy(missiontypestring,
					  "rsrch",
					  sizeof(missiontypestring)); // Not needed anymore, but older user files may have this
	} else if (strcmp(datatype, "CORPORATE") == 0) {
		UplinkStrncpy(missiontypestring, "corp", sizeof(missiontypestring));
	} else if (strcmp(datatype, "CUSTOMER") == 0) {
		UplinkStrncpy(missiontypestring, "custmr", sizeof(missiontypestring));
	} else if (strcmp(datatype, "SOFTWARE") == 0) {
		UplinkStrncpy(missiontypestring, "softw", sizeof(missiontypestring));
	} else {
		UplinkAbort("Unrecognised data type");
	}

	char stolendatatitle[64];
	UplinkSnprintf(stolendatatitle,
				   sizeof(stolendatatitle),
				   "%c%c%c%c-%s",
				   target->companyname[0],
				   target->companyname[1],
				   target->companyname[2],
	...
	if (foundsize == 0) {
		...
		if (foundencrypted) {
			MissionNotCompleted(mission,
								person,
								message,
								"We can see some files on the fileserver but they "
			...
			MissionNotCompleted(mission,
								person,
								message,
								"We can't find any of the files on the "
		...
		if (foundencrypted) {
			MissionNotCompleted(mission,
								person,
								message,
								"You haven't recovered very much data and a lot of it "
			...
			MissionNotCompleted(mission,
								person,
								message,
								"You haven't copied very much of the data - "
		...
		if (mission->payment < 100) {
			mission->payment = 100;
		}

		char newmessage[256];
		UplinkSnprintf(newmessage,
					   sizeof(newmessage),
...
bool MissionGenerator::IsMissionComplete_DestroyFile(Mission* mission, Person* person, Message* message)
{
	...
	if (strcmp(filename, "ALL") == 0) {
		...
		for (int in = 0; in < uplink->news.Size(); ++in) {
			...
			if (news->NEWSTYPE == NEWS_TYPE_COMPUTERDESTROYED && news->data1
				&& strcmp(news->data1, ip) == 0) {
		...
		MissionNotCompleted(
			mission, person, message, "We have not heard word yet that the damage has been done.");
		return false;
		...
		if (!data) {
			...
			MissionNotCompleted(
				mission, person, message, "According to our sources, the file has not yet been deleted.");
			return false;
		}
	}
}
...
bool MissionGenerator::IsMissionComplete_FindData(Mission* mission, Person* person, Message* message)
{
	...
	if (value1 && !(strstr(msgbody, value1))) {
		success = false;
	}
	if (value2 && !(strstr(msgbody, value2))) {
		success = false;
	}
	if (value3 && !(strstr(msgbody, value3))) {
		success = false;
	}
	if (value4 && !(strstr(msgbody, value4))) {
		success = false;
	}
	if (value5 && !(strstr(msgbody, value5))) {
		success = false;
	}
	...
	if (strcmp(field, "Balance") == 0) {
		...
		if (strstr(message->GetBody(), sbalance) != NULL) {
			...
			MissionNotCompleted(
				...
				person,
				message,
				"Could you please verify your results.  We have reason to believe they are not accurate.");
			return false;
		}
		...
		if (ispersonresponsible) {
			...
			MissionNotCompleted(mission,
								person,
								message,
								"Could you please verify your results.  We have reason to believe that "
...
bool MissionGenerator::IsMissionComplete_ChangeData(Mission* mission, Person* person, Message* message)
{
	...
	if (rec) {
		...
		const char* fieldvalue = rec->GetField(fieldname);
...
bool MissionGenerator::IsMissionComplete_FrameUser(Mission* mission, Person* person, Message* message)
{
	...
	if (!arrested) {
		...
		MissionNotCompleted(mission,
							person,
							message,
							"That person has not yet been arrested.  We will not make payment until then.");
		return false;
		...
		if (strcmp(framedfor, "HACKING") == 0) { // Bog standard hack
			...
			for (int in = 0; in < uplink->news.Size(); ++in) {
				...
				if (news->NEWSTYPE == NEWS_TYPE_DELETED && news->data1 && ip
					&& strcmp(news->data1, ip) == 0) {
		...
		MissionFailed(mission,
					  person,
					  "The person was arrested but not for the crimes we specifically asked for.  As such we "
					  "will not be making payment.");
		return false;
	}
}
...
bool MissionGenerator::IsMissionComplete_TraceUser(Mission* mission, Person* person, Message* message)
{
	...
	char* targetaccount = mission->completionC ? LowerCaseString(mission->completionC)
	...
	if (strstr(msgbody, personname) != NULL) {
		...
		if (targetaccount && strstr(msgbody, targetaccount) != NULL) {
			...
			MissionNotCompleted(mission,
								person,
								message,
								"We've determined that the money was transfered into "
...
bool MissionGenerator::IsMissionComplete_ChangeAccount(Mission* mission, Person* person, Message* message)
{
	...
	if (hasLogs
		|| ((source_balance - amount) >= source_account->balance
			&& (target_balance + amount) <= target_account->balance)) {
		...
		MissionNotCompleted(
			mission, person, message, "A transfer seem to have occured, verify the amount transfered.");
...
bool MissionGenerator::IsMissionComplete_RemoveComputer(Mission* mission, Person* person, Message* message)
{
	...
	for (int in = 0; in < uplink->news.Size(); ++in) {
		...
		} else if (news->NEWSTYPE == NEWS_TYPE_COMPUTERSHUTDOWN && news->data1
				   && strcmp(news->data1, sourceip) == 0) {
	...
	MissionNotCompleted(
		mission, person, message, "We have not heard word yet that the system has been destroyed.");
	return false;
}
...
bool MissionGenerator::IsMissionComplete_PayFine(Mission* mission, Person* person, Message* message)
{
	...
	if (success) {
		...
		body << "We have still not received payment for the following fine: \n"
			 << mission->GetDetails() << "\n\n"
			 << "Please ensure this payment reaches us soon to avoid further action." << '\x0';
	...
	if (strcmp(mission->description, PlotGenerator::SpecialMissionDescription(SPECIALMISSION_MOLE)) == 0) {
		...
		for (int i = 0; i < ourcomp->databank.GetDataSize(); ++i) {
			if (ourcomp->databank.GetDataFile(i)) {
				...
				if (strstr(thisfile->title, "Uplink_Agent_Data") != NULL && thisfile->encrypted == 0) {
				...
				if (strcmp(thisfile->title, "Uplink_Agent_List") == 0 && thisfile->encrypted == 0) {
		...
		if (foundsize == 0 && foundprogram == false) {
			...
			MissionNotCompleted(mission,
								person,
								message,
								"We don't see any data on our fileserver that contains "
			...
			MissionNotCompleted(mission,
								person,
								message,
								"Looks like you've uploaded the Uplink Agent List program, "
			...
			if (foundprogram) {
				payment += (int)(PAYMENT_SPECIALMISSION_MOLE * 0.25);
			}
			...
			if (foundprogram) {
				...
				body << "Unfortunately you didnt upload the List program used to access the data. This will "
						"make "
			...
			if (payment == PAYMENT_SPECIALMISSION_MOLE) {
				...
				body << "Based on this, we will only be paying you " << payment
					 << " credits for the mission, "
						"out of an available "
					 << PAYMENT_SPECIALMISSION_MOLE << ".\n\n";
			}
...
void MissionGenerator::MissionCompleted(Mission* mission, Person* person, Message* message)
{
	...
	body << "\n"
		 ...
		 << "THIS IS AN AUTOMATED RESPONSE.  PLEASE DO NOT REPLY." << '\x0';
	...
	if (mission->paymentmethod == MISSIONPAYMENT_HALFATSTART) {
		game->GetWorld()->GetPlayer()->ChangeBalance(mission->payment / 2, "Anonymous benefactor");
	} else if (mission->paymentmethod == MISSIONPAYMENT_AFTERCOMPLETION) {
		game->GetWorld()->GetPlayer()->ChangeBalance(mission->payment, "Anonymous benefactor");
	}
...
void MissionGenerator::MissionNotCompleted(Mission* mission,
										   Person* person,
										   Message* message,
										   const char* reason)
{
...
void MissionGenerator::MissionFailed(Mission* mission, Person* person, const char* reason)
{
