...
Connection::~Connection() { DeleteLListData(&vlocations); }

void Connection::SetOwner(const char* newowner)
{
...
void Connection::AddOrRemoveVLocation(const char* ip)
{
...
int Connection::GetSize() { return vlocations.Size(); }
...
void Connection::EndTrace() { traceinprogress = false; }
...
void Connection::Connect()
{
	...
	if (comp->security.IsAnythingDisabled()) {
		BeginTrace();
	}
		...
		if (bouncecomp) {
			...
			if (i == vlocations.Size() - 1) {
				...
				log->SetProperties(&(game->GetWorld()->date),
								   vlocations.GetData(i - 1),
								   owner,
								   LOG_NOTSUSPICIOUS,
								   LOG_TYPE_CONNECTIONOPENED);
				...
				log->SetProperties(&(game->GetWorld()->date), fromip, owner, LOG_NOTSUSPICIOUS, logtype);
...
void Connection::Disconnect()
{
	...
	int suspicious = (TraceInProgress() && !Traced()) ? LOG_SUSPICIOUS : LOG_NOTSUSPICIOUS;

	AccessLog* log1 = new AccessLog();
	log1->SetProperties(&(game->GetWorld()->date),
						vlocations.GetData(vlocations.Size() - 2),
						owner,
						LOG_NOTSUSPICIOUS,
						LOG_TYPE_CONNECTIONCLOSED);
	...
	if (!suspicious) {
		...
		for (int i = comp->logbank.logs.Size(); i >= 0; --i) {
			if (comp->logbank.logs.ValidIndex(i)) {
				...
				if (testdate.Before(&log1->date)) {
					break; // Every other log is > 5 mins before
				}

				if (al->TYPE == LOG_TYPE_CONNECTIONOPENED && strcmp(al->fromname, owner) == 0
		...
		if (rumbledindex >= 0) {
			...
			for (int i = rumbledindex; i >= 0; --i) {
				...
				if (game->GetWorld()->GetPlayer()->HasAccount(rumbledIP) != -1) {
					...
					if (changed) {
						DisconnectedScreen::AddLoginLost(comp->ip);
					}
				}
			}
		}
...
bool Connection::TraceInProgress() { return traceinprogress; }

bool Connection::Traced() { return (traceinprogress && traceprogress == GetSize() - 1); }

bool Connection::Connected() { return (strcmp(GetOwner()->localhost, GetOwner()->remotehost) != 0); }
...
bool Connection::Load(FILE* file)
{
	...
	if (strcmp(game->GetLoadedSavefileVer(), "SAV59") >= 0) {
		if (!LoadDynamicStringStatic(owner, sizeof(owner), file)) {
			return false;
		}
	} else {
	...
	if (!FileReadData(&traceinprogress, sizeof(traceinprogress), 1, file)) {
		return false;
	}
	if (!FileReadData(&traceprogress, sizeof(traceprogress), 1, file)) {
		return false;
	}

	if (!LoadLList(&vlocations, file)) {
		return false;
	}
...
std::string Connection::GetID() { return "CONNECT"; }
