...
void Message::SetTo(const char* newto)
{
...
void Message::SetFrom(const char* newfrom)
{
...
void Message::SetSubject(const char* newsubject)
{
...
void Message::SetBody(const char* newbody)
{
...
void Message::SetDate(Date* newdate) { date.SetDate(newdate); }

void Message::GiveLink(const char* ip)
{
...
void Message::GiveCode(const char* ip, const char* code)
{
...
bool Message::Load(FILE* file)
{
	...
	if (!LoadDynamicStringStatic(to, SIZE_PERSON_NAME, file)) {
		return false;
	}
	if (!LoadDynamicStringStatic(from, SIZE_PERSON_NAME, file)) {
		return false;
	}
	...
	if (!LoadDynamicStringPtr(&subject, file)) {
		return false;
	}
	if (body) {
		delete[] body;
	...
	if (!LoadDynamicStringPtr(&body, file)) {
		return false;
	}
...
std::string Message::GetID() { return "MESSAGE"; }

int Message::GetOBJECTID() { return OID_MESSAGE; }
