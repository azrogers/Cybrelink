...
void News::SetDate(Date* newdate) { date.SetDate(newdate); }

void News::SetHeadline(const char* newheadline)
{
...
void News::SetDetails(const char* newdetails)
{
...
char* News::GetDetails() { return details; }
...
bool News::Load(FILE* file)
{
	...
	if (!FileReadData(&NEWSTYPE, sizeof(NEWSTYPE), 1, file)) {
		return false;
	}

	if (!LoadDynamicStringStatic(headline, SIZE_NEWS_HEADLINE, file)) {
		return false;
	}
	if (!LoadDynamicStringStatic(data1, SIZE_NEWS_DATA, file)) {
		return false;
	}
	if (!LoadDynamicStringStatic(data2, SIZE_NEWS_DATA, file)) {
		return false;
	}
...
std::string News::GetID() { return "NEWS"; }

int News::GetOBJECTID() { return OID_NEWS; }
