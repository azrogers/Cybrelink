...
void Company::SetBoss(const char* bossname)
{
...
void Company::SetSize(int newsize) { size = newsize; }

void Company::SetTYPE(int newTYPE) { TYPE = newTYPE; }

void Company::SetGrowth(int newgrowth) { growth = newgrowth; }

void Company::SetAlignment(int newalignment) { alignment = newalignment; }
...
void Company::Grow(int dayspast)
{
	...
	if (sharehistory[month] < 1) {
		sharehistory[month] = 1;
	}
...
void Company::VaryGrowth() { growth = NumberGenerator::ApplyVariance(growth, 20); }
...
int Company::GetShareLastMonthSet() { return lastmonthset; }
...
bool Company::Load(FILE* file)
{
	...
	if (!LoadDynamicStringStatic(name, SIZE_COMPANY_NAME, file)) {
		return false;
	}
	if (!LoadDynamicStringStatic(boss, SIZE_PERSON_NAME, file)) {
		return false;
	}
	if (!LoadDynamicStringStatic(administrator, SIZE_PERSON_NAME, file)) {
		return false;
	}
	...
	if (!FileReadData(&growth, sizeof(growth), 1, file)) {
		return false;
	}
	if (!FileReadData(&alignment, sizeof(alignment), 1, file)) {
		return false;
	}

	for (int i = 0; i < 12; ++i) {
		if (!FileReadData(&sharehistory[i], sizeof(sharehistory[i]), 1, file)) {
			return false;
		}
	}

	if (!FileReadData(&lastmonthset, sizeof(lastmonthset), 1, file)) {
		return false;
	}
...
std::string Company::GetID() { return "COMPANY"; }

int Company::GetOBJECTID() { return OID_COMPANY; }
