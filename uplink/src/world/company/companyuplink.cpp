...
void CompanyUplink::CreateMission(
	char* employer, int payment, char* description, char* details, char* fulldetails)
{
...
Mission* CompanyUplink::GetMission(int index)
{
	...
	if (missions.ValidIndex(index)) {
		return missions.GetData(index);
	} else {
		return NULL;
	}
}
...
Sale* CompanyUplink::GetSWSale(int index)
{
	...
	if (sw_sales.ValidIndex(index)) {
		return sw_sales.GetData(index);
	} else {
		return NULL;
	}
}
...
Sale* CompanyUplink::GetHWSale(int index)
{
	...
	if (hw_sales.ValidIndex(index)) {
		return hw_sales.GetData(index);
	} else {
		return NULL;
	}
}
...
News* CompanyUplink::GetNews(int index)
{
	...
	if (news.ValidIndex(index)) {
		return news.GetData(index);
	} else {
		return NULL;
	}
}
...
bool CompanyUplink::Load(FILE* file)
{
	...
	if (!LoadLList((LList<UplinkObject*>*)&hw_sales, file)) {
		return false;
	}
	if (!LoadLList((LList<UplinkObject*>*)&sw_sales, file)) {
		return false;
	}
	if (!LoadLList((LList<UplinkObject*>*)&news, file)) {
		return false;
	}
...
std::string CompanyUplink::GetID() { return "CPNY_UPL"; }

int CompanyUplink::GetOBJECTID() { return OID_COMPANYUPLINK; }
