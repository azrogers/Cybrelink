...
void BankRobberyEvent::Run()
{
			...
			if (source->log.internallogs.ValidIndex(i)) {
				...
				if (al && al->TYPE == LOG_TYPE_TRANSFERTO) {
					...
					if (al->data1 && al->data2) {
						...
						if (strstr(al->data1, target_ip) != 0 && strstr(al->data1, target_accno) != 0
				...
				if (target->log.internallogs.ValidIndex(i)) {
					...
					if (al && al->TYPE == LOG_TYPE_TRANSFERFROM) {
						...
						if (al->data1 && al->data2) {
							...
							if (strstr(al->data1, source_ip) != 0 && strstr(al->data1, source_accno) != 0
								&& strcmp(al->data2, amount_s) == 0) {
	...
	if (rumbled) {
		...
		deathmsg << "Disavowed by Uplink Corporation at " << game->GetWorld()->date.GetLongString()
				 << "\nFor attempting to steal money from " << comp->name << "\n"
				 << '\x0';
...
char* BankRobberyEvent::GetLongString()
{
	...
	longstring << "Bank Robbery Event\n"
			   ...
			   << "Amount : " << amount << "\n"
			   << hackdate.GetLongString() << '\x0';

	return longstring.str();
}

void BankRobberyEvent::SetDetails(const char* newsourceip,
								  const char* newsourceaccno,
								  const char* newtargetip,
								  const char* newtargetaccno,
								  int newamount,
								  Date* newhackdate)
{
...
bool BankRobberyEvent::Load(FILE* file)
{
	...
	if (strcmp(game->GetLoadedSavefileVer(), "SAV59") >= 0) {
		if (!LoadDynamicStringStatic(source_ip, sizeof(source_ip), file)) {
			return false;
		}
		if (!LoadDynamicStringStatic(source_accno, sizeof(source_accno), file)) {
			return false;
		}
		if (!LoadDynamicStringStatic(target_ip, sizeof(target_ip), file)) {
			return false;
		}
		if (!LoadDynamicStringStatic(target_accno, sizeof(target_accno), file)) {
			return false;
		}
	} else {
	...
	if (!FileReadData(&amount, sizeof(amount), 1, file)) {
		return false;
	}
	if (!hackdate.Load(file)) {
		return false;
	}
...
std::string BankRobberyEvent::GetID() { return "EVT_BANK"; }

int BankRobberyEvent::GetOBJECTID() { return OID_BANKROBBERYEVENT; }
