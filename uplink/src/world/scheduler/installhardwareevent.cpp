...
void InstallHardwareEvent::Run()
{
	...
	if (hwsale->swhwTYPE == GATEWAYUPGRADETYPE_MEMORY) {
		...
		if (gateway->memorysize + sv->data > gd->maxmemory * 8) {
			success = false;
			UplinkStrncpy(
				failure, "There was not enough space to fit the memory into your gateway.", sizeof(failure));
	...
	if (hwsale->swhwTYPE == GATEWAYUPGRADETYPE_SECURITY) {
		...
		if (gateway->GetNumSecurity() + 1 > gd->maxsecurity) {
			success = false;
			UplinkStrncpy(failure,
						  "There was not enough space to fit the security device into your gateway.",
						  sizeof(failure));
			game->GetWorld()->GetPlayer()->ChangeBalance(sv->cost, "Refund from Uplink Corporation");
		} else {
			...
			if (strcmp(hwsale->title, "Gateway Self Destruct") == 0) {
				driver->SetTitle("Gateway_Nuke");
			} else if (strcmp(hwsale->title, "Gateway Motion Sensor") == 0) {
				driver->SetTitle("Motion_Sensor");
			}
	...
	if (success) {
		...
		body << "The following hardware item was not installed "
				"in your gateway machine : \n\n"
			 << hwsale->title << "\n\nReason: " << failure << "\nYour money has been refunded." << '\x0';
	}
...
void InstallHardwareEvent::RunWarning()
{
	...
	body << "We are about to begin installation of the following hardware item "
			"in your gateway machine : \n\n"
		 << hwsale->title << "\n\n"
		 << "We anticipate this will take 30 - 60 minutes to complete." << '\x0';
	Message* msg = new Message();
	msg->SetTo("PLAYER");
	msg->SetFrom("Uplink Corporation");
...
bool InstallHardwareEvent::Load(FILE* file)
{
	...
	bool hwsaleavail;
	if (!FileReadData(&hwsaleavail, sizeof(hwsaleavail), 1, file)) {
		return false;
	}
	if (hwsaleavail) {
		hwsale = new Sale();
...
std::string InstallHardwareEvent::GetID() { return "EVT_IHW"; }

int InstallHardwareEvent::GetOBJECTID() { return OID_INSTALLHWEVENT; }
