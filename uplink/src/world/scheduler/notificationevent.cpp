...
NotificationEvent::NotificationEvent() { TYPE = NOTIFICATIONEVENT_TYPE_NONE; }

NotificationEvent::~NotificationEvent() { }

void NotificationEvent::SetTYPE(int newTYPE) { TYPE = newTYPE; }
...
void NotificationEvent::Run()
{
	...
	case NOTIFICATIONEVENT_TYPE_GROWCOMPANIES:
		ApplyMonthlyGrowth();
		break;
	case NOTIFICATIONEVENT_TYPE_GENNEWMISSION:
		GenerateNewMission();
		break;
	case NOTIFICATIONEVENT_TYPE_CHECKFORSECURITYBREACHES:
		CheckForSecurityBreaches();
		break;
	case NOTIFICATIONEVENT_TYPE_CHECKMISSIONDUEDATES:
		CheckMissionDueDates();
		break;
	case NOTIFICATIONEVENT_TYPE_CHECKRECENTHACKCOUNT:
		CheckRecentHackCount();
		break;
	case NOTIFICATIONEVENT_TYPE_GIVEMISSIONTONPC:
		GiveMissionToNPC();
		break;
	case NOTIFICATIONEVENT_TYPE_UPLINKMONTHLYFEE:
		PayUplinkMonthlyFee();
		break;
	case NOTIFICATIONEVENT_TYPE_EXPIREOLDSTUFF:
		ExpireOldStuff();
		break;
	case NOTIFICATIONEVENT_TYPE_ADDINTERESTONLOANS:
		AddInterestOnLoans();
		break;
	case NOTIFICATIONEVENT_TYPE_DEMOGAMEOVER:
		DemoGameOver();
		break;
	case NOTIFICATIONEVENT_TYPE_DEMOGENNEWMISSION:
		DemoGenerateNewMission();
		break;
	case NOTIFICATIONEVENT_TYPE_BUYAGENTLIST:
		BuyAgentList();
		break;
	case NOTIFICATIONEVENT_TYPE_AGENTSONLISTDIE:
		AgentsOnListDie();
		break;
	case NOTIFICATIONEVENT_TYPE_WAREZGAMEOVER:
		WarezGameOver();
		break;
...
char* NotificationEvent::GetShortString()
{
	...
	case NOTIFICATIONEVENT_TYPE_GROWCOMPANIES:
		shortstring << "Grow companies";
		break;
	case NOTIFICATIONEVENT_TYPE_GENNEWMISSION:
		shortstring << "Generate new mission";
		break;
	case NOTIFICATIONEVENT_TYPE_CHECKFORSECURITYBREACHES:
		shortstring << "Check for security breaches";
		break;
	case NOTIFICATIONEVENT_TYPE_CHECKMISSIONDUEDATES:
		shortstring << "Check mission due dates";
		break;
	case NOTIFICATIONEVENT_TYPE_CHECKRECENTHACKCOUNT:
		shortstring << "Check recent hack count";
		break;
	case NOTIFICATIONEVENT_TYPE_GIVEMISSIONTONPC:
		shortstring << "Give mission to NPC";
		break;
	case NOTIFICATIONEVENT_TYPE_UPLINKMONTHLYFEE:
		shortstring << "Pay Uplink monthly fee";
		break;
	case NOTIFICATIONEVENT_TYPE_EXPIREOLDSTUFF:
		shortstring << "Expire old stuff";
		break;
	case NOTIFICATIONEVENT_TYPE_ADDINTERESTONLOANS:
		shortstring << "Add Interest to loans";
		break;
	case NOTIFICATIONEVENT_TYPE_DEMOGAMEOVER:
		shortstring << "Demo Game Over";
		break;
	case NOTIFICATIONEVENT_TYPE_DEMOGENNEWMISSION:
		shortstring << "Demo Generate Mission";
		break;
	case NOTIFICATIONEVENT_TYPE_BUYAGENTLIST:
		shortstring << "Buy Agent List";
		break;
	case NOTIFICATIONEVENT_TYPE_AGENTSONLISTDIE:
		shortstring << "Agents on list die";
		break;
	case NOTIFICATIONEVENT_TYPE_WAREZGAMEOVER:
		shortstring << "Warez Game Over";
		break;
...
char* NotificationEvent::GetLongString() { return GetShortString(); }
...
void NotificationEvent::ApplyMonthlyGrowth()
{
	...
	duedate.SetDate(duedate.GetSecond(),
					duedate.GetMinute(),
					duedate.GetHour(),
					1,
					duedate.GetMonth(),
...
void NotificationEvent::BuyAgentList() { ScriptLibrary::RunScript(71); }

void NotificationEvent::AgentsOnListDie() { ScriptLibrary::RunScript(72); }
		...
		if (account) {
			...
			if (account->loan > 0) {
				...
				body << "You have been charged interest on the following account:\n"
					 ...
					 << "Your new loan is " << account->loan << "c." << '\x0';
...
std::string NotificationEvent::GetID() { return "EVT_NOT"; }

int NotificationEvent::GetOBJECTID() { return OID_NOTIFICATIONEVENT; }
