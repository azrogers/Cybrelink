...
void WarningEvent::SetEvent(UplinkEvent* newevent) { event = newevent; }
...
bool WarningEvent::Load(FILE* file)
{
	...
	bool eventexists;
	if (!FileReadData(&eventexists, sizeof(eventexists), 1, file)) {
		return false;
	}

	if (eventexists) {
		int objectID;
		if (!FileReadData(&objectID, sizeof(objectID), 1, file)) {
			return false;
		}
...
std::string WarningEvent::GetID() { return "EVT_WARN"; }

int WarningEvent::GetOBJECTID() { return OID_WARNINGEVENT; }
