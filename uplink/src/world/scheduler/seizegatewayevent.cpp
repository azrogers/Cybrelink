...
void SeizeGatewayEvent::SetGatewayID(int newid) { gateway_id = newid; }
...
bool SeizeGatewayEvent::Load(FILE* file)
{
	...
	if (!LoadDynamicStringPtr(&reason, file)) {
		return false;
	}
	if (!FileReadData(&gateway_id, sizeof(gateway_id), 1, file)) {
		return false;
	}
...
std::string SeizeGatewayEvent::GetID() { return "EVT_SEZ"; }

int SeizeGatewayEvent::GetOBJECTID() { return OID_SEIZEGATEWAYEVENT; }
