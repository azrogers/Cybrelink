...
const char* Rating::GetUplinkRatingString(int urating)
{
...
const char* Rating::GetNeuromancerRatingString(int nrating)
{
...
const char* Rating::GetUplinkRating() { return GetUplinkRatingString(uplinkrating); }
...
const char* Rating::GetNeuromancerRating() { return GetNeuromancerRatingString(neuromancerrating); }
...
void Rating::ChangeUplinkScore(int amount)
{
	...
	uplinkscore += amount;
	if (amount < 0 && uplinkscore < UPLINKRATING[2].score) {
	...
	for (int i = 0; i < NUM_UPLINKRATINGS - 1; ++i) {
		if (uplinkscore >= UPLINKRATING[i].score && uplinkscore < UPLINKRATING[i + 1].score) {
			uplinkrating = i;
			break;
		}
	}
	...
	if (strcmp(owner, "PLAYER") == 0) {
		...
		if (uplinkrating == 6 && oldrating < 6) {
			if (!game->GetWorld()->GetPlayer()->gateway.HasHUDUpgrade(HUDUPGRADE_CONNECTIONANALYSIS)) {
				...
				m->SetBody(
					"Firstly allow us to congratulate you on your recent successes.  We have been following "
					"your "
					...
					"This item of software was designed by Uplink Corporation specifically for bypassing "
					"security "
	...
	if (uplinkrating > oldrating) {
		...
		if (game->GetWorld()->GetPerson(owner)) {
			...
			char* agenthandle = strcmp(owner, "PLAYER") != 0 ? owner : game->GetWorld()->GetPlayer()->handle;
			std::ostrstream body;
			body << "This is a warning Agent " << agenthandle
				 << " - your performance recently has been unsatisfactory.\n"
					"As such we have been forced to review your case, and have decided to decrease your "
					"Uplink rating.\n\n"
					"Your rating is now "
				 << GetUplinkRating()
				 << "."
...
void Rating::ChangeNeuromancerScore(int amount)
{
	...
	if (neuromancerrating > oldrating) {
		...
		if (game->GetWorld()->GetPerson(owner)) {
			...
			UplinkSnprintf(body,
						   sizeof(body),
						   "Agent %s - your Neuromancer rating appears to have changed.\n\n"
		...
		if (game->GetWorld()->GetPerson(owner)) {
			...
			UplinkSnprintf(
				body,
				sizeof(body),
				"It appears your neuromancer rating has changed, in view of your recent actions.\n\n"
...
void Rating::SetCreditRating(int rating) { creditrating = rating; }

void Rating::ChangeCreditRating(int amount) { creditrating += amount; }
...
bool Rating::Load(FILE* file)
{
	...
	if (!LoadDynamicStringStatic(owner, SIZE_PERSON_NAME, file)) {
		return false;
	}

	if (!FileReadData(&uplinkscore, sizeof(uplinkscore), 1, file)) {
		return false;
	}
	if (!FileReadData(&neuromancerscore, sizeof(neuromancerscore), 1, file)) {
		return false;
	}
	if (!FileReadData(&uplinkrating, sizeof(uplinkrating), 1, file)) {
		return false;
	}
	if (!FileReadData(&neuromancerrating, sizeof(neuromancerrating), 1, file)) {
		return false;
	}
	if (!FileReadData(&creditrating, sizeof(creditrating), 1, file)) {
		return false;
	}
...
std::string Rating::GetID() { return "RATING"; }
