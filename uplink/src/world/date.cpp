...
static char monthname[][12] = { "January", "February", "March",		"April",   "May",	   "June",
...
void Date::Activate() { updateme = true; }

void Date::DeActivate() { updateme = false; }
...
void Date::AdvanceYear(int n) { year += n; }
...
bool Date::Load(FILE* file)
{
	...
	if (!FileReadData(&second, sizeof(second), 1, file)) {
		return false;
	}
	if (!FileReadData(&minute, sizeof(minute), 1, file)) {
		return false;
	}
	if (!FileReadData(&hour, sizeof(hour), 1, file)) {
		return false;
	}
	if (!FileReadData(&day, sizeof(day), 1, file)) {
		return false;
	}
	if (!FileReadData(&month, sizeof(month), 1, file)) {
		return false;
	}
	if (!FileReadData(&year, sizeof(year), 1, file)) {
		return false;
	}

	if (!FileReadData(&updateme, sizeof(updateme), 1, file)) {
		return false;
	}
...
void Date::Print()
{
	...
	if (updateme) {
		printf("Syncronised with real world time\n");
	} else {
		printf("Not Syncronised with real world time\n");
	}
}
...
std::string Date::GetID() { return "DATE"; }
