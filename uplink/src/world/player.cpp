...
bool Player::IsPlayerAccount(const char* bankip, const char* accno)
{
...
void Player::TradeShares(char* companyname, int howmany)
{
	...
	if (!stringdata) { // No existing transactions
		...
		if (howmany > 0) { // We are buying more
			...
			if (howmany * company->GetSharePrice() > GetBalance()) {
...
void Player::Update()
{
	...
	if (IsConnected() && GetRemoteHost()->GetComputer()->tracespeed > -1
		&& GetConnection()->TraceInProgress()) {
		...
		if (GetConnection()->traceprogress < GetConnection()->GetSize() - 1) {
			...
			if (game->GetWorld()->date.After(&(GetConnection()->nexttrace))) {
				...
				char* thisip = GetConnection()->vlocations.GetData(GetConnection()->GetSize()
																   - GetConnection()->traceprogress - 1);
				UplinkAssert(thisip);

				int timetonexttrace = TimeToTrace(GetRemoteHost()->ip, thisip);
				timetonexttrace =
					NumberGenerator::ApplyVariance(timetonexttrace, (int)(TRACESPEED_VARIANCE * 100));
...
std::string Player::GetID() { return "PLAYER"; }

int Player::GetOBJECTID() { return OID_PLAYER; }
