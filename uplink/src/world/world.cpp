...
VLocation* World::CreateVLocation(const char* ip, int phys_x, int phys_y)
{
...
bool World::VerifyVLocation(const char* ip, int phys_x, int phys_y)
{
...
Computer* World::CreateComputer(const char* name, const char* companyname, const char* ip)
{
...
Person* World::CreatePerson(const char* name, const char* localhost)
{
...
GatewayDef* World::GetGatewayDef(const char* name)
{
...
bool World::Load(FILE* file)
{
	...
	if (!LoadBTree((BTree<UplinkObject*>*)&locations, file)) {
		return false;
	}
	if (!LoadBTree((BTree<UplinkObject*>*)&companies, file)) {
		return false;
	}
	if (!LoadBTree((BTree<UplinkObject*>*)&computers, file)) {
		return false;
	}
	if (!LoadBTree((BTree<UplinkObject*>*)&people, file)) {
		return false;
	}
...
std::string World::GetID() { return "WORLD"; }
