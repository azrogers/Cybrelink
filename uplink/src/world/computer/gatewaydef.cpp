...
void GatewayDef::LoadGatewayDefinition(istream& thefile)
{
	...
	thefile >> cost >> maxcpus >> maxmemory >> maxupgrades >> maxsecurity >> bandwidth >> width >> height
		>> ws;
	...
	if (strstr(description,
			   "An embedded broad-band radio transmitter provides total anonymity for the user - you cannot "
			   "be caught with this gateway.")) {
...
void GatewayDef::SetCost(int newcost) { cost = newcost; }

void GatewayDef::SetMaxMemory(int newmaxmemory) { maxmemory = newmaxmemory; }

void GatewayDef::SetMaxUpgrades(int newmaxupgrades) { maxupgrades = newmaxupgrades; }

void GatewayDef::SetMaxSecurity(int newmaxsecurity) { maxsecurity = newmaxsecurity; }

void GatewayDef::SetBandwidth(int newbandwidth) { bandwidth = newbandwidth; }
...
static bool LoadDArrayGatewayDefLocation(DArray<GatewayDefLocation*>* darray, FILE* file)
{
	...
	if (size < 0 || size > MAX_ITEMS_DATA_STRUCTURE) {
		UplinkPrintAbortArgs(
			"WARNING: LoadDArrayGatewayDefLocation, number of items appears to be wrong, size=%d", size);
		return false;
	}
	...
	for (int i = 0; i < size; ++i) {
		...
		if (index == -1) {
			...
			UplinkPrintAbortArgs(
				"WARNING: LoadDArrayGatewayDefLocation, number of items appears to be wrong, index=%d",
				index);
			return false;
...
bool GatewayDef::Load(FILE* file)
{
	...
	if (!LoadDynamicStringStatic(name, sizeof(name), file)) {
		return false;
	}
	if (!LoadDynamicStringStatic(description, sizeof(description), file)) {
		return false;
	}
	if (!LoadDynamicStringStatic(filename, sizeof(filename), file)) {
		return false;
	}
	if (!LoadDynamicStringStatic(thumbnail, sizeof(thumbnail), file)) {
		return false;
	}
	...
	if (!FileReadData(&maxcpus, sizeof(maxcpus), 1, file)) {
		return false;
	}
	if (!FileReadData(&maxmemory, sizeof(maxmemory), 1, file)) {
		return false;
	}
	if (!FileReadData(&maxupgrades, sizeof(maxupgrades), 1, file)) {
		return false;
	}
	if (!FileReadData(&maxsecurity, sizeof(maxsecurity), 1, file)) {
		return false;
	}
	if (!FileReadData(&bandwidth, sizeof(bandwidth), 1, file)) {
		return false;
	}

	if (!FileReadData(&width, sizeof(width), 1, file)) {
		return false;
	}
	if (!FileReadData(&height, sizeof(height), 1, file)) {
		return false;
	}

	if (!LoadDArrayGatewayDefLocation(&cpus, file)) {
		return false;
	}
	if (!LoadDArrayGatewayDefLocation(&memory, file)) {
		return false;
	}
	if (!LoadDArrayGatewayDefLocation(&security, file)) {
		return false;
	}

	if (!FileReadData(&modemX, sizeof(modemX), 1, file)) {
		return false;
	}
	if (!FileReadData(&modemY, sizeof(modemY), 1, file)) {
		return false;
	}
	if (!FileReadData(&powerX, sizeof(powerX), 1, file)) {
		return false;
	}
	if (!FileReadData(&powerY, sizeof(powerY), 1, file)) {
		return false;
	}
...
std::string GatewayDef::GetID() { return "GATEWAYDEF"; }
