...
void ComputerScreen::SetMainTitle(const char* newmaintitle)
{
...
void ComputerScreen::SetSubTitle(const char* newsubtitle)
{
...
Computer* ComputerScreen::GetComputer() { return game->GetWorld()->GetComputer(computer); }
...
bool ComputerScreen::Load(FILE* file)
{
	...
	if (!LoadDynamicStringStatic(maintitle, SIZE_COMPUTERSCREEN_MAINTITLE, file)) {
		return false;
	}
	if (!LoadDynamicStringStatic(subtitle, SIZE_COMPUTERSCREEN_SUBTITLE, file)) {
		return false;
	}
...
std::string ComputerScreen::GetID() { return "COMPSCR"; }

int ComputerScreen::GetOBJECTID() { return OID_UNKNOWN; }
