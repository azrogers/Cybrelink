...
void DialogScreen::AddWidget(const char* name,
							 int WIDGETTYPE,
							 int x,
							 int y,
							 int width,
...
void DialogScreen::RemoveWidget(const char* name)
{
...
void DialogScreen::SetReturnKeyButton(const char* name)
{
...
void DialogScreen::SetEscapeKeyButton(const char* name)
{
...
bool DialogScreen::Load(FILE* file)
{
	...
	if (!LoadLList((LList<UplinkObject*>*)&widgets, file)) {
		return false;
	}

	if (!LoadDynamicStringPtr(&returnkeybutton, file)) {
		return false;
	}
	if (!LoadDynamicStringPtr(&escapekeybutton, file)) {
		return false;
	}
...
std::string DialogScreen::GetID() { return "SCR_DLG"; }

int DialogScreen::GetOBJECTID() { return OID_DIALOGSCREEN; }
...
void DialogScreenWidget::SetName(const char* newname)
{
...
void DialogScreenWidget::SetTYPE(int NEWWIDGETTYPE) { WIDGETTYPE = NEWWIDGETTYPE; }
...
void DialogScreenWidget::SetTooltip(const char* newtooltip)
{
...
void DialogScreenWidget::SetCaption(const char* newcaption)
{
...
char* DialogScreenWidget::GetName() { return name; }

char* DialogScreenWidget::GetTooltip() { return tooltip; }

char* DialogScreenWidget::GetCaption() { return caption; }
...
void DialogScreenWidget::SetStringData(const char* newstringdata1, const char* newstringdata2)
{
...
char* DialogScreenWidget::GetStringData1() { return stringdata1; }

char* DialogScreenWidget::GetStringData2() { return stringdata2; }
...
bool DialogScreenWidget::Load(FILE* file)
{
	...
	if (!LoadDynamicStringPtr(&name, file)) {
		return false;
	}

	if (!FileReadData(&x, sizeof(x), 1, file)) {
		return false;
	}
	if (!FileReadData(&y, sizeof(y), 1, file)) {
		return false;
	}
	...
	if (!FileReadData(&WIDGETTYPE, sizeof(WIDGETTYPE), 1, file)) {
		return false;
	}

	if (!LoadDynamicStringPtr(&caption, file)) {
		return false;
	}
	if (!LoadDynamicStringPtr(&tooltip, file)) {
		return false;
	}

	if (!FileReadData(&data1, sizeof(data1), 1, file)) {
		return false;
	}
	if (!FileReadData(&data2, sizeof(data2), 1, file)) {
		return false;
	}

	if (!LoadDynamicStringPtr(&stringdata1, file)) {
		return false;
	}
	if (!LoadDynamicStringPtr(&stringdata2, file)) {
		return false;
	}
...
std::string DialogScreenWidget::GetID() { return "DLG_WIDG"; }

int DialogScreenWidget::GetOBJECTID() { return OID_DIALOGSCREENWIDGET; }
