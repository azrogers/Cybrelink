...
void LogScreen::SetNextPage(int newnextpage) { nextpage = newnextpage; }

void LogScreen::SetTARGET(int newTARGET) { TARGET = newTARGET; }
...
bool LogScreen::Load(FILE* file)
{
	...
	if (!FileReadData(&TARGET, sizeof(TARGET), 1, file)) {
		return false;
	}
...
std::string LogScreen::GetID() { return "SCR_LOGS"; }

int LogScreen::GetOBJECTID() { return OID_LOGSCREEN; }
