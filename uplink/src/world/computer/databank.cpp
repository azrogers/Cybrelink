...
DataBank::DataBank() { formatted = false; }

DataBank::~DataBank() { DeleteDArrayData((DArray<UplinkObject*>*)&data); }

void DataBank::SetSize(int newsize) { memory.SetSize(newsize); }

int DataBank::GetSize() { return memory.Size(); }

int DataBank::NumDataFiles() { return data.NumUsed(); }

int DataBank::GetDataSize() { return data.Size(); }
...
Data* DataBank::GetData(const char* title)
{
...
bool DataBank::ContainsData(const char* title, float version)
{
...
bool DataBank::Load(FILE* file)
{
	...
	if (!LoadDArray((DArray<UplinkObject*>*)&data, file)) {
		return false;
	}
	if (!LoadDArray(&memory, file)) {
		return false;
	}

	if (!FileReadData(&formatted, sizeof(formatted), 1, file)) {
		return false;
	}
...
std::string DataBank::GetID() { return "DATABANK"; }

int DataBank::GetOBJECTID() { return OID_DATABANK; }
...
void Data::SetTitle(const char* newtitle)
{
...
void Data::SetDetails(
	int newTYPE, int newsize, int newencrypted, int newcompressed, float newversion, int newsoftwareTYPE)
{
...
bool Data::Load(FILE* file)
{
	...
	if (!LoadDynamicStringStatic(title, SIZE_DATA_TITLE, file)) {
		return false;
	}
	...
	if (!FileReadData(&encrypted, sizeof(encrypted), 1, file)) {
		return false;
	}
	if (!FileReadData(&compressed, sizeof(compressed), 1, file)) {
		return false;
	}
	if (!FileReadData(&version, sizeof(version), 1, file)) {
		return false;
	}
	if (!FileReadData(&softwareTYPE, sizeof(softwareTYPE), 1, file)) {
		return false;
	}
...
std::string Data::GetID() { return "DATA"; }

int Data::GetOBJECTID() { return OID_DATA; }
