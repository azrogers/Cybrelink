...
bool LogBank::LogModified(int index)
{
	...
	return (log->TYPE != internallog->TYPE || log->SUSPICIOUS != internallog->SUSPICIOUS
			|| strcmp(log->fromip, internallog->fromip) != 0
			|| strcmp(log->fromname, internallog->fromname) != 0 || !log->date.Equal(&internallog->date));
}
...
char* LogBank::TraceLog(char* to_ip, char* logbank_ip, Date* date, int uplinkrating)
{
	...
	for (int i = 0; i < logs.Size(); ++i) {
		if (logs.ValidIndex(i)) {
			...
			if (al->TYPE == LOG_TYPE_DELETED && uplinkrating >= MINREQUIREDRATING_UNDELETELOGLEVEL1) {
			...
			} else if (al->TYPE != LOG_TYPE_DELETED && LogModified(i)
			...
			if (al->date.After(&lowerdate) && al->date.Before(&upperdate)) {

				// Now look at the log

				if (al->TYPE == LOG_TYPE_BOUNCEBEGIN && al->data1 && strcmp(al->data1, to_ip) == 0) {

					// This computer is the origin of the bounced call
					// And is therefore the solution to this trace
					return logbank_ip;

				} else if (al->TYPE == LOG_TYPE_BOUNCE && al->data1 && strcmp(al->data1, to_ip) == 0) {
					...
					if (vl) {
						...
						if (comp) {
							...
							if ((!isbank || (isbank && uplinkrating >= MINREQUIREDRATING_HACKBANKSERVER))
								&& (!isgov
									|| (isgov && uplinkrating >= MINREQUIREDRATING_HACKGOVERNMENTCOMPUTER))) {
...
bool LogBank::Load(FILE* file)
{
	...
	for (int i = 0; i < size; ++i) {
		...
		if (index == -1) {
			...
			UplinkPrintAbortArgs("WARNING: LogBank::Load, number of items appears to be wrong, index=%d",
								 index);
			return false;
		...
		bool modified;
		if (!FileReadData(&modified, sizeof(modified), 1, file)) {
			return false;
		}
...
std::string LogBank::GetID() { return "LOGBANK"; }

int LogBank::GetOBJECTID() { return OID_LOGBANK; }
...
void AccessLog::SetProperties(
	Date* newdate, const char* newfromip, const char* newfromname, int newSUSPICIOUS, int newTYPE)
{
...
void AccessLog::SetTYPE(int newTYPE) { TYPE = newTYPE; }

void AccessLog::SetFromIP(const char* newfromip)
{
...
void AccessLog::SetSuspicious(int newSUSPICIOUS) { SUSPICIOUS = newSUSPICIOUS; }

void AccessLog::SetData1(const char* newdata)
{
...
bool AccessLog::Load(FILE* file)
{
	...
	if (!LoadDynamicStringStatic(fromip, SIZE_VLOCATION_IP, file)) {
		return false;
	}
	if (!LoadDynamicStringStatic(fromname, SIZE_PERSON_NAME, file)) {
		return false;
	}
	...
	if (!FileReadData(&SUSPICIOUS, sizeof(SUSPICIOUS), 1, file)) {
		return false;
	}

	if (!LoadDynamicStringPtr(&data1, file)) {
		return false;
	}
	if (!LoadDynamicStringPtr(&data2, file)) {
		return false;
	}
	if (!LoadDynamicStringPtr(&data3, file)) {
		return false;
	}
...
void AccessLog::Print()
{
	...
	if (SUSPICIOUS == LOG_SUSPICIOUS) {
		printf("This was a suspicious action\n");
	} else if (SUSPICIOUS == LOG_SUSPICIOUSANDNOTICED) {
		printf("This was suspicious and was noticed\n");
	} else if (SUSPICIOUS == LOG_UNDERINVESTIGATION) {
		printf("This is under investigation\n");
	} else {
...
std::string AccessLog::GetID() { return "ACC_LOG"; }

int AccessLog::GetOBJECTID() { return OID_ACCESSLOG; }
