...
int LanComputer::AddLanSystem(LanComputerSystem* system) { return systems.PutData(system); }
...
int LanComputer::AddLanLink(int from, float fromX, float fromY, int to, float toX, float toY, int security)
{
...
bool LanComputer::Load(FILE* file)
{
	...
	if (!LoadDArray((DArray<UplinkObject*>*)&links, file)) {
		return false;
	}
...
void LanComputer::Update() { Computer::Update(); }

std::string LanComputer::GetID() { return "LANCOMP"; }

int LanComputer::GetOBJECTID() { return OID_LANCOMPUTER; }
...
bool LanComputerSystem::Load(FILE* file)
{
	...
	if (!FileReadData(&subnet, sizeof(subnet), 1, file)) {
		return false;
	}
	if (!FileReadData(&security, sizeof(security), 1, file)) {
		return false;
	}
	if (!FileReadData(&screenIndex, sizeof(screenIndex), 1, file)) {
		return false;
	}
	if (!FileReadData(&dataScreenIndex, sizeof(dataScreenIndex), 1, file)) {
		return false;
	}
	...
	if (!FileReadData(&data3, sizeof(data3), 1, file)) {
		return false;
	}

	if (!LoadDArray(&validSubnets, file)) {
		return false;
	}
...
std::string LanComputerSystem::GetID() { return "LANSYST"; }

int LanComputerSystem::GetOBJECTID() { return OID_LANCOMPUTERSYSTEM; }
...
bool LanComputerLink::Load(FILE* file)
{
	...
	if (!FileReadData(&from, sizeof(from), 1, file)) {
		return false;
	}
	if (!FileReadData(&to, sizeof(to), 1, file)) {
		return false;
	}
	if (!FileReadData(&port, sizeof(port), 1, file)) {
		return false;
	}
	if (!FileReadData(&security, sizeof(security), 1, file)) {
		return false;
	...
	if (!FileReadData(&fromX, sizeof(fromX), 1, file)) {
		return false;
	}
	if (!FileReadData(&fromY, sizeof(fromY), 1, file)) {
		return false;
	}
	if (!FileReadData(&toX, sizeof(toX), 1, file)) {
		return false;
	}
	if (!FileReadData(&toY, sizeof(toY), 1, file)) {
		return false;
	}
...
std::string LanComputerLink::GetID() { return "LANLINK"; }

int LanComputerLink::GetOBJECTID() { return OID_LANCOMPUTERLINK; }
