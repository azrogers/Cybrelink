...
BankComputer::~BankComputer() { DeleteBTreeData((BTree<UplinkObject*>*)&accounts); }
...
int BankComputer::CreateBankAccount(
	const char* name, const char* password, int security, int balance, int loan)
{
...
bool BankComputer::Load(FILE* file)
{
	...
	if (!FileReadData(&previousaccno, sizeof(previousaccno), 1, file)) {
		return false;
	}

	if (!LoadBTree((BTree<UplinkObject*>*)&accounts, file)) {
		return false;
	}
...
void BankComputer::Update() { Computer::Update(); }

std::string BankComputer::GetID() { return "BCOMP"; }

int BankComputer::GetOBJECTID() { return OID_BANKCOMPUTER; }
