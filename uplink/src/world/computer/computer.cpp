...
Computer::~Computer() { DeleteDArrayData((DArray<UplinkObject*>*)&screens); }

void Computer::SetTYPE(int newTYPE) { TYPE = newTYPE; }

void Computer::SetName(const char* newname)
{
...
void Computer::SetCompanyName(const char* newname)
{
...
void Computer::SetIP(const char* newip)
{
...
void Computer::SetTraceSpeed(int newtracespeed) { tracespeed = newtracespeed; }

void Computer::SetTraceAction(int newtraceaction) { traceaction = newtraceaction; }

void Computer::SetIsTargetable(bool value) { istargetable = value; }

void Computer::SetIsExternallyOpen(bool value) { isexternallyopen = value; }

void Computer::SetIsRunning(bool value) { isrunning = value; }
...
void Computer::CheckForSecurityBreaches()
{
	...
	if (!isrunning) {
		...
		if (!isinfected_revelation) {
			...
			if (databank.formatted) {
				NewsGenerator::ComputerDestroyed(this, true);
			} else {
				NewsGenerator::ComputerDestroyed(this, false);
			}
		}
	...
	for (int i = 0; i < logbank.logs.Size(); ++i) {
		if (logbank.logs.ValidIndex(i)) {
			...
			if (logbank.internallogs.ValidIndex(i)) {
				internallog = logbank.internallogs.GetData(i);
			}
			...
			if (al->SUSPICIOUS == LOG_SUSPICIOUS) {
				...
				if (!ignoreme) {
					...
					if (internallog) {
						internallog->SetSuspicious(LOG_SUSPICIOUSANDNOTICED);
					}
				...
				if (internallog) {
					internallog->SetSuspicious(LOG_UNDERINVESTIGATION);
				}
			}
		}
	}
	...
	if (databank.formatted) {
		...
		const char* softwaretype[4] = { "SCIENTIFIC", "CORPORATE", "CUSTOMER", "SOFTWARE" };
	...
	if (security.IsAnythingDisabled() || timetochangeunderware) {
...
bool Computer::ChangeSecurityCodes()
{
	...
	case COMPUTER_TYPE_UNKNOWN: {
		...
		if (strcmp(ip, IP_SOCIALSECURITYDATABASE) == 0 || strcmp(ip, IP_GLOBALCRIMINALDATABASE) == 0
...
void Computer::UpdateRecentHacks()
{
	...
	if (numhackslastmonth < 0) {
		numhackslastmonth = 0;
	}
}

int Computer::NumRecentHacks() { return numrecenthacks; }
...
bool Computer::Load(FILE* file)
{
	...
	if (!LoadDynamicStringStatic(name, SIZE_COMPUTER_NAME, file)) {
		return false;
	}
	if (!LoadDynamicStringStatic(companyname, SIZE_COMPANY_NAME, file)) {
		return false;
	}
	if (!LoadDynamicStringStatic(ip, SIZE_VLOCATION_IP, file)) {
		return false;
	}

	if (!FileReadData(&tracespeed, sizeof(tracespeed), 1, file)) {
		return false;
	}
	if (!FileReadData(&traceaction, sizeof(traceaction), 1, file)) {
		return false;
	}

	if (!FileReadData(&numrecenthacks, sizeof(numrecenthacks), 1, file)) {
		return false;
	}
	if (!FileReadData(&numhacksthismonth, sizeof(numhacksthismonth), 1, file)) {
		return false;
	}
	if (!FileReadData(&numhackslastmonth, sizeof(numhackslastmonth), 1, file)) {
		return false;
	}

	if (!FileReadData(&istargetable, sizeof(istargetable), 1, file)) {
		return false;
	}
	if (!FileReadData(&isexternallyopen, sizeof(isexternallyopen), 1, file)) {
		return false;
	}
	if (!FileReadData(&isrunning, sizeof(isrunning), 1, file)) {
		return false;
	}
	if (!FileReadData(&isinfected_revelation, sizeof(isinfected_revelation), 1, file)) {
		return false;
	}

	if (!LoadDArray((DArray<UplinkObject*>*)&screens, file)) {
		return false;
	}

	if (!databank.Load(file)) {
		return false;
	}
	if (!logbank.Load(file)) {
		return false;
	}
	if (!recordbank.Load(file)) {
		return false;
	}
	if (!security.Load(file)) {
		return false;
	}
	if (!infectiondate.Load(file)) {
		return false;
	}
...
void Computer::Print()
{
	...
	printf("TYPE = %d, Companyname = %s, ip = %s, tracespeed = %d, traceaction = %d, targetable = %d, "
		   "externally open = %d, running = %d\n",
		   TYPE,
		   companyname,
		   ip,
		   tracespeed,
...
void Computer::Update()
{
	...
	if (databank.NumDataFiles() > 0 && NumberGenerator::RandomNumber(1000) == 0) {
...
void Computer::GenerateAccessCode(const char* code, char* result, size_t resultsize)
{
	UplinkSnprintf(result, resultsize, "%s:'%s'", "CODE", code);
}

void Computer::GenerateAccessCode(const char* name, const char* code, char* result, size_t resultsize)
{
	UplinkSnprintf(result, resultsize, "%s:'%s', %s:'%s'", "NAME", name, "CODE", code);
}

void Computer::GenerateAccessCode(int accNo, const char* code, char* result, size_t resultsize)
{
	UplinkSnprintf(result, resultsize, "%s:'%d', %s:'%s'", "NAME", accNo, "CODE", code);
}

std::string Computer::GetID() { return "COMPUT"; }

int Computer::GetOBJECTID() { return OID_COMPUTER; }
