...
void LanMonitor::EndAttack() { ResetAll(); }
...
void LanMonitor::SetCurrentSelected(int newCurrentSelected) { currentSelected = newCurrentSelected; }

void LanMonitor::SetCurrentSpoof(int newCurrentSpoof) { currentSpoof = newCurrentSpoof; }
...
bool LanMonitor::IsInConnection(int node) { return (GetNodeIndex(node) != -1); }
...
bool LanMonitor::IsAccessable(int newNode)
{
	...
	if (newSystem->visible == LANSYSTEMVISIBLE_NONE || newSystem->visible == LANSYSTEMVISIBLE_AWARE) {
		return false;
	}
	...
	if (newNode != currentSystem) {
		...
		for (int i = 0; i < lanComputer->links.Size(); ++i) {
			if (lanComputer->links.ValidIndex(i)) {
				...
				if ((link->to == newNode || link->from == newNode) && link->visible == LANLINKVISIBLE_AWARE) {
					knownLinks = true;
					break;
				}
			}
	...
	if (newSystem->TYPE == LANSYSTEM_LOCK && newSystem->data1 == 1) {
		return false;
	}
	...
	if (newSystem->TYPE == LANSYSTEM_MAINSERVER) {
		...
		if (newSystem->data1 != -1 && lanComputer->systems.ValidIndex(newSystem->data1)
			&& lanComputer->systems.GetData(newSystem->data1)->data1 == 1) {
			return false;
		}
		if (newSystem->data2 != -1 && lanComputer->systems.ValidIndex(newSystem->data2)
			&& lanComputer->systems.GetData(newSystem->data2)->data1 == 1) {
			return false;
		}
		if (newSystem->data3 != -1 && lanComputer->systems.ValidIndex(newSystem->data3)
			&& lanComputer->systems.GetData(newSystem->data3)->data1 == 1) {
			return false;
		}
	}
