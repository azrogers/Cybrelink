...
void Gateway::IncrementProximity(int n) { proximity += n; }
...
void Gateway::ExchangeGatewayComplete()
{
	...
	while (newgateway->maxcpus < GetNumCPUs()) {
		...
		if (indexSlowest != -1) {
			...
			removedItems.PutData(const_cast<char*>(upgradeSlowest->name));
	...
	if (newgateway->maxmemory * 8 < databank.GetSize()) {
		...
		UplinkSnprintf(
			removedMemory, removedMemorysize, "%dGq of memory", memorysize - (newgateway->maxmemory * 8));
	...
	while (newgateway->maxsecurity < numSecurity) {
		...
		for (int i = 0; i < hardware.Size(); ++i) {
			...
			if (cu->TYPE == GATEWAYUPGRADETYPE_SECURITY) {
				removedItems.PutData(const_cast<char*>(cu->name));
...
void Gateway::SetModemType(const char* newmodem, int newmodemspeed)
{
...
void Gateway::ChangeMemorySize(int amount) { SetMemorySize(memorysize + amount); }

void Gateway::GiveHUDUpgrade(char upgrade) { hudupgrades = hudupgrades | upgrade; }

bool Gateway::HasHUDUpgrade(char upgrade) { return (hudupgrades & upgrade); }

char Gateway::GetHudUpgrades() { return hudupgrades; }

bool Gateway::HasAnyHUDUpgrade() { return (hudupgrades != 0); }

void Gateway::GiveCPU(const char* CPUName)
{
...
LList<char*>* Gateway::GetCPUs()
{
	...
	for (int i = 0; i < hardware.Size(); ++i) {
		...
		if (cu->TYPE == GATEWAYUPGRADETYPE_CPU) {
			list->PutData(const_cast<char*>(cu->name));
		}
	}
...
bool Gateway::IsHWInstalled(const char* name)
{
...
bool Gateway::Load(FILE* file)
{
	...
	if (strcmp(game->GetLoadedSavefileVer(), "SAV61") >= 0) {
		...
		if (!curgatewaydef->Load(file)) {
			return false;
		}

		int exists;
		if (!FileReadData(&exists, sizeof(exists), 1, file)) {
			return false;
		}
		if (exists != 0) {
			newgatewaydef = new GatewayDef();
			if (!newgatewaydef->Load(file)) {
				return false;
			}
		}
	} else {
		if (!FileReadData(&old_type, sizeof(old_type), 1, file)) {
			return false;
		}
		if (!FileReadData(&old_newtype, sizeof(old_newtype), 1, file)) {
			return false;
		}
	}
	...
	if (!FileReadData(&nuked, sizeof(nuked), 1, file)) {
		return false;
	}
	if (!FileReadData(&proximity, sizeof(proximity), 1, file)) {
		return false;
	}

	if (!FileReadData(&modemspeed, sizeof(modemspeed), 1, file)) {
		return false;
	}
	if (!FileReadData(&memorysize, sizeof(memorysize), 1, file)) {
		return false;
	}
	if (!FileReadData(&modemtype, sizeof(modemtype), 1, file)) {
		return false;
	}

	if (!FileReadData(&hudupgrades, sizeof(hudupgrades), 1, file)) {
		return false;
	}
	...
	if (strcmp(game->GetLoadedSavefileVer(), "SAV61") < 0) {
		...
		if (old_type == -1) {
			if (strcmp(game->GetLoadedSavefileVer(), "SAV59") >= 0) {
				if (!LoadDynamicStringStatic(old_name, sizeof(old_name), file)) {
					return false;
				}
			} else {
...
void Gateway::Print()
{
	...
	printf("Gateway : name='%s', id=%d\n"
		   "nuked=%d, proximity=%d\n",
		   (curgatewaydef) ? curgatewaydef->name : "NULL",
		   id,
		   nuked,
		   proximity);
	printf("Upgrades : %c\n", hudupgrades);
...
void Gateway::Update() { databank.Update(); }

std::string Gateway::GetID() { return "GATEWAY"; }
