...
BankAccount* BankAccount::GetAccount(const char* bankip, const char* accno)
{
	...
	if (!(comp->GetOBJECTID() == OID_BANKCOMPUTER)) {
		return NULL;
	}
...
bool BankAccount::TransferMoney(const char* source_ip,
								...
								Person* person)
{
	...
	if (source_acc && target_acc && amount > 0 && source_acc->balance >= amount) {
		...
		source_log->SetProperties(
			&(game->GetWorld()->date), source_ip, person->name, LOG_NOTSUSPICIOUS, LOG_TYPE_TRANSFERTO);
		...
		target_log->SetProperties(
			&(game->GetWorld()->date), source_ip, person->name, LOG_NOTSUSPICIOUS, LOG_TYPE_TRANSFERFROM);
...
void BankAccount::SetOwner(const char* newname)
{
...
void BankAccount::SetSecurity(const char* newpassword, int newsecurity)
{
...
void BankAccount::SetAccNumber(int newaccnumber) { accountnumber = newaccnumber; }

void BankAccount::ChangeBalance(int amount, const char* description = NULL)
{
	...
	if (description) {
		source_log->SetData1(description);
	} else {
		source_log->SetData1("Unknown");
	}

	log.AddLog(source_log);
}

bool BankAccount::HasTransferOccured(
	const char* s_ip, const char* t_ip, int t_accno, int amount, bool partial)
{
	...
	for (i = 0; i < log.internallogs.Size(); ++i) {
		if (log.internallogs.ValidIndex(i)) {
			...
			if (al->TYPE == LOG_TYPE_TRANSFERTO) {
				...
				if (strstr(al->data1, t_ip) != 0 && strstr(al->data1, t_accno_s) != 0
	...
	for (i = 0; i < target_acc->log.internallogs.Size(); ++i) {
		if (target_acc->log.internallogs.ValidIndex(i)) {
			...
			if (al->TYPE == LOG_TYPE_TRANSFERFROM) {
				...
				if (strstr(al->data1, s_ip) != 0 && strstr(al->data1, s_accno_s) != 0
					&& strcmp(al->data2, amount_s) == 0) {
...
bool BankAccount::HasTransferOccured(const char* person, int amount)
{
	...
	for (int i = 0; i < log.internallogs.Size(); ++i) {
		if (log.internallogs.ValidIndex(i)) {
			...
			if (al->TYPE == LOG_TYPE_TRANSFERFROM) {
				...
				if (strcmp(al->data2, amount_s) == 0 && strcmp(al->data3, person) == 0) {
...
bool BankAccount::Load(FILE* file)
{
	...
	if (!FileReadData(&accountnumber, sizeof(accountnumber), 1, file)) {
		return false;
	}
	if (!FileReadData(&balance, sizeof(balance), 1, file)) {
		return false;
	}
	if (!FileReadData(&loan, sizeof(loan), 1, file)) {
		return false;
	}
...
std::string BankAccount::GetID() { return "ACCOUNT"; }

int BankAccount::GetOBJECTID() { return OID_BANKACCOUNT; }
