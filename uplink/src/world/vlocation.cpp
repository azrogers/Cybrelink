...
void VLocation::SetIP(const char* newip)
{
...
bool VLocation::VerifyIP(const char* newip)
{
...
void VLocation::SetComputer(const char* newcomputer)
{
...
bool VLocation::VerifyPLocation(int newx, int newy)
{
	...
	if (!(newx >= 0 && newx < VIRTUAL_WIDTH) || !(newy >= 0 && newy < VIRTUAL_HEIGHT)) {
...
void VLocation::SetListed(bool newlisted) { listed = newlisted; }

void VLocation::SetDisplayed(bool newdisplayed) { displayed = newdisplayed; }

void VLocation::SetColorDisplayed(bool newcolored) { colored = newcolored; }
	...
	while (tree) {
		...
		if (tree) {
			...
			if (comp && strcmp(comp->ip, ip) == 0) {
				return comp;
			} else if (comp && comp->TYPE == COMPUTER_TYPE_LAN && GetOBJECTID() == OID_VLOCATIONSPECIAL) {
				return comp;
			} else {
...
bool VLocation::Load(FILE* file)
{
	...
	if (!LoadDynamicStringStatic(computer, SIZE_COMPUTER_NAME, file)) {
		return false;
	}
	...
	if (!FileReadData(&listed, sizeof(listed), 1, file)) {
		return false;
	}
	if (!FileReadData(&displayed, sizeof(displayed), 1, file)) {
		return false;
	}

	if (strcmp(game->GetLoadedSavefileVer(), "SAV60") >= 0) {
		if (!FileReadData(&colored, sizeof(colored), 1, file)) {
			return false;
		}
	}
...
std::string VLocation::GetID() { return "VIRTLOC"; }

int VLocation::GetOBJECTID() { return OID_VLOCATION; }
...
void VLocationSpecial::SetScreenIndex(int newScreenIndex) { screenIndex = newScreenIndex; }

void VLocationSpecial::SetSecuritySystemIndex(int newIndex) { securitySystemIndex = newIndex; }
...
bool VLocationSpecial::Load(FILE* file)
{
	...
	if (!FileReadData(&securitySystemIndex, sizeof(securitySystemIndex), 1, file)) {
		return false;
	}
...
std::string VLocationSpecial::GetID() { return "VLOCSPEC"; }

int VLocationSpecial::GetOBJECTID() { return OID_VLOCATIONSPECIAL; }
