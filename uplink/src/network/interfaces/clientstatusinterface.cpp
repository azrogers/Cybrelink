...
ClientStatusInterface::ClientStatusInterface()
{
	...
	news.PutDataAtStart(const_cast<char*>(SPACE));
}

ClientStatusInterface::~ClientStatusInterface() { DeleteLListData(&news); }
...
void ClientStatusInterface::GatewayPanelDraw(Button* button, bool highlighted, bool clicked)
{
	...
	ClientStatusInterface* thisint =
		(ClientStatusInterface*)app->GetNetwork()->GetClient()->GetNetworkScreen();
...
void ClientStatusInterface::Create()
{
	...
	if (!IsVisible()) {
		...
		EclRegisterButton(
			13, 38, 193, 15, "Gateway Status", "Shows the status of your Gateway", "status_gatewaytitle");
		...
		EclRegisterButton(
			223, 38, 193, 15, "World Status", "Shows the status of the world", "status_worldtitle");
		...
		EclRegisterButton(
			433, 38, 193, 15, "Personal Status", "Shows your personal status", "status_personaltitle");
	}
}
...
bool ClientStatusInterface::IsVisible() { return (EclGetButton("status_mainmenu") != NULL); }

int ClientStatusInterface::ScreenID() { return CLIENT_STATUS; }
