...
void ClientConnection::SetConnectionTime(int newtime) { connectiontime = newtime; }
...
void ClientConnection::Handle_ClientCommsInterface()
{
	...
	if (!sentgateway) {
		...
		UplinkAssert(vl);
		msgstream << "CLIENTCOMMS-IPNAME " << vl->ip << " " << vl->x << " " << vl->y << " " << vl->computer;
		msgstream << (char)10 << (char)13 << '#' << '\x0';
	...
	if (links->Size() > numlinks) {
		...
		for (int i = 0; i < numnewlinks; ++i) {
			...
			UplinkAssert(vl);
			msgstream << "CLIENTCOMMS-IPNAME " << vl->ip << " " << vl->x << " " << vl->y << " "
					  << vl->computer;
			msgstream << (char)10 << (char)13 << '#' << '\x0';
...
void ClientConnection::Handle_ClientStatusInterface()
{
	...
	if (crimrec && strcmp(convictions, crimrec) != 0) {
		...
		std::ostrstream msgstream;
		msgstream << "CLIENTSTATUS-CRIMINAL " << crimrec;
	...
	if (player->gateway.GetHudUpgrades() != hudupgrades) {
		...
		if (!(player->gateway.HasAnyHUDUpgrade())) {
			msgstream << "None\n";
		}
		if (player->gateway.HasHUDUpgrade(HUDUPGRADE_MAPSHOWSTRACE)) {
			msgstream << "Show trace on map\n";
		}
		if (player->gateway.HasHUDUpgrade(HUDUPGRADE_CONNECTIONANALYSIS)) {
			msgstream << "Connection analysis\n";
		}
		if (player->gateway.HasHUDUpgrade(HUDUPGRADE_IRCCLIENT)) {
			msgstream << "IRC Client\n";
		}
		if (player->gateway.HasHUDUpgrade(HUDUPGRADE_LANVIEW)) {
			msgstream << "LAN View\n";
		}
	...
	if (player->gateway.GetCPUSpeed() * player->gateway.GetBandwidth() * player->gateway.memorysize
			!= cpumodemmemory
		...
		cpumodemmemory =
			player->gateway.GetCPUSpeed() * player->gateway.GetBandwidth() * player->gateway.memorysize;
	...
	if (latest->date.After(&lastnewsdate)) {
		...
		for (int in = indexofnew; in >= 0; --in) {
			...
			std::ostrstream msgstream;
			msgstream << "CLIENTSTATUS-NEWS - " << thisnews->date.GetShortString() << ": "
					  << thisnews->headline;
			msgstream << (char)10 << (char)13 << '#' << '\x0';
			char* message = msgstream.str();
		...
		if (game->GameSpeed() > GAMESPEED_PAUSED) {
			...
			if (EclGetAccurateTime() - previousupdate > 1000) {
				...
				if (clienttype == CLIENT_COMMS) {
					Handle_ClientCommsInterface();
				} else if (clienttype == CLIENT_STATUS) {
					Handle_ClientStatusInterface();
				} else {
...
void ClientConnection::SetIndex(int newindex) { index = newindex; }

std::string ClientConnection::GetID() { return "CL_CON"; }
