...
template <typename T> bool SaveSerializeable(T& obj, FILE* file) { return obj.Write(file); }

template <typename T> bool LoadSerializeable(T& obj, FILE* file) { return obj.Read(file); }
...
void SaveDynamicString(const char* string, FILE* file); // Works with NULL
void SaveDynamicString(const char* string, int maxsize, FILE* file); // Works with NULL
bool LoadDynamicStringInt(const char* _file,
						  int _line,
						  char** string,
						  FILE* file); // Assigns space for string
bool LoadDynamicStringInt(
	const char* _file, int _line, char* string, int maxsize, FILE* file); // Does not allocate space

#define LoadDynamicStringPtr(string, file) LoadDynamicStringInt(__FILE__, __LINE__, string, file)
#define LoadDynamicStringStatic(string, maxsize, file)                                                       \
	LoadDynamicStringInt(__FILE__, __LINE__, string, maxsize, file)
...
#define FileReadData(_DstBuf, _ElementSize, _Count, _File)                                                   \
	FileReadDataInt(__FILE__, __LINE__, _DstBuf, _ElementSize, _Count, _File)
