...
bool LoadBTree(BTree<UplinkObject*>* btree, FILE* file)
{
	...
	for (int i = 0; i < size; ++i) {
		...
		char* id = NULL;
		if (!LoadDynamicStringPtr(&id, file)) {
			return false;
		}
		if (!id) {
		...
		int OBJECTID;
		if (!FileReadData(&OBJECTID, sizeof(int), 1, file)) {
			return false;
		}
...
bool LoadLList(LList<char*>* llist, FILE* file)
{
	...
	for (int i = 0; i < size; ++i) {
		...
		if (!LoadDynamicStringPtr(&stringdata, file)) {
			return false;
		}
...
void SaveDArray(DArray<UplinkObject*>* darray, FILE* file)
{
	...
	if (size > MAX_ITEMS_DATA_STRUCTURE) {
		UplinkPrintAbortArgs(
			"WARNING: SaveDArray, number of items appears to be too big, size=%d, maxsize=%d",
			size,
			MAX_ITEMS_DATA_STRUCTURE);
		size = MAX_ITEMS_DATA_STRUCTURE;
	}
...
bool LoadDArray(DArray<UplinkObject*>* darray, FILE* file)
{
	...
	for (int i = 0; i < size; ++i) {
		...
		int index;
		if (!FileReadData(&index, sizeof(index), 1, file)) {
			return false;
		}
		...
		if (darray->ValidIndex(i)) {
			if (darray->GetData(i)) {
				try {
					...
					DEBUG_PRINTF(
						"Oops! Tried to delete something that wasn't there [%s (%d)].\n", file, line);
				}
			}
		}
	}
...
UplinkObject* CreateUplinkObject(int OBJECTID)
{
	...
	case OID_OPTION:
		uo = new Option();
		break;
	case OID_VLOCATIONSPECIAL:
		uo = new VLocationSpecial();
		break;
	...
	case OID_RECORD:
		uo = new Record();
		break;
	case OID_BANKCOMPUTER:
		uo = new BankComputer();
		break;
	case OID_BANKACCOUNT:
		uo = new BankAccount();
		break;
	case OID_SECURITYSYSTEM:
		uo = new SecuritySystem();
		break;
	case OID_LANCOMPUTER:
		uo = new LanComputer();
		break;
	case OID_LANCOMPUTERSYSTEM:
		uo = new LanComputerSystem();
		break;
	case OID_LANCOMPUTERLINK:
		uo = new LanComputerLink();
		break;

	case OID_GENERICSCREEN:
		uo = new GenericScreen();
		break;
	case OID_MESSAGESCREEN:
		uo = new MessageScreen();
		break;
	case OID_PASSWORDSCREEN:
		uo = new PasswordScreen();
		break;
	case OID_MENUSCREEN:
		uo = new MenuScreen();
		break;
	case OID_MENUSCREENOPTION:
		uo = new MenuScreenOption();
		break;
	case OID_DIALOGSCREEN:
		uo = new DialogScreen();
		break;
	case OID_DIALOGSCREENWIDGET:
		uo = new DialogScreenWidget();
		break;
	...
	case OID_SHARESLISTSCREEN:
		uo = new SharesListScreen();
		break;
	case OID_CYPHERSCREEN:
		uo = new CypherScreen();
		break;
	case OID_HIGHSECURITYSCREEN:
		uo = new HighSecurityScreen();
		break;
	case OID_DISCONNECTEDSCREEN:
		uo = new DisconnectedScreen();
		break;

	case OID_COMPANY:
		uo = new Company();
		break;
	case OID_COMPANYUPLINK:
		uo = new CompanyUplink();
		break;
	case OID_MISSION:
		uo = new Mission();
		break;
	...
	case OID_NOTIFICATIONEVENT:
		uo = new NotificationEvent();
		break;
	case OID_ARRESTEVENT:
		uo = new ArrestEvent();
		break;
	case OID_SHOTBYFEDSEVENT:
		uo = new ShotByFedsEvent();
		break;
	case OID_SEIZEGATEWAYEVENT:
		uo = new SeizeGatewayEvent();
		break;
	case OID_INSTALLHWEVENT:
		uo = new InstallHardwareEvent();
		break;
	case OID_WARNINGEVENT:
		uo = new WarningEvent();
		break;
	case OID_ATTEMPTMISSIONEVENT:
		uo = new AttemptMissionEvent();
		break;
	case OID_RUNPLOTSCENEEVENT:
		uo = new RunPlotSceneEvent();
		break;
	case OID_CHANGEGATEWAYEVENT:
		uo = new ChangeGatewayEvent();
		break;
	case OID_BANKROBBERYEVENT:
		uo = new BankRobberyEvent();
		break;

	case OID_UNKNOWN:
		UplinkPrintAbort("OBJECTID Not assigned");
		break;
	default:
		UplinkPrintAbortArgs("Unrecognised OBJECTID=%d", OBJECTID);
	}
...
bool LoadDynamicStringInt(const char* _file, int _line, char** string, FILE* file)
{
	...
	*string = NULL;
	...
	if (size == -1) {
		...
		UplinkPrintAbortArgs(
			"WARNING: LoadDynamicString, size appears to be wrong, size=%d, %s:%d", size, _file, _line);
		return false;
...
bool LoadDynamicStringInt(const char* _file, int _line, char* string, int maxsize, FILE* file)
{
	...
	if (size == -1) {
		...
		UplinkPrintAbortArgs("WARNING: LoadDynamicString, size > maxsize, size=%d, maxsize=%d, %s:%d",
							 size,
							 maxsize,
							 _file,
							 _line);
		...
		if (maxsize > 0) {
			...
			UplinkPrintAbortArgs(
				"WARNING: LoadDynamicString, possible string=%s, %s:%d", string, _file, _line);
		}
...
void SaveDynamicString(const char* string, FILE* file) { SaveDynamicString(string, -1, file); }

void SaveDynamicString(const char* string, int maxsize, FILE* file)
{
	...
	if (string) {
		...
		if (size <= realmaxsize) {
			...
			UplinkPrintAbortArgs("WARNING: SaveDynamicString, size appears to be too long, size=%d, "
								 "maxsize=%d, absolute maxsize=%d",
								 size,
								 maxsize,
								 MAX_LENGTH_DYMANIC_STRING);
...
bool FileReadDataInt(
	const char* _file, int _line, void* _DstBuf, size_t _ElementSize, size_t _Count, FILE* _File)
{
	size_t sizeRead = fread(_DstBuf, _ElementSize, _Count, _File);
	if (sizeRead != _Count) {
		UplinkPrintAbortArgs("WARNING: FileReadDataInt, request read count is different then the readed "
							 "count, request=%d, readed=%d, errno=%d, %s:%d",
							 _Count,
							 sizeRead,
							 errno,
							 _file,
