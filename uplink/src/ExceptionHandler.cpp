...
static bool DumpModuleInfo(HANDLE LogFile, HINSTANCE ModuleHandle, int nModuleNo)
{
	...
	__try {
		if (GetModuleFileName(ModuleHandle, szModName, sizeof(szModName) - 2) > 0) {
			...
			HANDLE ModuleFile = CreateFile(
				szModName, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
			...
			hprintf(LogFile,
					_T("Image Base: 0x%08x  Image Size: 0x%08x\r\n"),
					NTHeader->OptionalHeader.ImageBase,
					NTHeader->OptionalHeader.SizeOfImage),

				hprintf(LogFile,
						_T("Checksum:   0x%08x  Time Stamp: 0x%08x\r\n"),
...
static void DumpSystemInformation(HANDLE LogFile)
{
	...
	hprintf(
		LogFile, _T("%d MBytes user address space free.\r\n"), (MemInfo.dwAvailVirtual + ONEM - 1) / ONEM);
}
...
static void DumpRegisters(HANDLE LogFile, PCONTEXT Context)
{
	...
	hprintf(LogFile,
			_T("EDI:    0x%08x  ESI: 0x%08x  EAX:   0x%08x\r\n"),
			Context->Edi,
			Context->Esi,
			Context->Eax);
	hprintf(LogFile,
			_T("EBX:    0x%08x  ECX: 0x%08x  EDX:   0x%08x\r\n"),
			Context->Ebx,
			Context->Ecx,
			Context->Edx);
	hprintf(LogFile,
			_T("EIP:    0x%08x  EBP: 0x%08x  SegCs: 0x%08x\r\n"),
			Context->Eip,
			Context->Ebp,
			Context->SegCs);
	hprintf(LogFile,
			_T("EFlags: 0x%08x  ESP: 0x%08x  SegSs: 0x%08x\r\n"),
			Context->EFlags,
			Context->Esp,
			Context->SegSs);
}
...
int __cdecl RecordExceptionInfo(PEXCEPTION_POINTERS pExceptPtrs, LPCTSTR lpszMessage)
{
	...
	HANDLE hLogFile = CreateFile(
		szModuleName, GENERIC_WRITE, 0, 0, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_WRITE_THROUGH, 0);
	...
	if (VirtualQuery((void*)Context->Eip, &MemInfo, sizeof(MemInfo))
		&& (GetModuleFileName(
				(HINSTANCE)MemInfo.AllocationBase, szCrashModulePathName, sizeof(szCrashModulePathName) - 2)
			> 0)) {
	...
	// Create the file
	HANDLE hMiniDumpFile = CreateFile(szModuleName,
									  GENERIC_WRITE,
									  0,
									  NULL,
									  CREATE_ALWAYS,
