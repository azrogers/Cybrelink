...
void GatewayNuke::YesClick(Button* button)
{
	...
	int pid;
	sscanf(button->name.c_str(), "gatenuke_yes %d", &pid);
...
void GatewayNuke::NoClick(Button* button)
{
	...
	int pid;
	sscanf(button->name.c_str(), "gatenuke_no %d", &pid);
...
void GatewayNuke::Initialise() { status = GATEWAYNUKE_STATUS_START; }
...
void GatewayNuke::CreateInterface()
{
	...
	if (!IsInterfaceVisible()) {
		...
		EclRegisterButton(
			40, 20, 107, 15, "Nuke Gateway?", "Click yes to destroy your gateway, no to abort", title);
