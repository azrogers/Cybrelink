...
void VoiceAnalyser::PlayClick(Button* button)
{
	...
	if (thistask->STATUS == VOICEANALYSER_STATUS_READY) {
		...
		if (game->GetInterface()->GetRemoteInterface()->GetInterfaceScreen()->ScreenID()
			== SCREEN_VOICEANALYSIS) {

			VoiceAnalysisScreenInterface* vas = (VoiceAnalysisScreenInterface*)game->GetInterface()
													->GetRemoteInterface()
													->GetInterfaceScreen();
			UplinkAssert(vas);
...
void VoiceAnalyser::SetTarget(UplinkObject* uo, char* uos, int uoi)
{
	...
	if (version < 2.0) {
		return; // This function not available for version 1.0
	}
	...
	if (STATUS == VOICEANALYSER_STATUS_WAITING) {
			...
			if (((DataBank*)uo)->GetData(uoi)) {
				...
				if (loc) {
					...
					if (comp && comp->TYPE == COMPUTER_TYPE_VOICEPHONESYSTEM) {
						...
						numticksrequired =
							(int)(TICKSREQUIRED_COPY
								  * ((float)data->size
									 / (float)game->GetWorld()->GetPlayer()->gateway.GetBandwidth()));
						progress = 0;
		...
		if (uo->GetOBJECTID() == OID_DATABANK) {
			...
			if (memoryindex == -1) {
				memoryindex = db->FindValidPlacement(datacopy);
			}
	...
	if (IsInterfaceVisible()) {
		...
		case VOICEANALYSER_STATUS_WAITING: {
			...
			if (game->GetInterface()->GetRemoteInterface()->GetInterfaceScreen()->ScreenID()
				...
				VoicePhoneScreenInterface* vps = (VoicePhoneScreenInterface*)(game->GetInterface()
																				  ->GetRemoteInterface()
																				  ->GetInterfaceScreen());
				UplinkAssert(vps);
				...
				if (vps->STATUS == VOICESTATUS_TALKING) {
					...
					UplinkStrncpy(
						personname, vps->GetComputerScreen()->GetComputer()->companyname, sizeof(personname));
		...
		case VOICEANALYSER_STATUS_INPROGRESS: {
			...
			for (int count = 0; count < n; ++count) {
				...
				++progress;
				char caption[64];
				UplinkSnprintf(caption,
							   sizeof(caption),
							   "Downloading %d%% ...",
							   (int)(((float)progress / (float)numticksrequired) * 100));
