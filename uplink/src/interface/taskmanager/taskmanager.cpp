...
void TaskManager::RunHighestVersionOf(const char* name)
{
...
void TaskManager::RunSoftware(const char* name, float version)
{
	...
	if (strcmp(name, "Password_Breaker") == 0) {
		task = new PasswordBreaker();
	} else if (strcmp(name, "Dictionary_Hacker") == 0) {
		task = new DictionaryHacker();
	} else if (strcmp(name, "Trace_Tracker") == 0) {
		task = new TraceTracker();
	} else if (strcmp(name, "File_Copier") == 0) {
		task = new FileCopier();
		task->SetFollowMouse(true);
	} else if (strcmp(name, "File_Deleter") == 0) {
		task = new FileDeleter();
		task->SetFollowMouse(true);
	} else if (strcmp(name, "Defrag") == 0) {
		task = new Defrag();
	} else if (strcmp(name, "Log_Deleter") == 0) {
		task = new LogDeleter();
		task->SetFollowMouse(true);
	} else if (strcmp(name, "Log_UnDeleter") == 0) {
		task = new LogUnDeleter();
		task->SetFollowMouse(true);
	} else if (strcmp(name, "Log_Modifier") == 0) {
		task = new LogModifier();
		task->SetFollowMouse(true);
	} else if (strcmp(name, "Decrypter") == 0) {
		task = new Decrypter();
		task->SetFollowMouse(true);
	} else if (strcmp(name, "IP_Lookup") == 0) {
		task = new IPLookup();
	} else if (strcmp(name, "Gateway_Nuke") == 0) {
		task = new GatewayNuke();
	} else if (strcmp(name, "Motion_Sensor") == 0) {
		task = new MotionSensor();
	} else if (strcmp(name, "Proxy_Disable") == 0) {
		task = new ProxyDisable();
	} else if (strcmp(name, "Firewall_Disable") == 0) {
		task = new FirewallDisable();
	} else if (strcmp(name, "IP_Probe") == 0) {
		task = new IPProbe();
	} else if (strcmp(name, "Proxy_Bypass") == 0) {
		task = new SecurityBypass(SECURITY_TYPE_PROXY);
		task->SetFollowMouse(true);
	} else if (strcmp(name, "Firewall_Bypass") == 0) {
		task = new SecurityBypass(SECURITY_TYPE_FIREWALL);
		task->SetFollowMouse(true);
	} else if (strcmp(name, "Encryption_Bypass") == 0) {
		task = new SecurityBypass(SECURITY_TYPE_ENCRYPTION);
		task->SetFollowMouse(true);
	} else if (strcmp(name, "Monitor_Bypass") == 0) {
		task = new SecurityBypass(SECURITY_TYPE_MONITOR);
		task->SetFollowMouse(true);
	} else if (strcmp(name, "Revelation") == 0) {
		task = new Revelation();
	} else if (strcmp(name, "Tutorial") == 0) {
		task = new Tutorial();
	} else if (strcmp(name, "Decypher") == 0) {
		task = new Decypher();
		task->SetFollowMouse(true);
	} else if (strcmp(name, "Voice_Analyser") == 0) {
		task = new VoiceAnalyser();
	} else if (strcmp(name, "Revelation_Tracker") == 0) {
		task = new RevelationTracker();
		task->SetFollowMouse(true);
	} else if (strcmp(name, "Uplink_Agent_List") == 0) {
		task = new UplinkAgentList();
	} else if (strcmp(name, "Faith") == 0) {
		task = new Faith();
	} else if (strcmp(name, "LAN_Scan") == 0) {
		task = new LanScan();
	} else if (strcmp(name, "LAN_Probe") == 0) {
		task = new LanProbe();
		task->SetFollowMouse(true);
	} else if (strcmp(name, "LAN_Spoof") == 0) {
		task = new LanSpoof();
		task->SetFollowMouse(true);
	} else if (strcmp(name, "LAN_Force") == 0) {
		task = new LanForce();
		task->SetFollowMouse(true);
	}
...
void TaskManager::SetProgramTarget(UplinkObject* newprogramtarget, std::string targetstring, int targetint)
{
...
void TaskManager::Print() { printf("Task Manager \n"); }
...
std::string TaskManager::GetID() { return "TASKMAN"; }
