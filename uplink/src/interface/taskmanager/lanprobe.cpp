...
void LanProbe::SetTarget(UplinkObject* uo, char* uos, int uoi)
{
	...
	if (status == LANPROBE_UNUSED || status == LANPROBE_FINISHED) {
		...
		if (uo->GetOBJECTID() == OID_LANCOMPUTER) {
			...
			comp = (LanComputer*)uo;
			if (!comp->systems.ValidIndex(uoi)) {
				return;
			}
			systemIndex = uoi;
...
void LanProbe::Tick(int n)
{
	...
	if (IsInterfaceVisible()) {
		...
		if (status == LANPROBE_UNUSED) {
			...
			if (system->visible >= LANSYSTEMVISIBLE_TYPE || progress >= numticksrequired) {
			...
			if (LanMonitor::IsAccessable(systemIndex)) {
				...
				int portDone =
					(int)(LAN_LINKPORTRANGE * ((float)progress / (float)TICKSREQUIRED_SCANLANLINKS));
				...
				for (int i = 0; i < comp->links.Size(); ++i) {
					if (comp->links.ValidIndex(i)) {
						...
						if (link->visible < LANLINKVISIBLE_AWARE && link->port <= portDone) {
...
void LanProbe::CreateInterface()
{
	...
	if (!IsInterfaceVisible()) {
		...
		EclRegisterButton(
			285, 450, 120, 13, "Select target system", "Shows the progress of the LAN Probe", sprogress);
