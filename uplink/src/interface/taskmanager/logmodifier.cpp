...
static const char* logtypes[] = { "Deleted",		  "None",		 "Text",   "ConectionOpened",
								  "ConnectionClosed", "BounceBegin", "Bounce", "TransferTo",
								  "TransferFrom" };
...
void LogModifier::CreateExpandedInterface()
{
	...
	if (!IsExpandedInterfaceVisible()) {
		...
		if (source && source->logs.ValidIndex(sourceindex)) {
			...
			EclRegisterButton(
				btitle->x + 215 + 100, btitle->y + 10, 15, 15, ">", "Change the type of this log", nexttype);
			EclRegisterButtonCallback(nexttype, NextTypeClick);

			EclRegisterButton(btitle->x + 165,
							  btitle->y + 85,
							  50,
							  15,
							  "Commit",
							  "Click here to commit your changes",
							  commit);
			EclRegisterButtonCallback(commit, CommitClick);

			EclRegisterButton(
				btitle->x + 220, btitle->y + 85, 50, 15, "Close", "Close the log modifier", close);
			...
			if (log->data1) {
				EclRegisterCaptionChange(data1, log->data1);
			}
