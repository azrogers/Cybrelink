...
void MotionSensor::SensorDraw(Button* button, bool highlighted, bool clicked)
{
	...
	int numpeople;
	sscanf(button->caption.c_str(), "%d", &numpeople);
...
void MotionSensor::SensorClose(Button* button)
{
	...
	int pid;
	sscanf(button->name.c_str(), "motionsensor_main %d", &pid);
...
void MotionSensor::Tick(int n)
{
	...
	if (IsInterfaceVisible()) {
		...
		if (numpeople == 0) {
			EclGetButton(bname)->SetTooltip("There is nobody near your Gateway");
		} else if (numpeople == 1) {
			EclGetButton(bname)->SetTooltip("There is someone near your Gateway");
		} else if (numpeople == 2) {
			EclGetButton(bname)->SetTooltip("There is more than one person near your Gateway");
		} else if (numpeople >= 3) {
			EclGetButton(bname)->SetTooltip("There are a lot of people near your Gateway");
		}
	}
}
...
void MotionSensor::CreateInterface()
{
	...
	if (!IsInterfaceVisible()) {
		...
		EclRegisterButton(
			280, 20, 40, 5, "0", "Shows the status of the Motion sensor on your Gateway", bname);
