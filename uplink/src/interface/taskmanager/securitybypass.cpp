...
void SecurityBypass::PauseClick(Button* button)
{
	...
	if (pb->status == 1) {
		pb->PauseCurrentBypass();
	} else if (pb->status == 2) {
		pb->ResumeCurrentBypass();
	}
}
...
void SecurityBypass::SetTYPE(int newTYPE) { TYPE = newTYPE; }
...
void SecurityBypass::SetMoveToTime(int time) { moveToTime = time; }
...
void SecurityBypass::CreateInterface()
{
	...
	if (!IsInterfaceVisible()) {
		...
		std::string bypassCaption, pauseCaption, closeCaption;
		switch (TYPE) {
		...
		button_assignbitmaps(
			main, "software/proxybypass.tif", "software/proxybypass.tif", "software/proxybypass.tif");
		...
		button_assignbitmaps(
			status, "software/greenlight.tif", "software/redlight.tif", "software/redlight.tif");
