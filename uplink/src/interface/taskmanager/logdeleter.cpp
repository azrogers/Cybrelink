			...
			if (status == LOGDELETER_OFF) {
				...
				if (progress >= numticksrequired) {
					...
					if (version == 1.0) {
						...
						if (source->logs.ValidIndex(sourceindex)) {
							...
							// Replace it with a "Deleted" marker
							AccessLog* al = new AccessLog();
							al->SetProperties(&logdate, " ", " ", LOG_NOTSUSPICIOUS, LOG_TYPE_DELETED);
							source->logs.PutData(al, sourceindex);
						}
				...
				if (currentreplaceprogress > 20) {
					...
					if (currentreplaceindex >= source->logs.Size() - 1) {
						...
						++currentreplaceindex;
						int percentagedone =
							100 * (currentreplaceindex - sourceindex) / (source->logs.Size() - sourceindex);
						char caption[128];
...
void LogDeleter::CreateInterface()
{
	...
	if (!IsInterfaceVisible()) {
		...
		EclRegisterButton(
			285, 450, 120, 13, "Select target log", "Shows the progress of the log delete", sprogress);
