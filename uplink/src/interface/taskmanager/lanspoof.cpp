...
void LanSpoof::SetTarget(UplinkObject* uo, char* uos, int uoi)
{
	...
	if (status == LANSPOOF_UNUSED || status == LANSPOOF_SPOOFED || status == LANSPOOF_FAILED) {
...
void LanSpoof::Tick(int n)
{
	...
	if (IsInterfaceVisible()) {
		...
		if (status == LANSPOOF_UNUSED) {
			...
			// EclGetButton ( sprogress )->width = 120 * ( (float) min( progress, numticksrequired ) / (float)
			// numticksrequired );
...
void LanSpoof::CreateInterface()
{
	...
	if (!IsInterfaceVisible()) {
		...
		EclRegisterButton(
			285, 450, 120, 13, "Select target system", "Shows the progress of the LAN Spoof", sprogress);
