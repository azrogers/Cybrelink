...
void LogUnDeleter::SetTarget(UplinkObject* uo, char* uos, int uoi)
{
	...
	if (status == LOGUNDELETER_OFF) {
		...
		if (uo->GetOBJECTID() == OID_LOGBANK) {
			...
			if (((LogBank*)uo)->logs.ValidIndex(uoi) && ((LogBank*)uo)->internallogs.ValidIndex(uoi)) {
				...
				Computer* comp =
					game->GetInterface()->GetRemoteInterface()->GetComputerScreen()->GetComputer();
				UplinkAssert(comp);
		...
		for (int tick = 0; tick < n; ++tick) {
			...
			if (status == LOGUNDELETER_OFF) {
				...
				if (progress >= numticksrequired) {
					...
					source->logs.GetData(sourceindex)
						->SetProperties(source->internallogs.GetData(sourceindex));
...
void LogUnDeleter::CreateInterface()
{
	...
	if (!IsInterfaceVisible()) {
		...
		EclRegisterButton(
			285, 450, 120, 13, "Select target log", "Shows the progress of the log undelete", sprogress);
