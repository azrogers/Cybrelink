...
void PasswordBreaker::SetTarget(UplinkObject* uo, char* uos, int uoi)
{
	...
	if (uo->GetOBJECTID() == OID_PASSWORDSCREEN) {
		...
		if (strcmp(EclGetButton("useridscreen_name")->caption.c_str(), "") == 0) {
		...
		const char* name = EclGetButton("useridscreen_name")->caption.c_str();
		...
		const char* recPassword = rec ? rec->GetField(RECORDBANK_PASSWORD) : NULL;
		const char* recSecurity = rec ? rec->GetField(RECORDBANK_SECURITY) : NULL;
			...
			for (int count = 0; count < n; ++count) {
				...
				if (currentchar < length) {
					...
					if (numticksrequired != -1 && progress >= numticksrequired) {
						...
						// int timetonextletter = NumberGenerator::ApplyVariance ( difficulty,
						// HACKDIFFICULTY_VARIANCE * 100 ); nextletter.SetDate ( &(game->GetWorld ()->date) );
					...
					if (IsInterfaceVisible()) {
						char buttonname[64];
						UplinkSnprintf(
							buttonname, sizeof(buttonname), "passwordbreaker %d", SvbLookupPID(this));
					...
					char newcode[128];
					if (username) {
						Computer::GenerateAccessCode(username, password, newcode, sizeof(newcode));
					} else {
						Computer::GenerateAccessCode(password, newcode, sizeof(newcode));
					}
					game->GetWorld()->GetPlayer()->GiveCode(game->GetWorld()->GetPlayer()->remotehost,
															newcode);
...
void PasswordBreaker::PasswordDraw(Button* button, bool highlighted, bool clicked)
{
	...
	*/
	...
	if (strcmp(button->caption.c_str(), "Select target") != 0) {
