...
void UplinkAgentList::UplinkAgentListDraw(Button* button, bool highlighted, bool clicked)
{
	...
	for (int i = 0; i < 7; ++i) {
		...
		if (ual->handles.ValidIndex(index) && ual->names.ValidIndex(index)) {
			...
			} else if (strcmp(name, "Encrypted") == 0) {
...
void UplinkAgentList::UpdateLists()
{
	...
	int foundsize = 0;
	int numfiles = 10;
	const char* datatitle = "Uplink_Agent_Data";
	...
	for (int j = 0; j < sorted.Size(); ++j) {
		...
		if (person == game->GetWorld()->GetPlayer()) {
			...
			if (fileindex >= 0 && fileindex < numfiles) {
				...
				if (filefound[fileindex] == 0) {
					UplinkStrncpy(name, "Unknown", namesize);
				} else if (filefound[fileindex] == -1) {
					UplinkStrncpy(name, "Encrypted", namesize);
				} else if (filefound[fileindex] == 1) {
					UplinkStrncpy(name, person->name, namesize);
				}
			}
		}
