					...
					for (int wordno = 0; wordno < numwords; ++wordno) {
						...
						if (game->GetWorld()->GetPassword(currentindex)) {
							...
							if (strcmp(game->GetWorld()->GetPassword(currentindex), password) == 0) {
								...
								char newcode[128];
								if (username) {
									Computer::GenerateAccessCode(
										username, password, newcode, sizeof(newcode));
								} else {
									Computer::GenerateAccessCode(password, newcode, sizeof(newcode));
								}

								game->GetWorld()->GetPlayer()->GiveCode(
									game->GetWorld()->GetPlayer()->remotehost, newcode);
...
void DictionaryHacker::DictionaryHackerDraw(Button* button, bool highlighted, bool clicked)
{
	...
	glColor3ub(36, 72, 146);
	glVertex2i(button->x + button->width, button->y);
	glColor3ub(82, 134, 206);
	glVertex2i(button->x + button->width, button->y + button->height);
	glColor3ub(73, 122, 194);
...
void DictionaryHacker::SetTarget(UplinkObject* uo, const char* uos, int uoi)
{
	...
	if (uo->GetOBJECTID() == OID_PASSWORDSCREEN) {
		...
		const char* passwd = rec ? rec->GetField(RECORDBANK_PASSWORD) : NULL;
...
void DictionaryHacker::CreateInterface()
{
	...
	if (!IsInterfaceVisible()) {
		...
		EclRegisterButtonCallbacks(
			bname, DictionaryHackerDraw, DictionaryHackerClick, button_click, button_highlight);
