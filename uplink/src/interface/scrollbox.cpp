...
void ScrollBox::SetName(const char* newname)
{
...
void ScrollBox::SetCallback(void (*newcallback)(const char*, int)) { callback = newcallback; }
...
void ScrollBox::DrawScrollBar(Button* button, bool highlighted, bool clicked)
{
	...
	char unused[64];
	char name[128];
	sscanf(button->name.c_str(), "%s %s", unused, name);
	...
	if (thisBox) {
		...
		int x = thisBox->x;
		int w = thisBox->w;
		int y = (int)(thisBox->y + 15
					  + ((float)thisBox->currentIndex / (float)thisBox->numItems) * (thisBox->h - 30));
...
void ScrollBox::ClickScrollBar(Button* button)
{
	...
	if (thisBox) {
		...
		if (mouseY < y) {
			...
			if (thisBox->callback) {
				thisBox->callback(name, thisBox->currentIndex);
			}
...
void ScrollBox::MouseMoveScrollBar(Button* button) { button_highlight(button); }
...
void ScrollBox::UpdateGrabScroll()
{
	...
	if (thisBox) {
		...
		if (thisBox->windowSize >= thisBox->numItems) {
			return;
		}
		...
		if (fractionThrough < 0.0f) {
			fractionThrough = 0.0f;
		}
		if (fractionThrough > 1.0f) {
			fractionThrough = 1.0f;
		}
		...
		if (thisBox->callback) {
			thisBox->callback(thisBox->name, thisBox->currentIndex);
		}
...
void ScrollBox::CreateInterface()
{
	...
	EclRegisterButtonCallbacks(
		scrollBarName, DrawScrollBar, ClickScrollBar, MouseDownScrollBar, MouseMoveScrollBar);
...
bool ScrollBox::IsInterfaceVisible() { return true; }

ScrollBox* ScrollBox::CreateScrollBox(const char* name,
									  int x,
									  int y,
									  int w,
									  int h,
