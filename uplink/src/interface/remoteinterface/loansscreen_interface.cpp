...
void LoansScreenInterface::IncreaseLoanClick(Button* button)
{
	...
	if (thisint->account && thisint->IsVisible() && (accountowner = thisint->account->GetPerson())) {
		...
		if (thisint->account->loan < maxcredit) {
			...
			char rate[16];
			if (newloan <= SMALLLOAN_MAX) {
				UplinkSnprintf(rate, sizeof(rate), "(%d%% apr)", int(100.0 * SMALLLOAN_INTEREST));
			} else if (newloan <= MEDIUMLOAN_MAX) {
				UplinkSnprintf(rate, sizeof(rate), "(%d%% apr)", int(100.0 * MEDIUMLOAN_INTEREST));
			} else if (newloan <= LARGELOAN_MAX) {
				UplinkSnprintf(rate, sizeof(rate), "(%d%% apr)", int(100.0 * LARGELOAN_INTEREST));
			} else {
				UplinkSnprintf(rate, sizeof(rate), "(%d%% apr)", int(100.0 * MAXLOAN_INTEREST));
			}
...
void LoansScreenInterface::DecreaseLoanClick(Button* button)
{
	...
	if (thisint->account && thisint->IsVisible() && (accountowner = thisint->account->GetPerson())) {
		...
		thisint->loanchange -= 100;
		if (thisint->account->loan + thisint->loanchange < 0) {
			thisint->loanchange = thisint->account->loan * -1;
		}
...
void LoansScreenInterface::CloseClick(Button* button)
{
	...
	if (nextpage != -1) {
		game->GetInterface()->GetRemoteInterface()->RunScreen(nextpage,
															  thisint->GetComputerScreen()->GetComputer());
	}
}
...
void LoansScreenInterface::Create()
{
	...
	if (cs) {
		...
		printf("LoansScreenInterface::Create, tried to create when GenericScreen==NULL\n");
	}
}
...
void LoansScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		EclRegisterButton(70,
						  150,
						  100,
						  15,
						  "Account Name",
						  "Shows the name your account is in",
						  "loansscreen_accountname_title");
		...
		EclRegisterButton(
			70, 175, 100, 15, "Account No.", "Shows your account number", "loansscreen_accountnumber_title");
		...
		EclRegisterButton(
			70, 200, 100, 15, "Balance", "Shows your bank balance", "loansscreen_balance_title");
		...
		EclRegisterButton(
			70, 225, 100, 15, "Loan", "Shows your the size of your loan", "loansscreen_loan_title");
		...
		EclRegisterButton(150,
						  ...
						  15,
						  "Increase loan",
						  "Click to take out a larger loan",
						  "loansscreen_increaseloan");
		...
		EclRegisterButton(150,
						  ...
						  15,
						  "Decrease loan",
						  "Click to pay off some of the loan",
						  "loansscreen_decreaseloan");
		...
		if (account && (accountowner = account->GetPerson())) {
			UplinkSnprintf(maxloan,
						   sizeof(maxloan),
						   "Max loan: %dc",
						   account->loan + accountowner->rating.creditrating * 100);
		} else {
		...
		EclRegisterButton(
			150, 340, 100, 15, "OK", "Return to the account management screen", "loansscreen_close");
...
bool LoansScreenInterface::IsVisible() { return (EclGetButton("loansscreen_maintitle") != NULL); }

int LoansScreenInterface::ScreenID() { return SCREEN_LOANSSCREEN; }

GenericScreen* LoansScreenInterface::GetComputerScreen() { return (GenericScreen*)cs; }
