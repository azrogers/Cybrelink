...
void MessageScreenInterface::Click(Button* button)
{
	...
	sscanf(button->name.c_str(), "messagescreen_click %d %s", &nextpage, ip);
...
void MessageScreenInterface::Create()
{
	...
	if (cs) {
		...
		printf("MessageScreenInterface::Create, tried to create when MessageScreen==NULL\n");
	}
}
...
void MessageScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		if (GetComputerScreen()->buttonmessage) {
			...
			UplinkSnprintf(name,
						   sizeof(name),
						   "messagescreen_click %d %s",
						   GetComputerScreen()->nextpage,
						   GetComputerScreen()->GetComputer()->ip);
			EclRegisterButton(320 - width / 2,
							  370,
							  width,
							  20,
							  GetComputerScreen()->buttonmessage,
							  GetComputerScreen()->buttonmessage,
							  name);
		...
		if (GetComputerScreen()->mailthistome) {
			...
			EclRegisterButton(b->x - 120,
							  b->y,
							  100,
							  20,
							  "Mail this to me",
							  "Click to send this information to yourself in an email",
...
bool MessageScreenInterface::IsVisible() { return (EclGetButton("messagescreen_message") != NULL); }
...
int MessageScreenInterface::ScreenID() { return SCREEN_MESSAGESCREEN; }
