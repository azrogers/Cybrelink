...
void DisconnectedScreenInterface::Click(Button* button)
{
	...
	sscanf(button->name.c_str(), "disconnectedscreen_click %d %s", &nextpage, ip);
...
void DisconnectedScreenInterface::Create()
{
	...
	if (cs) {
		...
		printf("DisconnectedScreenInterface::Create, tried to create when MessageScreen==NULL\n");
	}
}
...
void DisconnectedScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		EclRegisterButton(
			80, 60, 350, 25, GetComputerScreen()->maintitle, "", "disconnectedscreen_maintitle");
...
bool DisconnectedScreenInterface::IsVisible() { return (EclGetButton("disconnectedscreen_message") != NULL); }
...
int DisconnectedScreenInterface::ScreenID() { return SCREEN_DISCONNECTEDSCREEN; }
