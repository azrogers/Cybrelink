...
void SharesListScreenInterface::ShareClick(Button* button)
{
	...
	int fileindex;
	sscanf(button->name.c_str(), "shareslistscreen_share %d", &fileindex);
	fileindex += baseoffset;

	SharesListScreenInterface* thisint =
		(SharesListScreenInterface*)game->GetInterface()->GetRemoteInterface()->GetInterfaceScreen();
	...
	if (thisint->filteredlist.ValidIndex(fileindex)) {
		...
		SharesViewScreenInterface* newint =
			(SharesViewScreenInterface*)game->GetInterface()->GetRemoteInterface()->GetInterfaceScreen();
		UplinkAssert(newint);
...
void SharesListScreenInterface::ShareDraw(Button* button, bool highlighted, bool clicked)
{
	...
	int shareindex;
	sscanf(button->name.c_str(), "shareslistscreen_share %d", &shareindex);
...
void SharesListScreenInterface::FilterClick(Button* button)
{
	...
	SharesListScreenInterface* thisinterface =
		((SharesListScreenInterface*)game->GetInterface()->GetRemoteInterface()->GetInterfaceScreen());
	UplinkAssert(thisinterface);

	const char* filter = EclGetButton("shareslistscreen_filtertext")->caption.c_str();
...
void SharesListScreenInterface::ApplyFilter(const char* filter)
{
...
void SharesListScreenInterface::Create()
{
	...
	if (cs) {
		...
		printf("SharesListScreenInterface::Create, tried to create when cs==NULL\n");
	}
}
...
void SharesListScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		if (GetComputerScreen()->SCREENTYPE == SHARESLISTSCREENTYPE_USERSHARES) {
			...
			for (int i = 0; i < allcompanies->Size(); ++i) {
				if (allcompanies->ValidIndex(i)) {
					...
					UplinkAssert(company);
					if (company->TYPE == COMPANYTYPE_INDUSTRIAL || company->TYPE == COMPANYTYPE_COMMERCIAL
		...
		if (fulllist.Size() >= 14) {
			...
			EclRegisterButton(
				390, 145 + 13 * 15, 15, 15, "v", "Scroll downwards", "shareslistscreen_scrolldown");
			...
			EclRegisterButton(
				30, 365, 120, 15, "Filter", "Click here to apply the filter", "shareslistscreen_filter");
			EclRegisterButtonCallback("shareslistscreen_filter", FilterClick);

			EclRegisterButton(
				155, 365, 200, 15, "", "Enter filter pattern here", "shareslistscreen_filtertext");
			EclRegisterButtonCallbacks(
				"shareslistscreen_filtertext", FilterDraw, NULL, button_click, button_highlight);
...
bool SharesListScreenInterface::IsVisible() { return (EclGetButton("shareslistscreen_maintitle") != 0); }
...
int SharesListScreenInterface::ScreenID() { return SCREEN_SHARESLISTSCREEN; }
