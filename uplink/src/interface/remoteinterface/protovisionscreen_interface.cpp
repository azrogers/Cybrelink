...
void ProtovisionScreenInterface::Close(Button* button)
{
	...
	sscanf(button->name.c_str(), "protovisionscreen_close %d %s", &nextpage, ip);
...
void ProtovisionScreenInterface::NuclearWar(Button* button)
{
	...
	char ip[SIZE_VLOCATION_IP] = { 0 };
	sscanf(button->name.c_str(), "protovisionscreen_nuclearwar %s", ip);
...
void ProtovisionScreenInterface::Create()
{
	...
	if (cs) {
		...
		printf("ProtovisionScreenInterface::Create, tried to create when GenericScreen==NULL\n");
	}
}
...
void ProtovisionScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		const char* textmessage = "Falken's maze\n"
								  "Black jack\n"
								  "Gin rummy\n"
								  "hearts\n"
								  "bridge\n"
		...
		UplinkSnprintf(namenuclearwar,
					   sizeof(namenuclearwar),
					   "protovisionscreen_nuclearwar %s",
					   GetComputerScreen()->GetComputer()->ip);
		EclRegisterButton(50, 340, 300, 20, "", "", namenuclearwar);
		EclRegisterButtonCallbacks(
			namenuclearwar, NuclearWarDraw, NuclearWar, button_click, button_highlight);

		int width = 80;
		char name[128 + SIZE_VLOCATION_IP + 1];
		UplinkSnprintf(name,
					   sizeof(name),
					   "protovisionscreen_close %d %s",
...
bool ProtovisionScreenInterface::IsVisible() { return (EclGetButton("protovisionscreen_message") != NULL); }
...
int ProtovisionScreenInterface::ScreenID() { return SCREEN_PROTOVISION; }
