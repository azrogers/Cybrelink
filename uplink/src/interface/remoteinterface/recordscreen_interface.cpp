...
void RecordScreenInterface::CloseClick(Button* button)
{
	...
	sscanf(button->name.c_str(), "recordscreen_click %d %s", &nextpage, ip);
...
void RecordScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		EclRegisterButton(
			20, 370, 50, 20, "Prev", "Scroll to the previous record", "recordscreen_scrollleft");
		...
		EclRegisterButton(
			230, 370, 80, 20, "Commit", "Commit the changes you have made", "recordscreen_commit");
		EclRegisterButtonCallback("recordscreen_commit", CommitClick);

		char name[128 + SIZE_VLOCATION_IP + 1];
		UplinkSnprintf(name,
					   sizeof(name),
					   "recordscreen_click %d %s",
...
void RecordScreenInterface::CreateRecord(int index)
{
	...
	if (rec) {
		...
		for (int j = 0; j < field_titles->Size(); ++j) {
			...
			// The real change should be made in BankComputer::CreateBankAccount, but due to buggy code, it's
			// safer to change it here
			bool found = false;
					...
					if (field_titles->ValidIndex(ii)) {
						if (strcmp(field_titles->GetData(ii), RECORDBANK_ACCNO) == 0) {
							if (field_values->ValidIndex(ii)) {
								BankAccount* account =
									BankAccount::GetAccount(cs->GetComputer()->ip, field_values->GetData(ii));
								if (account) {
									EclRegisterButton(
										140, currentY, 300, buttonheight, account->name, "", bname_value);
									found = true;
								}
							}
							break;
			...
			if (!found) {
				EclRegisterButton(
					140, currentY, 300, buttonheight, field_values->GetData(j), "", bname_value);
			}
...
bool RecordScreenInterface::IsVisible() { return (EclGetButton("recordscreen_maintitle") != 0); }

int RecordScreenInterface::ScreenID() { return SCREEN_RECORDSCREEN; }
