...
HWSalesScreenInterface::HWSalesScreenInterface() { HWType = -1; }
...
void HWSalesScreenInterface::SetHWType(int newType) { HWType = newType; }

void HWSalesScreenInterface::ShowSalesMenuClick(Button* button)
{

	int newHWType;
	sscanf(button->name.c_str(), "hwsales_showmenu %d", &newHWType);

	HWSalesScreenInterface* thisint =
		(HWSalesScreenInterface*)game->GetInterface()->GetRemoteInterface()->GetInterfaceScreen();
...
void HWSalesScreenInterface::ClickHWButton(Button* button)
{
	...
	int index;
	sscanf(button->name.c_str(), "HWsale %d", &index);
	...
	if (sale) {
		...
		if (game->GetWorld()->GetPlayer()->GetBalance() < sv->cost) {
			...
			EclRegisterCaptionChange("hwsales_details",
									 "You do not have enough credits to purchase this device.");
			return;
		}
		...
		if (sale->swhwTYPE == GATEWAYUPGRADETYPE_MEMORY) {
			...
			if (gateway->memorysize + sv->data > gd->maxmemory * 8) {
				EclRegisterCaptionChange("hwsales_details",
										 "You cannot fit this amount of memory in your gateway.");
				return;
			}
		}
		...
		if (sale->swhwTYPE == GATEWAYUPGRADETYPE_SECURITY) {
			...
			if (gateway->GetNumSecurity() + 1 > gd->maxsecurity) {
				EclRegisterCaptionChange("hwsales_details",
										 "You cannot fit this security device in your gateway.");
				return;
			}

			if ((strcmp(sale->title, "Gateway Self Destruct") == 0
				 || strcmp(sale->title, "Gateway Motion Sensor") == 0)) {
				...
				if (alldatasize >= db->GetSize()) {
					EclRegisterCaptionChange("hwsales_details",
											 "You cannot fit this security device in your gateway, you need "
											 "at least 1Gq of free memory for the driver.");
					return;
				}
			}
		}
		...
		if (sale->swhwTYPE == GATEWAYUPGRADETYPE_MODEM) {
			...
			if (gd->bandwidth < sv->data) {
				EclRegisterCaptionChange(
					"hwsales_details",
					"Your gateway does not have sufficient bandwidth to run a modem of that speed.");
				return;
			}
		}
...
void HWSalesScreenInterface::CreateMenu(ComputerScreen* newcs)
{
	...
	if (!IsVisibleMenu()) {
		...
		button_assignbitmaps(
			"hwsales_showmenu 1", "upgrades/cpu.tif", "upgrades/cpu_h.tif", "upgrades/cpu_c.tif");
		...
		button_assignbitmaps(
			"hwsales_showmenu 2", "upgrades/modem.tif", "upgrades/modem_h.tif", "upgrades/modem_c.tif");
		...
		/*
				...
				button_assignbitmaps ( "hwsales_showmenu 3", "upgrades/cooling.tif", "upgrades/cooling_h.tif",
		   "upgrades/cooling_c.tif" ); EclRegisterButton ( 115, 280, 100, 15, "Cooling", " ", "hwsales_caption
		   3" );
		*/
		// Memory
		...
		button_assignbitmaps(
			"hwsales_showmenu 4", "upgrades/memory.tif", "upgrades/memory_h.tif", "upgrades/memory_c.tif");
		...
		button_assignbitmaps("hwsales_showmenu 5",
							 "upgrades/security.tif",
							 "upgrades/security_h.tif",
							 "upgrades/security_c.tif");
		...
		/*
				...
				button_assignbitmaps ( "hwsales_showmenu 6", "upgrades/power.tif", "upgrades/power_h.tif",
		   "upgrades/power_c.tif" ); EclRegisterButton ( 315, 280, 100, 15, "Power", " ", "hwsales_caption 6"
		   );
		*/

		EclRegisterButton(280, 320, 32, 32, " ", "Return to the main menu", "hwsales_exit");
		button_assignbitmaps(
			"hwsales_exit", "upgrades/exit.tif", "upgrades/exit_h.tif", "upgrades/exit_c.tif");
...
void HWSalesScreenInterface::CreateSalesMenu(ComputerScreen* newcs)
{
	...
	if (!IsVisibleSalesMenu()) {
		...
		EclRegisterButton(
			20, 30, 328, 15, "Name", "This column shows the name of the item for sale", "hwsales_name");
		EclRegisterButton(
			350, 30, 60, 15, "Cost", "This column shows the cost of the item in credits", "hwsales_cost");
		...
		for (int i = 0; i < 12; ++i) {
			...
			EclRegisterButtonCallbacks(
				name, DrawHWButton, ClickHWButton, MousedownHWButton, HighlightHWButton);
		}
		...
		EclRegisterButton(355,
						  387,
						  72,
						  15,
						  "Exit Market",
						  "Close the screen and return to the hardware menu",
						  "hwsales_exit");
		EclRegisterButtonCallback("hwsales_exit", ExitClick);
...
bool HWSalesScreenInterface::IsVisible() { return (IsVisibleMenu() || IsVisibleSalesMenu()); }

bool HWSalesScreenInterface::IsVisibleMenu() { return (EclGetButton("hwsales_showmenu 1") != 0); }

bool HWSalesScreenInterface::IsVisibleSalesMenu() { return (EclGetButton("hwsales_exit") != 0); }

int HWSalesScreenInterface::ScreenID() { return SCREEN_HWSALESSCREEN; }
