...
void BBSScreenInterface::ClickBBSButton(Button* button)
{
	...
	int index;
	sscanf(button->name.c_str(), "BBmessage %d", &index);
	...
	if (cu->GetMission(index)) {
		...
		if (game->GetWorld()->GetPlayer()->rating.uplinkrating >= cu->GetMission(index)->minuplinkrating) {
			...
			EclRegisterCaptionChange(
				"bbs_details", "You do not have a sufficient Uplink Rating to view this message.", 2000);
		}
	}
}
...
void BBSScreenInterface::DrawBBSButton(Button* button, bool highlighted, bool clicked)
{
	...
	if (mission) {
		...
		if (!encrypted) {
			...
			float ratingdif =
				(float)(game->GetWorld()->GetPlayer()->rating.uplinkrating - mission->minuplinkrating);
			if (ratingdif > 5.0f) {
				ratingdif = 5.0f;
			}
	...
	if (mission) {
		...
		if (mission->acceptrating == -1) {
			...
			EclRegisterCaptionChange("bbs_details",
									 "This mission cannot be auto-accepted.  You must contact the employers"
		...
		if (mission->TYPE == MISSION_TRACEUSER && strcmp(mission->completionA, "PLAYER") == 0) {

			EclRegisterCaptionChange("bbs_details",
									 "We cannot allow you to accept this mission, on the specific "
		...
		if (game->GetWorld()->GetPlayer()->rating.uplinkrating >= mission->acceptrating) {
			...
			EclRegisterCaptionChange("bbs_details",
									 "You do not have a sufficient Uplink Rating to auto-accept this mission."
...
void BBSScreenInterface::ContactClick(Button* button)
{
	...
	if (mission && bbs->contactpage != -1) {
		...
		if (mission->TYPE == MISSION_TRACEUSER && strcmp(mission->completionA, "PLAYER") == 0) {
			...
			EclRegisterCaptionChange("bbs_details",
									 "We cannot allow you to contact this employer, "
		...
		if (game->GetWorld()->GetPlayer()->rating.uplinkrating >= mission->minuplinkrating) {
			...
			ContactScreenInterface* cs =
				(ContactScreenInterface*)game->GetInterface()->GetRemoteInterface()->GetInterfaceScreen();
			...
			EclRegisterCaptionChange("bbs_details",
									 "You do not have a sufficient Uplink Rating to contact this employer.");
		}
	}
}

void BBSScreenInterface::ScrollChange(const char* scrollname, int newValue)
{
...
void BBSScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		EclRegisterButton(
			20, 30, 100, 15, "Date", "This column shows the date the message was submitted", "bbs_date");
		EclRegisterButton(125,
						  30,
						  22 + SY(388) - 125 + 15,
						  15,
						  "Subject",
						  "This column shows the subject of the message",
						  "bbs_subject");
		...
		for (int i = 0; i < NumItemsOnScreen(); ++i) {
			...
			EclRegisterButtonCallbacks(
				name, DrawBBSButton, ClickBBSButton, MousedownBBSButton, HighlightBBSButton);
		}
		...
		EclRegisterButton(
			baseX, baseY + 20, 70, 15, "Contact", "Contact the employer of this mission", "bbs_contact");
		EclRegisterButtonCallback("bbs_contact", ContactClick);

		EclRegisterButton(baseX,
						  baseY + 110 - 15,
						  70,
						  15,
						  "Exit BBS",
						  "Close the BBS screen and return to the main menu",
						  "bbs_exit");
		EclRegisterButtonCallback("bbs_exit", ExitClick);

		CompanyUplink* cu = (CompanyUplink*)game->GetWorld()->GetCompany("Uplink");
		UplinkAssert(cu);
		ScrollBox::CreateScrollBox("bbs_scroll",
								   22 + SY(388),
								   47,
								   15,
								   NumItemsOnScreen() * 20,
...
bool BBSScreenInterface::IsVisible() { return (EclGetButton("bbs_details") != 0); }
...
int BBSScreenInterface::ScreenID() { return SCREEN_BBSSCREEN; }
