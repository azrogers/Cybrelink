...
ChangeGatewayScreenInterface::ChangeGatewayScreenInterface() { currentselect = 0; }
...
void ChangeGatewayScreenInterface::GatewayButtonDraw(Button* button, bool highlighted, bool clicked)
{
	...
	char unused[64];
	int index;
	sscanf(button->name.c_str(), "%s %d", unused, &index);
	index += baseOffset;
	...
	ChangeGatewayScreenInterface* thisint =
		(ChangeGatewayScreenInterface*)game->GetInterface()->GetRemoteInterface()->GetInterfaceScreen();
	...
	//); 			glColor4f ( 0.6f, 0.6f, 0.6f, ALPHA );		glVertex2i ( button->x + button->width,
	// button->y + button->height ); 			glColor4f ( 0.4f, 0.4f, 0.4f, ALPHA );		glVertex2i (
	// button->x, button->y + button->height ); 		glEnd ();
	//
...
void ChangeGatewayScreenInterface::BuyClick(Button* button)
{
	...
	if (!game->GetWorld()->gatewaydefs.ValidIndex(thisint->currentselect)
		|| !game->GetWorld()->gatewaydefs.GetData(thisint->currentselect)) {
		return;
	}
...
void ChangeGatewayScreenInterface::ScrollChange(const char* scrollname, int newIndex)
{
...
void ChangeGatewayScreenInterface::ShowGateway(int index)
{
	...
	if (gd) {
		...
		char newstats2[128];
		UplinkSnprintf(
			newstats1, sizeof(newstats1), "Max CPUs : %d\nMax Memory : %dGq", gd->maxcpus, gd->maxmemory * 8);
		UplinkSnprintf(newstats2,
					   sizeof(newstats2),
					   "Max Security : %d\nBandwidth : %dGqs",
					   gd->maxsecurity,
					   gd->bandwidth);
	...
	if (!IsVisible()) {
		...
		for (int i = 0; i < numRows; ++i) {
			...
			EclRegisterButtonCallbacks(
				bname, GatewayButtonDraw, GatewayButtonClick, button_click, button_highlight);
		}
		...
		if (numItems > numRows) {
			ScrollBox::CreateScrollBox(
				"changegatewayscroll", 5, 140, 15, numRows * 20, numItems, 10, baseOffset, ScrollChange);
		}
		...
		EclRegisterButton(20,
						  ...
						  15,
						  "Upgrade",
						  "Click to change your gateway for this one",
						  "changegateway_buy");
...
void ChangeGatewayScreenInterface::CreateAreYouSure()
{
	...
	if (!game->GetWorld()->gatewaydefs.ValidIndex(currentselect)
		|| !game->GetWorld()->gatewaydefs.GetData(currentselect)) {
		return;
	}
	...
	EclRegisterButton(
		130, 380, 100, 15, "Proceed", "Click to go ahead with the exchange", "changegateway_buy");
	...
	caption
		<< "You have chosen to part exchange your current gateway for a " << newgateway->name
		<< ".\n"
	...
	if (price < 0) {
		caption
			<< "Because you are downgrading, you may lose some software or hardware during the transfer.\n";
	}

	if (price > 0) {
		caption << "This exchange will cost you " << price << " credits.\n";
	} else {
		caption << "You will be given " << price * -1 << " credits to balance the exchange.\n";
	}
...
bool ChangeGatewayScreenInterface::IsVisible() { return (EclGetButton("changegateway_maintitle") != NULL); }

bool ChangeGatewayScreenInterface::ReturnKeyPressed() { return false; }
...
int ChangeGatewayScreenInterface::ScreenID() { return SCREEN_CHANGEGATEWAY; }
