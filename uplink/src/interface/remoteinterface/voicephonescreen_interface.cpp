...
bool VoicePhoneScreenInterface::ReturnKeyPressed() { return false; }

void VoicePhoneScreenInterface::AddToText(const char* newtext)
{
	...
	if (IsVisible()) {
		...
		if (timesync != -1 && EclGetAccurateTime() > timesync) {
			...
			case VOICESTATUS_TALKING: {
				...
				// #ifndef DEMOGAME
				UplinkSnprintf(
					filename, sizeof(filename), "sounds/analyser/hello%d_%d.wav", voiceindex, samplenumber);
...
bool VoicePhoneScreenInterface::IsVisible() { return (EclGetButton("voicescreen_maintitle") != NULL); }

int VoicePhoneScreenInterface::ScreenID() { return SCREEN_VOICEPHONE; }
