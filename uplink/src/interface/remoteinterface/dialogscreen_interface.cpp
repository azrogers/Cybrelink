...
void DialogScreenInterface::NextPageClick(Button* button)
{
	...
	sscanf(button->name.c_str(), "%s %d %d %s", bname, &data1, &data2, ip);
...
void DialogScreenInterface::ScriptButtonClick(Button* button)
{
	...
	if (scriptindex != -1) {
		ScriptLibrary::RunScript(scriptindex);
	}
...
void DialogScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		for (int i = 0; i < ds->widgets.Size(); ++i) {
			...
			UplinkSnprintf(
				nameip, sizeof(nameip), "%s %d %d %s", dsw->GetName(), dsw->data1, dsw->data2, comp->ip);
			...
			case WIDGET_BASIC:
				EclRegisterButton(
					dsw->x, dsw->y, dsw->width, dsw->height, dsw->GetCaption(), dsw->GetTooltip(), name);
			...
			case WIDGET_NEXTPAGE:
				EclRegisterButton(
					dsw->x, dsw->y, dsw->width, dsw->height, dsw->GetCaption(), dsw->GetTooltip(), nameip);
				EclRegisterButtonCallbacks(
					nameip, button_draw, NextPageClick, button_click, button_highlight);
				break;
			...
			case WIDGET_IMAGE:
				...
				button_assignbitmaps(
					name, dsw->GetStringData1(), dsw->GetStringData2(), dsw->GetStringData2());
				break;
			...
			case WIDGET_IMAGEBUTTON:
				EclRegisterButton(
					dsw->x, dsw->y, dsw->width, dsw->height, dsw->GetCaption(), dsw->GetTooltip(), nameip);
				button_assignbitmaps(
					nameip, dsw->GetStringData1(), dsw->GetStringData2(), dsw->GetStringData2());
			...
			case WIDGET_SCRIPTBUTTON:
				EclRegisterButton(
					dsw->x, dsw->y, dsw->width, dsw->height, dsw->GetCaption(), dsw->GetTooltip(), nameip);
				EclRegisterButtonCallbacks(
					nameip, button_draw, ScriptButtonClick, button_click, button_highlight);
				break;
			...
			case WIDGET_FIELDVALUE:
					...
					dsw->x, dsw->y, dsw->width, dsw->height, dsw->GetCaption(), dsw->GetTooltip(), name);
				{
					EclRegisterButtonCallbacks(name, textbutton_draw, NULL, NULL, NULL);
					EclMakeButtonEditable(name);
					Record* rec = ds->GetComputer()->recordbank.GetRecordFromName(
						game->GetInterface()->GetRemoteInterface()->security_name);
					if (rec) {
						const char* value = rec->GetField(dsw->GetStringData1());
						if (value) {
							EclGetButton(name)->SetCaption(value);
						}
					}
				}
				break;
			...
			case WIDGET_NONE:
				printf("Error : DialogScreenInterface::Create, WIDGETTYPE==NONE\n");
				break;
			}
		}
	}
...
bool DialogScreenInterface::IsVisible() { return (EclGetButton("dialogscreen") != NULL); }

int DialogScreenInterface::ScreenID() { return SCREEN_DIALOGSCREEN; }
