...
void VoiceAnalysisScreenInterface::ClickAccess(Button* button)
{
	...
	VoiceAnalysisScreenInterface* thisint =
		(VoiceAnalysisScreenInterface*)game->GetInterface()->GetRemoteInterface()->GetInterfaceScreen();
	...
	if (thisint->STATUS == VOICEANALYSISSCREEN_PASSED) {
		...
		if (nextpage != -1) {
			game->GetInterface()->GetRemoteInterface()->RunScreen(
				nextpage, thisint->GetComputerScreen()->GetComputer());
		}
	}
}
...
bool VoiceAnalysisScreenInterface::ReturnKeyPressed() { return false; }
...
void VoiceAnalysisScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		EclRegisterCaptionChange(
			"voicescreen_instructions", "Please read the following sentence into your voice device.", 0);
...
bool VoiceAnalysisScreenInterface::IsVisible() { return (EclGetButton("voicescreen_maintitle") != NULL); }

int VoiceAnalysisScreenInterface::ScreenID() { return SCREEN_VOICEANALYSIS; }
