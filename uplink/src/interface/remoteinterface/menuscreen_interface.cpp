...
void MenuScreenInterface::ClickMenuScreenOption(Button* button)
{
	...
	sscanf(button->name.c_str(), "%s %d %d %s", text, &unused, &nextpage, ip);
...
void MenuScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		for (int i = 0; i < GetComputerScreen()->NumOptions(); ++i) {
			...
			if (game->GetInterface()->GetRemoteInterface()->security_level
				<= GetComputerScreen()->GetSecurity(i)) {

				// You have clearance to view this option

				char bname1[32 + SIZE_VLOCATION_IP + 1], bname2[32 + SIZE_VLOCATION_IP + 1];
				UplinkSnprintf(bname1,
							   sizeof(bname1),
							   "menuscreenimage %d %d %s",
							   i,
							   GetComputerScreen()->GetNextPage(i),
							   comp->ip);
				UplinkSnprintf(bname2,
							   sizeof(bname2),
							   "menuscreentext %d %d %s",
							   i,
							   GetComputerScreen()->GetNextPage(i),
							   comp->ip);
				...
				button_assignbitmaps(
					bname1, "menuscreenoption.tif", "menuscreenoption_h.tif", "menuscreenoption_c.tif");
				EclRegisterButtonCallback(bname1, ClickMenuScreenOption);

				EclRegisterButton(-300,
								  120 + i * 30 - 5,
								  300,
								  32,
								  GetComputerScreen()->GetCaption(i),
								  GetComputerScreen()->GetTooltip(i),
								  bname2);
				EclRegisterButtonCallbacks(
					bname2, DrawMenuOption, ClickMenuScreenOption, button_click, button_highlight);
...
bool MenuScreenInterface::IsVisible() { return (EclGetButton("menuscreeninterface") != NULL); }

int MenuScreenInterface::ScreenID() { return SCREEN_MENUSCREEN; }
