...
void CriminalScreenInterface::AddConvictionClick(Button* button)
{
	...
	if (csi->recordindex != -1) {
		...
		std::string newconviction = EclGetButton("criminal_newconvictiontext")->caption;
		...
		const char* existing = rec->GetField("Convictions");
		...
		if (existing) {
			...
			newconvictions << newconviction << '\x0';
...
void CriminalScreenInterface::ArrestClick(Button* button)
{
	...
	if (csi->recordindex != -1) {
		...
		const char* convictions = rec->GetField("Convictions");
		...
		if (convictions) {
			...
			char* current = const_cast<char*>(strchr(convictions, '\n'));
		...
		if (numconvictions >= 2 && brokenparole) {
			...
			create_msgbox("Arrest Not Authorised",
...
void CriminalScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		if (game->GetInterface()->GetRemoteInterface()->security_level <= 2) {
			...
			EclRegisterButton(
				180, 354, 180, 16, "", "Enter new convictions here", "criminal_newconvictiontext");
			EclRegisterButtonCallbacks(
				"criminal_newconvictiontext", HistoryDraw, NULL, NULL, button_highlight);

			EclRegisterButton(
				360, 355, 30, 15, "Add", "Click to add this conviction", "criminal_newconvictionadd");
			EclRegisterButtonCallback("criminal_newconvictionadd", AddConvictionClick);

			// Control buttons

			EclRegisterButton(
				30, 315, 128, 15, "Clear history", "Click here to clear this man's record", "criminal_clear");
			EclRegisterButtonCallback("criminal_clear", ClearClick);

			EclRegisterButton(30,
							  335,
							  128,
							  15,
							  "Authorise Arrest",
							  "Click here to authorise this man's arrest",
							  "criminal_arrest");
...
void CriminalScreenInterface::UpdateScreen()
{
	...
	if (IsVisible()) {
		...
		const char* history = rec->GetField("Convictions");
		...
		if (thisname) {
			EclGetButton("criminal_name")->SetCaption(thisname);
		}
		if (history) {
			EclGetButton("criminal_history")->SetCaption(history);
		}
...
bool CriminalScreenInterface::IsVisible() { return (EclGetButton("criminal_photo") != NULL); }

int CriminalScreenInterface::ScreenID() { return SCREEN_CRIMINALSCREEN; }

GenericScreen* CriminalScreenInterface::GetComputerScreen() { return (GenericScreen*)cs; }
