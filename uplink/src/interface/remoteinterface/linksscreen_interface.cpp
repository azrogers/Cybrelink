...
void LinksScreenInterface::LinkClick(Button* button)
{
	...
	if (game->GetInterface()->GetLocalInterface()->GetHUD()->wmi.IsVisibleWorldMapInterface()
		== WORLDMAP_LARGE) {
		return;
	}
	...
	int fileindex;
	sscanf(button->name.c_str(), "linksscreen_link %d", &fileindex);
...
void LinksScreenInterface::AfterPhoneDialler(char* ip, char* info)
{
	...
	if (info != NULL && strcmp(info, "middleclick") == 0) {
		...
		if (dest->GetComputer()->TYPE == COMPUTER_TYPE_CENTRALMAINFRAME) {
			char tempname[256];
			UplinkSnprintf(
				tempname, sizeof(tempname), "%s Internal Services Machine", dest->GetComputer()->companyname);
...
void LinksScreenInterface::LinkDraw(Button* button, bool highlighted, bool clicked)
{
	...
	int linkindex;
	sscanf(button->name.c_str(), "linksscreen_link %d", &linkindex);
...
void LinksScreenInterface::DeleteLinkDraw(Button* button, bool highlighted, bool clicked)
{
	...
	int linkindex;
	sscanf(button->name.c_str(), "linksscreen_deletelink %d", &linkindex);
	linkindex += baseoffset;
...
void LinksScreenInterface::DeleteLinkClick(Button* button)
{
	...
	if (link && game->GetWorld()->GetPlayer()->HasLink(link) && strcmp(link, IP_INTERNIC) != 0) {

		game->GetWorld()->GetPlayer()->RemoveLink(link);

		LinksScreenInterface* thisinterface =
			((LinksScreenInterface*)game->GetInterface()->GetRemoteInterface()->GetInterfaceScreen());
		...
		if (filterbutton) {
			const char* filter = filterbutton->caption.c_str();
			if (filter[0] == '\x0') {
				thisinterface->ApplyFilter(NULL);
			} else {
				thisinterface->ApplyFilter(filter);
			}
...
void LinksScreenInterface::AddLinkDraw(Button* button, bool highlighted, bool clicked)
{
	...
	int linkindex;
	sscanf(button->name.c_str(), "linksscreen_addlink %d", &linkindex);
	linkindex += baseoffset;
...
void LinksScreenInterface::ShowLinkDraw(Button* button, bool highlighted, bool clicked)
{
	...
	int linkindex;
	sscanf(button->name.c_str(), "linksscreen_showlink %d", &linkindex);
	linkindex += baseoffset;
...
void LinksScreenInterface::FilterClick(Button* button)
{
	...
	const char* filter = EclGetButton("linksscreen_filtertext")->caption.c_str();
...
void LinksScreenInterface::ScrollChange(const char* scrollname, int newValue)
{
...
void LinksScreenInterface::CloseClick(Button* button)
{
	...
	if (nextpage != -1) {
		game->GetInterface()->GetRemoteInterface()->RunScreen(nextpage,
															  lsi->GetComputerScreen()->GetComputer());
	}
}
...
void LinksScreenInterface::ApplyFilter(const char* filter)
{
	...
	if (sb) {
		sb->SetNumItems(filteredlist.Size());
	}
}
...
void LinksScreenInterface::Create()
{
	...
	if (cs) {
		...
		printf("LinksScreenInterface::Create, tried to create when GenericScreen==NULL\n");
	}
}
...
void LinksScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		for (int li = 0; li < numLinks; ++li) {
			...
			if (GetComputerScreen()->SCREENTYPE == LINKSSCREENTYPE_PLAYERLINKS) {
				...
				EclRegisterButtonCallbacks(
					name, DeleteLinkDraw, DeleteLinkClick, button_click, button_highlight);
				...
				EclRegisterButtonCallbacks(
					displaylink, ShowLinkDraw, ShowLinkClick, button_click, ShowLinkMouseMove);
		...
		if (fulllist.Size() >= NumLinksOnScreen()) {
			...
			scrollBox = ScrollBox::CreateScrollBox("linksscreen_scroll",
												   50 + SY(375),
												   145,
												   15,
												   NumLinksOnScreen() * 15,
			...
			EclRegisterButton(15,
							  150 + NumLinksOnScreen() * 15,
							  135,
							  15,
							  "Filter",
			...
			EclRegisterButton(155,
							  150 + NumLinksOnScreen() * 15,
							  width,
							  15,
							  "",
							  "Enter filter pattern here",
							  "linksscreen_filtertext");
			EclRegisterButtonCallbacks(
				"linksscreen_filtertext", FilterDraw, NULL, button_click, button_highlight);
		...
		if (GetComputerScreen()->SCREENTYPE != LINKSSCREENTYPE_PLAYERLINKS
			...
			EclRegisterButton(xpos + 13 + 5,
							  125,
							  width,
							  15,
							  "IP Address",
							  "Shows the IP address of the location",
							  "linksscreen_iptitle");
			EclRegisterButton(155,
							  125,
							  (48 + SY(375)) - 155,
							  15,
							  "Location name",
							  "Shows the computer name at that location",
...
bool LinksScreenInterface::IsVisible() { return (EclGetButton("linksscreen_maintitle") != 0); }
...
int LinksScreenInterface::ScreenID() { return SCREEN_LINKSSCREEN; }
