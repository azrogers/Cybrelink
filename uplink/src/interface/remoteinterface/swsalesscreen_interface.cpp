...
void SWSalesScreenInterface::ClickSWButton(Button* button)
{
	...
	int index;
	sscanf(button->name.c_str(), "SWsale %d", &index);
...
void SWSalesScreenInterface::DrawSWButton(Button* button, bool highlighted, bool clicked)
{
	...
	if (sale) {
		...
		if (index == currentselect) {
			sv = sale->GetVersion(currentversion);
		} else {
			sv = sale->GetVersion(1);
		}
...
void SWSalesScreenInterface::AcceptClick(Button* button)
{
	...
	if (sale) {
		...
		if ((memoryindex != -1 || sale->swhwTYPE == SOFTWARETYPE_HUDUPGRADE) && hasmoney) {
			...
			if (sale->swhwTYPE == SOFTWARETYPE_HUDUPGRADE) {
				...
				if (!game->GetWorld()->GetPlayer()->gateway.HasHUDUpgrade(HUDUPGRADE_CONNECTIONANALYSIS)) {
					...
					EclRegisterCaptionChange(
						"swsales_details",
						"You require a HUD_ConnectionAnalysis upgrade before purchasing any bypasser.");
					return;
				...
				if (!game->GetWorld()->GetPlayer()->gateway.HasHUDUpgrade(HUDUPGRADE_LANVIEW)) {
					...
					EclRegisterCaptionChange(
						"swsales_details",
						"You require a HUD_LanView upgrade before purchasing any LAN tools.");
					return;
			...
			EclRegisterCaptionChange("swsales_details",
									 "You do not have enough memory to store this program.");
			...
			EclRegisterCaptionChange("swsales_details",
									 "You do not have enough credits to purchase this program.");
		}
	}
}
...
void SWSalesScreenInterface::ScrollChange(const char* scrollname, int newValue)
{
...
void SWSalesScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		EclRegisterButton(
			20, 30, 237, 15, "Name", "This column shows the name of the item for sale", "swsales_name");
		EclRegisterButton(260,
						  30,
						  22,
						  15,
						  "Ver",
						  "This column shows the version number of the software",
						  "swsales_version");
		EclRegisterButton(
			285, 30, 60, 15, "Cost", "This column shows the cost of the item in credits", "swsales_cost");
		EclRegisterButton(350,
						  30,
						  57,
						  15,
						  "Size",
						  "This column shows the space required for this item",
						  "swsales_requires");
		...
		for (int i = 0; i < NumItemsOnScreen(); ++i) {
			...
			EclRegisterButtonCallbacks(
				name, DrawSWButton, ClickSWButton, MousedownSWButton, HighlightSWButton);
		}
		...
		EclRegisterButton(
			baseX, baseY + 20, 72, 15, "Next Version", "View the next version", "swsales_nextver");
		EclRegisterButtonCallback("swsales_nextver", NextVersionClick);

		EclRegisterButton(
			baseX, baseY + 40, 72, 15, "Prev Version", "View the previous version", "swsales_prevver");
		...
		EclRegisterButton(baseX,
						  ...
						  15,
						  "Exit Market",
						  "Close the screen and return to the main menu",
						  "swsales_exit");
		EclRegisterButtonCallback("swsales_exit", ExitClick);

		CompanyUplink* cu = (CompanyUplink*)game->GetWorld()->GetCompany("Uplink");
		UplinkAssert(cu);
		ScrollBox::CreateScrollBox("swsales_scroll",
								   22 + SY(390),
								   47,
								   15,
								   NumItemsOnScreen() * 20,
...
bool SWSalesScreenInterface::IsVisible() { return (EclGetButton("swsales_exit") != 0); }

int SWSalesScreenInterface::ScreenID() { return SCREEN_SWSALESSCREEN; }
