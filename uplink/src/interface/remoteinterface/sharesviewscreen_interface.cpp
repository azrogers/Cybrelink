...
void SharesViewScreenInterface::DrawPriceGraph(Button* button, bool highlighted, bool clicked)
{
	...
	SharesViewScreenInterface* thisint =
	...
	if (thisint->companyname[0] != ' ') {
		...
		for (int it = 0; it < 12; ++it) {
			...
			UplinkSnprintf(date,
						   ...
						   month > monthnow ? yearnow - 1 : yearnow);
...
void SharesViewScreenInterface::Create()
{
	...
	if (cs) {
		...
		printf("SharesViewScreenInterface::Create, tried to create when cs==NULL\n");
	}
}
...
void SharesViewScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		if (game->GetInterface()->GetRemoteInterface()->security_level < 10) {
			...
			EclRegisterButton(
				30, 350, 30, 15, "Buy", "Click to buy shares in this company", "sharesviewscreen_buy");
			EclRegisterButtonCallback("sharesviewscreen_buy", BuyClick);

			EclRegisterButton(
				30, 365, 30, 15, "Sell", "Click to sell shares in this company", "sharesviewscreen_sell");
...
bool SharesViewScreenInterface::IsVisible() { return (EclGetButton("sharesviewscreen_maintitle") != NULL); }

int SharesViewScreenInterface::ScreenID() { return SCREEN_SHARESVIEWSCREEN; }
