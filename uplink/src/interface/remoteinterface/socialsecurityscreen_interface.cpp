...
void SocialSecurityScreenInterface::CommitClick(Button* button)
{
	...
	SocialSecurityScreenInterface* sssi =
		(SocialSecurityScreenInterface*)GetInterfaceScreen(SCREEN_SOCSECSCREEN);
	UplinkAssert(sssi);
...
void SocialSecurityScreenInterface::UpdateScreen()
{
	...
	if (IsVisible()) {
		...
		if (person) {
			...
			const char* socsecnum = rec->GetField("Social Security");
			const char* dob = rec->GetField("D.O.B");
			const char* marital = rec->GetField("Marital Status");
			const char* personal = rec->GetField("Personal Status");
...
bool SocialSecurityScreenInterface::IsVisible() { return (EclGetButton("ss_photo") != NULL); }

int SocialSecurityScreenInterface::ScreenID() { return SCREEN_SOCSECSCREEN; }

GenericScreen* SocialSecurityScreenInterface::GetComputerScreen() { return (GenericScreen*)cs; }
