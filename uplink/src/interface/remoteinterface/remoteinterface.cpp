...
static void li_draw(Button* button, bool highlighted, bool clicked)
{
...
void RemoteInterface::SetSecurity(const char* newname, int newvalue)
{
...
void RemoteInterface::RunScreen(int screenindex, Computer* compref)
{
	...
	if (cs) {
		...
		case OID_PASSWORDSCREEN:
			screen = new PasswordScreenInterface();
			break;
		case OID_MENUSCREEN:
		...
		case OID_USERIDSCREEN:
			screen = new UserIDScreenInterface();
			break;
		case OID_LOGSCREEN:
			screen = new LogScreenInterface();
			break;
		case OID_BBSSCREEN:
			screen = new BBSScreenInterface();
			break;
		case OID_LINKSSCREEN:
			screen = new LinksScreenInterface();
			break;
		case OID_SHARESLISTSCREEN:
			screen = new SharesListScreenInterface();
			break;
		case OID_CYPHERSCREEN:
			screen = new CypherScreenInterface();
			break;
		case OID_HIGHSECURITYSCREEN:
			screen = new HighSecurityScreenInterface();
			break;
		case OID_DISCONNECTEDSCREEN:
			screen = new DisconnectedScreenInterface();
			break;
		...
		case OID_GENERICSCREEN:
			...
			case SCREEN_FILESERVERSCREEN:
				screen = new FileServerScreenInterface();
				break;
			case SCREEN_SWSALESSCREEN:
				screen = new SWSalesScreenInterface();
				break;
			case SCREEN_HWSALESSCREEN:
				screen = new HWSalesScreenInterface();
				break;
			case SCREEN_RECORDSCREEN:
				screen = new RecordScreenInterface();
				break;
			case SCREEN_ACCOUNTSCREEN:
				screen = new AccountScreenInterface();
				break;
			case SCREEN_CONTACTSCREEN:
				screen = new ContactScreenInterface();
				break;
			case SCREEN_NEWSSCREEN:
				screen = new NewsScreenInterface();
				break;
			case SCREEN_CRIMINALSCREEN:
				screen = new CriminalScreenInterface();
				break;
			case SCREEN_SECURITYSCREEN:
				screen = new SecurityScreenInterface();
				break;
			case SCREEN_ACADEMICSCREEN:
				screen = new AcademicScreenInterface();
				break;
			case SCREEN_RANKINGSCREEN:
				screen = new RankingScreenInterface();
				break;
			case SCREEN_CONSOLESCREEN:
				screen = new ConsoleScreenInterface();
				break;
			case SCREEN_SOCSECSCREEN:
				screen = new SocialSecurityScreenInterface();
				break;
			case SCREEN_LOANSSCREEN:
				screen = new LoansScreenInterface();
				break;
			case SCREEN_SHARESVIEWSCREEN:
				screen = new SharesViewScreenInterface();
				break;
			case SCREEN_FAITHSCREEN:
				screen = new FaithScreenInterface();
				break;
			case SCREEN_VOICEANALYSIS:
				screen = new VoiceAnalysisScreenInterface();
				break;
			case SCREEN_COMPANYINFO:
				screen = new CompanyInfoScreenInterface();
				break;
			case SCREEN_VOICEPHONE:
				screen = new VoicePhoneScreenInterface();
				break;
			case SCREEN_NEARESTGATEWAY:
				screen = new NearestGatewayScreenInterface();
				break;
			case SCREEN_CHANGEGATEWAY:
				screen = new ChangeGatewayScreenInterface();
				break;
			case SCREEN_CODECARD:
				screen = new CodeCardScreenInterface();
				break;
			case SCREEN_PROTOVISION:
				screen = new ProtovisionScreenInterface();
				break;
			case SCREEN_NUCLEARWAR:
				screen = new NuclearWarScreenInterface();
				break;
			case SCREEN_RADIOTRANSMITTER:
				screen = new RadioTransmitterScreenInterface();
				break;

			default:
				UplinkAbortArgs("Unrecognised GenericScreen %d, computer '%s' (%s)",
								((GenericScreen*)cs)->SCREEN_TYPE,
								comp->name,
								comp->ip);
			}
			break;
		...
		default:
			UplinkAbortArgs("Unrecognised ComputerScreen %d, computer '%s' (%s)",
							cs->GetOBJECTID(),
							comp->name,
							comp->ip);
		}
		...
		UplinkAbortArgs(
			"Tried to run an invalid screenid %d, computer '%s' (%s)", screenindex, comp->name, comp->ip);
	}
}
...
bool RemoteInterface::VerifyScreen(int screenindex)
{
	...
	if (cs) {
		...
		case OID_GENERICSCREEN:
			...
			default:
				UplinkPrintAbortArgs("Unrecognised GenericScreen %d, computer '%s' (%s)",
									 ((GenericScreen*)cs)->SCREEN_TYPE,
									 comp->name,
									 comp->ip);
				return false;
		...
		default:
			UplinkPrintAbortArgs("Unrecognised ComputerScreen %d, computer '%s' (%s)",
								 cs->GetOBJECTID(),
								 comp->name,
								 comp->ip);
			return false;
...
bool RemoteInterface::IsVisible() { return (screen && GetInterfaceScreen()->IsVisible()); }
...
bool RemoteInterface::Load(FILE* file)
{
	...
	if (!FileReadData(&previousscreenindex, sizeof(previousscreenindex), 1, file)) {
		return false;
	}
	if (!FileReadData(&currentscreenindex, sizeof(currentscreenindex), 1, file)) {
		return false;
	}
	if (strcmp(game->GetLoadedSavefileVer(), "SAV59") >= 0) {
		if (!LoadDynamicStringStatic(security_name, sizeof(security_name), file)) {
			return false;
		}
	} else {
	...
	if (!FileReadData(&security_level, sizeof(security_level), 1, file)) {
		return false;
	}

	if (!VerifyScreen(currentscreenindex)) {
		return false;
	}
...
std::string RemoteInterface::GetID() { return "RINT"; }
