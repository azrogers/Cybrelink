...
void AcademicScreenInterface::UpdateScreen()
{
	...
	if (IsVisible()) {
		...
		if (person) {
			...
			const char* collegequals = rec->GetField("College");
			const char* uniquals = rec->GetField("University");
			const char* otherquals = rec->GetField("Other");
...
bool AcademicScreenInterface::IsVisible() { return (EclGetButton("academic_photo") != NULL); }

int AcademicScreenInterface::ScreenID() { return SCREEN_ACADEMICSCREEN; }

GenericScreen* AcademicScreenInterface::GetComputerScreen() { return (GenericScreen*)cs; }
