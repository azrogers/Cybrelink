...
void RadioTransmitterScreenInterface::ConnectClick(Button* button) { ScriptLibrary::RunScript(85); }
...
void RadioTransmitterScreenInterface::Create()
{
	...
	if (cs) {
		...
		printf("RadioTransmitterScreenInterface::Create, tried to create when GenericScreen==NULL\n");
	}
}
...
void RadioTransmitterScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		EclRegisterButton(100,
						  120,
						  300,
						  100,
						  "Wireless LAN transmitter online\nReady to establish Uplink.",
						  " ",
						  "radioscreen_text");
		...
		EclRegisterButton(180,
						  300,
						  180,
						  15,
						  "Establish uplink",
						  "Click to establish the radio link",
						  "radioscreen_connect");
...
bool RadioTransmitterScreenInterface::IsVisible() { return ((bool)EclGetButton("radioscreen_maintitle")); }

int RadioTransmitterScreenInterface::ScreenID() { return SCREEN_RADIOTRANSMITTER; }
	...
	if (comp->TYPE == COMPUTER_TYPE_LAN) {
		...
		for (int i = 0; i < lc->systems.Size(); ++i) {
			if (lc->systems.ValidIndex(i)) {
				...
				if (lcs->TYPE == LANSYSTEM_RADIORECEIVER && lcs->data1 == frequencyGhz
