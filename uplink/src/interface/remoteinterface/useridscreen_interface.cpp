...
void UserIDScreenInterface::ProceedClick(Button* button)
{
	...
	std::string name = EclGetButton("useridscreen_name")->caption;
	std::string code = EclGetButton("useridscreen_code")->caption;

	if (name.length() >= SIZE_PERSON_NAME) {
		name = name.substr(0, SIZE_PERSON_NAME);
	}

	if (code.length() >= 64) {
		code = code.substr(0, 64);
	}
	...
	Record* rec = comp->recordbank.GetRecordFromNamePassword(name.c_str(), code.c_str());
	...
	if (rec) {
		...
		game->GetInterface()->GetRemoteInterface()->SetSecurity(name.c_str(), security);
...
void UserIDScreenInterface::CodeButtonClick(Button* button)
{
	...
	game->GetInterface()->GetTaskManager()->SetProgramTarget(
		uid->GetComputerScreen(), "useridscreen_code", -1);
}
...
void UserIDScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		EclRegisterButtonCallbacks(
			"useridscreen_image", imagebutton_draw, CodeButtonClick, button_click, button_highlight);

		EclRegisterButton(227, 210, 148, 15, "", "Enter your userID here", "useridscreen_name");
		EclRegisterButtonCallbacks(
			"useridscreen_name", UserIDButtonDraw, CodeButtonClick, button_click, button_highlight);

		EclRegisterButton(227, 236, 148, 15, "", "Enter your access code here", "useridscreen_code");
		EclRegisterButtonCallbacks(
			"useridscreen_code", CodeButtonDraw, CodeButtonClick, button_click, button_highlight);
			...
			for (int i = 0; i < codes->Size(); ++i) {
				if (codes->ValidIndex(i) && ips->ValidIndex(i)) {
					if (strcmp(ips->GetData(i), cs->GetComputer()->ip) == 0) {
						...
						EclRegisterButton(
							200, 330 + currentcode * 15, 250, 15, codes->GetData(i), "Use this code", name);
						EclRegisterButtonCallbacks(
							name, textbutton_draw, AccessCodeClick, button_click, button_highlight);
						++currentcode;
					}
				}
			}
		...
		EclRegisterButton(
			3, 20, 30, 15, "#", "Click here to bypass this screen (DEBUG MODE ONLY)", "useridscreen_bypass");
		EclRegisterButtonCallbacks(
			"useridscreen_bypass", textbutton_draw, BypassClick, button_click, button_highlight);
#endif
	}
}
...
bool UserIDScreenInterface::IsVisible() { return (EclGetButton("useridscreen_image") != NULL); }

int UserIDScreenInterface::ScreenID() { return SCREEN_USERIDSCREEN; }
