...
void NuclearWarScreenInterface::DrawMainMap(Button* button, bool highlighted, bool clicked)
{
	...
	NuclearWarScreenInterface* nwsi =
		(NuclearWarScreenInterface*)game->GetInterface()->GetRemoteInterface()->GetInterfaceScreen();
	UplinkAssert(nwsi);
	...
	for (int i = 0; i < nwsi->nukes.Size(); ++i) {
		...
		if (timediff > 10000) {
			...
			if (!nuke->sound) {
				char explosion[128];
				UplinkSnprintf(explosion,
							   sizeof(explosion),
							   "sounds/explosion%d.wav",
							   NumberGenerator::RandomNumber(3) + 1);
...
bool NuclearWarScreenInterface::ReturnKeyPressed() { return false; }
...
void NuclearWarScreenInterface::CloseClick(Button* button)
{
	...
	if (!game->GetWorld()->plotgenerator.PlayerCompletedSpecialMission(SPECIALMISSION_WARGAMES)) {
		...
		news->SetDetails(
			"It emerged today that a high-profile experimental Government computer system "
...
void NuclearWarScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		EclRegisterCaptionChange("nuclearwar_helptext",
...
void NuclearWarScreenInterface::Update() { EclDirtyButton("nuclearwar_largemap"); }

bool NuclearWarScreenInterface::IsVisible() { return (EclGetButton("nuclearwar_largemap") != NULL); }

int NuclearWarScreenInterface::ScreenID() { return SCREEN_NUCLEARWAR; }
