...
void NewsScreenInterface::ClickNewsButton(Button* button)
{
	...
	int index;
	sscanf(button->name.c_str(), "news_story %d", &index);
	...
	if (cu->GetNews(index)) {
		...
		if (scrollBox) {
			...
			std::list<std::string_view> wrappedtext = wordwraptext(newDetails, detailsButton->width);
...
void NewsScreenInterface::ScrollChange(const char* scrollname, int newValue)
{
...
void NewsScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		EclRegisterButton(
			20, 34, 104, 15, "Date", "This column shows the date the story was posted", "news_date");
		EclRegisterButton(125,
						  34,
						  itemWidth - 105 + 15,
						  15,
						  "Subject",
						  "This column shows the subject of the story",
						  "news_subject");
		...
		for (int i = 0; i < numRows; ++i) {
			...
			EclRegisterButtonCallbacks(
				name, DrawNewsButton, ClickNewsButton, MousedownNewsButton, HighlightNewsButton);
		}
		...
		EclRegisterButton(20 + itemWidth + 16 - SX(70),
						  51 + numRows * 40 + SY(105),
						  SX(70),
						  15,
						  "Exit News",
						  "Close the News screen and return to the main menu",
						  "news_exit");
		...
		ScrollBox::CreateScrollBox(
			"news_scroll", itemWidth + 21, 50, 15, numRows * 40, cu->news.Size(), numRows, 0, ScrollChange);
...
bool NewsScreenInterface::IsVisible() { return (EclGetButton("news_exit") != 0); }
...
int NewsScreenInterface::ScreenID() { return SCREEN_NEWSSCREEN; }
