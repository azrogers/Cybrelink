...
void HighSecurityScreenInterface::SystemTitleDraw(Button* button, bool highlighted, bool clicked)
{
	...
	HighSecurityScreen* hs =
		(HighSecurityScreen*)game->GetInterface()->GetRemoteInterface()->GetComputerScreen();
	UplinkAssert(hs);
...
void HighSecurityScreenInterface::SystemClick(Button* button)
{
	...
	if (nextpage != -1) {
		game->GetInterface()->GetRemoteInterface()->RunScreen(nextpage, hs->GetComputer());
	}
}
...
void HighSecurityScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		for (int i = 0; i < hs->systems.Size(); ++i) {
			...
			EclRegisterButton(
				30, 140 + i * 40, 300, 25, mso->caption, "Click here to access this system", title);
			...
			EclRegisterButtonCallbacks(
				"highsecurity_bypass", textbutton_draw, BypassClick, button_click, button_highlight);
#endif
		}
	}
}
...
bool HighSecurityScreenInterface::IsVisible() { return (EclGetButton("highsecurity_maintitle") != NULL); }

int HighSecurityScreenInterface::ScreenID() { return SCREEN_HIGHSECURITYSCREEN; }

bool HighSecurityScreenInterface::ReturnKeyPressed() { return false; }
