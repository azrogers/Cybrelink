...
void LogScreenInterface::CloseClick(Button* button)
{
	...
	ComputerScreen* compscreen =
		game->GetInterface()->GetRemoteInterface()->GetInterfaceScreen()->GetComputerScreen();
...
void LogScreenInterface::LogClick(Button* button)
{
	...
	int logindex;
	sscanf(button->name.c_str(), "logscreen_log %d", &logindex);
...
void LogScreenInterface::ScrollUpClick ( Button *button )
{
	...
	LogScreen *ls = (LogScreen *) game->GetInterface ()->GetRemoteInterface ()->GetInterfaceScreen
()->GetComputerScreen (); int numlogs = ls->GetTargetLogBank ()->logs.Size ();
...
void LogScreenInterface::ScrollChange(const char* scrollname, int newValue)
{
...
void LogScreenInterface::Create()
{
	...
	if (cs) {
		...
		printf("LogScreenInterface::Create, tried to create when LogScreen==NULL\n");
	}
}
...
void LogScreenInterface::CreateScrollBar(int nbItems)
{
	...
	ScrollBox::CreateScrollBox("logscreen_scroll", 420, 140, 15, 15 * 15, nbItems, 15, 0, ScrollChange);
}
...
bool LogScreenInterface::IsVisible() { return (EclGetButton("logscreen_maintitle") != NULL); }
...
int LogScreenInterface::ScreenID() { return SCREEN_LOGSCREEN; }
