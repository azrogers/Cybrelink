...
void CompanyInfoScreenInterface::MdAddClick(Button* button)
{
	...
	CompanyInfoScreenInterface* thisint =
		(CompanyInfoScreenInterface*)game->GetInterface()->GetRemoteInterface()->GetInterfaceScreen();
...
void CompanyInfoScreenInterface::Create() { Create(cs); }
...
void CompanyInfoScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		if (boss && admin) {
			...
			UplinkSnprintf(
				adminemail, sizeof(adminemail), "%s@%s.net", administratornameEmail, companynameEmail);
		...
		if (boss && !game->GetWorld()->GetPlayer()->HasLink(boss->phonenumber)) {
			EclRegisterButton(
				35, 211, 13, 13, " ", "Add this phone number to your contact list", "companyscreen_mdadd");
		...
		if (admin && !game->GetWorld()->GetPlayer()->HasLink(admin->phonenumber)) {
			EclRegisterButton(285,
							  211,
							  13,
							  13,
							  " ",
							  "Add this phone number to your contact list",
							  "companyscreen_adminadd");
...
bool CompanyInfoScreenInterface::IsVisible() { return (EclGetButton("companyscreen_maintitle") != NULL); }

int CompanyInfoScreenInterface::ScreenID() { return SCREEN_COMPANYINFO; }
