...
void ContactScreenInterface::QuestionClick(Button* button)
{
	...
	int index;
	char name[32];
	sscanf(button->name.c_str(), "%s %d", name, &index);
...
void ContactScreenInterface::PutMessage(int userid, const char* message)
{
	...
	if (userid == 0) {
		fullmessage << message << '\x0';
	} else {
		fullmessage << thisint->users.GetData(userid) << " : " << message << '\x0';
	}
...
void ContactScreenInterface::SetTYPE(int newTYPE) { TYPE = newTYPE; }
...
void ContactScreenInterface::SetMission(Mission* newmission)
{
	...
	UplinkAssert(rec);
	const char* crimrec = rec->GetField("Convictions");
	UplinkAssert(crimrec);

	int numconvictions = 0;
	char* currentconviction = const_cast<char*>(strchr(crimrec, '\n'));
...
void ContactScreenInterface::AddUser(const char* name)
{
...
void ContactScreenInterface::AddQuestion(const char* question, int index)
{
	...
	case CONTACT_TYPE_MISSION: {
		...
		case 2: // How secure is the target
			...
			if (thisint->mission->GetHowSecure()) {
				...
				case 0:
				case 1:
					PutMessage(2, "The security will be non-existent.");
					break;
				case 2:
				case 3:
					PutMessage(2, "The security will be low.");
					break;
				case 4:
				case 5:
					PutMessage(2, "There will be some defense in action.");
					break;
				case 6:
				case 7:
					PutMessage(2, "We expect security to be high.");
					break;
				case 8:
				case 9:
					PutMessage(2, "Security will be very tight.");
					break;
				case 10:
				case 11:
				case 12:
				...
				case 19:
				case 20:
					PutMessage(2, "Security will be extremely high.");
					break;

				default:
					UplinkAbortArgs("Unrecognised mission difficulty %d", thisint->mission->difficulty);
				};
		...
		case 3: // I want more money
			...
			if (thisint->askedpayment) {
				...
				int ratingdifference =
					game->GetWorld()->GetPlayer()->rating.uplinkrating - (thisint->mission->acceptrating + 3);
				int chance = 90 + ratingdifference * 30;

				if (chance >= 100 || (chance > 0 && NumberGenerator::RandomNumber(100) <= chance)) {
					...
					if (thisint->mission->maxpayment == 0
						...
						char message[128];
						UplinkSnprintf(message,
									   sizeof(message),
									   "OK, we'll give you %dc because of your reputation.",
									   thisint->mission->maxpayment);
		...
		case 6: // OK, agreed
		{
			...
			game->GetInterface()->GetRemoteInterface()->RunScreen(
				thisint->GetComputerScreen()->nextpage, thisint->GetComputerScreen()->GetComputer());
...
void ContactScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		/*
				...
				EclRegisterButtonCallbacks ( "contact_typehere", TypeHereDraw, NULL, button_click,
		   button_highlight );

				EclRegisterButton ( 375, ybottom, 50, 15, "Post", "Click here to post your message",
		   "contact_post" ); EclRegisterButtonCallback ( "contact_post", PostClick );
...
bool ContactScreenInterface::IsVisible() { return (EclGetButton("contact_title") != NULL); }
...
int ContactScreenInterface::ScreenID() { return SCREEN_CONTACTSCREEN; }

GenericScreen* ContactScreenInterface::GetComputerScreen() { return (GenericScreen*)cs; }
