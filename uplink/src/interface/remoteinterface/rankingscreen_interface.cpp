...
void RankingScreenInterface::PlayerDraw(Button* button, bool highlighted, bool clicked)
{
	...
	glVertex2i(button->x - 1, button->y);
	...
	if (!IsVisible()) {
		...
		for (int ip = 0; ip < allpeople->Size(); ++ip) {
			if (allpeople->ValidIndex(ip)) {
				...
				if (p->GetOBJECTID() == OID_AGENT || p->GetOBJECTID() == OID_PLAYER) {
		...
		for (int i = 0; i < 10; ++i) {
			...
			if (sorted.ValidIndex(i)) {
				...
				if (agent->GetStatus() == PERSON_STATUS_NONE) {
					UplinkStrncpy(cstatus, "Active", sizeof(cstatus));
				} else if (agent->GetStatus() == PERSON_STATUS_INJAIL) {
					UplinkStrncpy(cstatus, "In Jail", sizeof(cstatus));
				} else if (agent->GetStatus() == PERSON_STATUS_DEAD) {
					UplinkStrncpy(cstatus, "Deceased", sizeof(cstatus));
				} else {
					UplinkStrncpy(cstatus, "Unknown", sizeof(cstatus));
				}
		...
		EclRegisterButton(
			355, 387, 70, 15, "Exit", "Close the screen and return to the main menu", "ranking_exit");
...
bool RankingScreenInterface::IsVisible() { return (EclGetButton("ranking_maintitle") != NULL); }

int RankingScreenInterface::ScreenID() { return SCREEN_RANKINGSCREEN; }
