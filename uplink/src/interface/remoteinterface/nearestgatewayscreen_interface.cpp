...
void NearestGatewayScreenInterface::ClickLocation(Button* button)
{
	...
	NearestGatewayScreenInterface* thisint =
		(NearestGatewayScreenInterface*)game->GetInterface()->GetRemoteInterface()->GetInterfaceScreen();
...
bool NearestGatewayScreenInterface::ReturnKeyPressed() { return false; }
...
void NearestGatewayScreenInterface::Create(ComputerScreen* newcs)
{
	...
	if (!IsVisible()) {
		...
		for (int i = 0; i < numPhysicalGatewayLocations; ++i) {
			...
			EclRegisterButton(x1 + (int)(((float)pgl->x / (float)VIRTUAL_WIDTH) * fullsizeX),
							  y1 + (int)(((float)pgl->y / (float)VIRTUAL_HEIGHT) * fullsizeY),
							  7,
							  7,
							  pgl->city,
							  tooltip,
...
bool NearestGatewayScreenInterface::IsVisible() { return (EclGetButton("nearestgateway_largemap") != NULL); }

int NearestGatewayScreenInterface::ScreenID() { return SCREEN_NEARESTGATEWAY; }
