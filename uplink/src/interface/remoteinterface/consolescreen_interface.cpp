...
bool ConsoleScreenInterface::IsVisibleInterface() { return (EclGetButton("console_title") != NULL); }
...
void ConsoleScreenInterface::PostClick(Button* button)
{
	...
	char* actualcommand = const_cast<char*>(strchr(b->caption.c_str(), '>'));

	if (actualcommand) { // Copy actual command here
		actualcommand = new char[b->caption.length() + 1]; // For use below as the button caption
		UplinkSafeStrcpy(actualcommand,
						 strchr(b->caption.c_str(), '>') + 1); // Will be set to '' by the caption change
	}
	...
	if (actualcommand) {
		thisint->RunCommand(actualcommand);
	} else {
		thisint->RunCommand("bollocks");
	}
...
void ConsoleScreenInterface::AddUser(const char* name)
{

	users.PutDataAtEnd(const_cast<char*>(name));
...
void ConsoleScreenInterface::SetCurrentDir(const char* newcurrentdir)
{
	...
	if (strcmp(lcdir, "pub") == 0 || strcmp(lcdir, "usr") == 0 || strcmp(lcdir, "log") == 0
		|| strcmp(lcdir, "rec") == 0 || strcmp(lcdir, "sys") == 0 || strcmp(lcdir, "/") == 0) {

		UplinkStrncpy(currentdir, lcdir, sizeof(currentdir));

	} else if (strcmp(lcdir, ".") == 0 || strcmp(lcdir, "..") == 0) {
...
void ConsoleScreenInterface::PutText(int userid, const char* text)
{
...
void ConsoleScreenInterface::PutTextAtStart(int userid, const char* text)
{
...
void ConsoleScreenInterface::RunCommand(const char* command)
{
	...
	if (strstr(lccommand, "help")) {
		queue.PutDataAtEnd(new ConsoleCommand(CMDTYPE_HELP, NULL, 0));
	} else if (strstr(lccommand, "dir")) {
		queue.PutDataAtEnd(new ConsoleCommand(CMDTYPE_DIR, NULL, 0));
	} else if (strstr(lccommand, "cd ")) {
		queue.PutDataAtEnd(new ConsoleCommand(CMDTYPE_CD, lccommand + 3, 0));
	} else if (strstr(lccommand, "delete")) {
		queue.PutDataAtEnd(new ConsoleCommand(CMDTYPE_DELETEALL, NULL, 0));
	} else if (strstr(lccommand, "run ")) {
		queue.PutDataAtEnd(new ConsoleCommand(CMDTYPE_RUN, lccommand + 4, 0));
	} else if (strstr(lccommand, "exit")) {
		queue.PutDataAtEnd(new ConsoleCommand(CMDTYPE_EXIT, NULL, 0));
	} else if (strstr(lccommand, "shutdown")) {
		queue.PutDataAtEnd(new ConsoleCommand(CMDTYPE_SHUTDOWN, NULL, 0));
	} else if (strstr(lccommand, "disconnect")) {
		queue.PutDataAtEnd(new ConsoleCommand(CMDTYPE_DISCONNECT, NULL, 0));
	} else {
...
void ConsoleScreenInterface::RunCommand(ConsoleCommand* cc)
{
	...
	if (cc->time > 0) {
		...
		case CMDTYPE_EXIT:
			RunCommand_EXIT();
			break;
		case CMDTYPE_DELETEALL:
			RunCommand_DELETEALL(cc->data1);
			break;
		case CMDTYPE_RUN:
			RunCommand_RUN(cc->data1, false);
			break;
		case CMDTYPE_RUNPROGRAM:
			RunCommand_RUN(cc->data1, true);
			break;
		case CMDTYPE_CD:
			SetCurrentDir(cc->data1);
			break;
		case CMDTYPE_SHUTDOWN:
			RunCommand_SHUTDOWN();
			break;
		case CMDTYPE_DISCONNECT:
			RunCommand_DISCONNECT();
			break;
...
void ConsoleScreenInterface::RunCommand_DELETEALL(char* dir)
{
	...
	if (dir) {
		...
		if (strcmp(dir, "sys") == 0) {
			...
			for (int i = 0; i < comp->logbank.logs.Size(); ++i) {
				...
				if (comp->logbank.logs.ValidIndex(i)) {
					...
					al->SetProperties(&logdate, "Unknown", " ", LOG_NOTSUSPICIOUS, LOG_TYPE_DELETED);
		...
		if (strcmp(currentdir, "sys") == 0) {
			...
			// Fix the problem of getting files _not_ destroyed message when there is no file on the server (
			// the databank is not 'formatted' )
			...
			for (int i = 0; i < comp->logbank.logs.Size(); ++i) {
				if (comp->logbank.logs.ValidIndex(i)) {
					...
					if (al) {
						...
						queue.PutDataAtStart(
							new ConsoleCommand(CMDTYPE_TEXT, caption, TIMEREQUIRED_DELETEONELOG));
					}
				}
			}
			...
			for (int i = 0; i < comp->databank.NumDataFiles(); ++i) {
				...
				if (file) {
					...
					queue.PutDataAtStart(new ConsoleCommand(
						CMDTYPE_TEXT, caption, TIMEREQUIRED_DELETEONEGIGAQUAD * file->size));
				}
			}
...
void ConsoleScreenInterface::RunCommand_RUN(char* program, bool actuallyrun)
{
	...
	if (actuallyrun) {
			...
			if (isDataTheProgram) {
				...
				if (strcmp(program, "revelation") == 0) {
					...
					if (comp->isinfected_revelation > 0.0) {
						queue.PutDataAtStart(
							new ConsoleCommand(CMDTYPE_TEXT, "Revelation is still running", 0));
						queue.PutDataAtStart(new ConsoleCommand(CMDTYPE_TEXT, "Failed", 0));
					} else {
						queue.PutDataAtStart(
							new ConsoleCommand(CMDTYPE_TEXT, "Revelation has been purged", 0));
					...
					queue.PutDataAtStart(new ConsoleCommand(
						CMDTYPE_TEXT, "Listening for Revelation activity on port 666...", 0));
					queue.PutDataAtStart(new ConsoleCommand(
						CMDTYPE_TEXT, "RevelationTracer is now INVISIBLE to other users", 0));
					queue.PutDataAtStart(new ConsoleCommand(CMDTYPE_TEXT, "Done.", 0));
					queue.PutDataAtStart(
						new ConsoleCommand(CMDTYPE_TEXT, "Running StEaLtH routines...", 1000));
		...
		if (strcmp(currentdir, "usr") == 0) {
			...
			for (int i = 0; i < comp->databank.GetDataSize(); ++i) {
				...
				if (isDataTheProgram) {
					...
					if (data->TYPE == DATATYPE_PROGRAM) {
						...
						queue.PutDataAtStart(
							new ConsoleCommand(CMDTYPE_TEXT, "This data file is not executable.", 0));
					}
			...
			queue.PutDataAtStart(
				new ConsoleCommand(CMDTYPE_TEXT, "Only programs in the usr dir may be run remotely.", 0));
		}
	}
}
...
void ConsoleScreenInterface::RunCommand_SHUTDOWN()
{
	...
	if (deleted_sys) {
		...
		queue.PutDataAtStart(
			new ConsoleCommand(CMDTYPE_TEXT, "System failure - disconnecting remote users...", 2000));
...
bool ConsoleScreenInterface::IsVisible() { return IsVisibleInterface(); }
...
int ConsoleScreenInterface::ScreenID() { return SCREEN_CONSOLESCREEN; }

GenericScreen* ConsoleScreenInterface::GetComputerScreen() { return (GenericScreen*)cs; }
...
ConsoleCommand::ConsoleCommand(int newTYPE, const char* newdata1, int newtime)
{
