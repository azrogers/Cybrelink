		...
		if (comp->security.NumSystems() == 0) {
			...
			for (int i = 0; i < comp->security.NumSystems(); ++i) {
				...
				if (ss) {
					...
					EclRegisterButton(
						50, 140 + i * 40, 150, 25, ss->GetName(), "Click here to toggle its status", title);
					EclRegisterButton(
						200, 140 + i * 40, 100, 25, leveltext, "Click here to toggle its status", level);
					EclRegisterButton(
						300, 140 + i * 40, 100, 25, "", "Click here to toggle its status", status);

					EclRegisterButtonCallbacks(
						title, SystemTitleDraw, StatusClick, button_click, button_highlight);
					EclRegisterButtonCallbacks(
						level, SystemTitleDraw, StatusClick, button_click, button_highlight);
					EclRegisterButtonCallbacks(
						status, SystemTitleDraw, StatusClick, button_click, button_highlight);
				}
			}
		}
...
bool SecurityScreenInterface::IsVisible() { return (EclGetButton("securityscreen_maintitle") != NULL); }

int SecurityScreenInterface::ScreenID() { return SCREEN_SECURITYSCREEN; }
