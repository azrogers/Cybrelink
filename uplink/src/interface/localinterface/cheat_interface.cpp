...
{
#ifdef CHEATMODES_ENABLED
	game->GetWorld()->GetPlayer()->rating.SetUplinkRating(game->GetWorld()->GetPlayer()->rating.uplinkrating
														  + 1);
...
void CheatInterface::Create()
{
	...
	if (!IsVisible()) {
		...
		EclRegisterButton(
			screenw - panelwidth, paneltop + 50, panelwidth - 7, 15, "Give all links", "cheat_alllinks");
		...
		EclRegisterButton(
			screenw - panelwidth, paneltop + 110, panelwidth - 7, 15, "Give lots of money", "cheat_money");
		EclRegisterButtonCallback("cheat_money", LotsOfMoneyClick);

		// Next rating

		EclRegisterButton(
			screenw - panelwidth, paneltop + 130, panelwidth - 7, 15, "Give next rating", "cheat_nextrating");
		EclRegisterButtonCallback("cheat_nextrating", NextRatingClick);

		// Maximum ratings

		EclRegisterButton(
			screenw - panelwidth, paneltop + 150, panelwidth - 7, 15, "Give max ratings", "cheat_maxratings");
		...
		EclRegisterButton(
			screenw - panelwidth, paneltop + 190, panelwidth - 7, 15, "End Game", "cheat_endgame");
		EclRegisterButtonCallback("cheat_endgame", EndGameClick);

		// Debug Print

		EclRegisterButton(
			screenw - panelwidth, paneltop + 210, panelwidth - 7, 15, "Debug Print", "cheat_debugprint");
		EclRegisterButtonCallback("cheat_debugprint", DebugPrintClick);

		// Event Queue

		EclRegisterButton(
			screenw - panelwidth, paneltop + 230, panelwidth - 7, 15, "Event Queue", "cheat_eventqueue");
		EclRegisterButtonCallback("cheat_eventqueue", EventQueueClick);

		// Run Revelation

		EclRegisterButton(
			screenw - panelwidth, paneltop + 250, panelwidth - 7, 15, "Run Revelation", "cheat_revelation");
...
bool CheatInterface::IsVisible() { return (EclGetButton("cheat_title") != NULL); }

int CheatInterface::ScreenID() { return SCREEN_CHEATS; }
