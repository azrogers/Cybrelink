...
void EventQueueInterface::EventDraw(Button* button, bool highlighted, bool clicked)
{
	...
	int index;
	sscanf(button->name.c_str(), "evtqueue_event %d", &index);
	index += baseoffset;
...
void EventQueueInterface::EventClick(Button* button)
{
	...
	if (index < game->GetWorld()->scheduler.events.Size()) {
		...
		EclRegisterButton(screenw - panelwidth - 3,
						  paneltop + 20,
						  panelwidth,
						  15,
						  date,
		...
		create_stextbox(
			screenw - panelwidth - 3, paneltop + 40, panelwidth, 9 * 30, longdesc, "evtqueue_fulldetails");
...
void EventQueueInterface::DeleteEventDraw(Button* button, bool highlighted, bool clicked)
{
	...
	int index;
	sscanf(button->name.c_str(), "evtqueue_deleteevent %d", &index);
	index += baseoffset;
...
void EventQueueInterface::Create()
{
	...
	if (!IsVisible()) {
		...
		for (int i = 0; i < 9; ++i) {
			...
			EclRegisterButton((screenw - panelwidth - 3) + panelwidth - 35,
							  paneltop + 20 + i * 30,
							  13,
							  13,
							  " ",
							  "Click to delete this event",
							  deletename);
			button_assignbitmaps(deletename, "close.tif", "close_h.tif", "close_c.tif");
			EclRegisterButtonCallbacks(
				deletename, DeleteEventDraw, DeleteEventClick, button_click, button_highlight);
		}
...
bool EventQueueInterface::IsVisible() { return (EclGetButton("evtqueue_title") != NULL); }

int EventQueueInterface::ScreenID() { return SCREEN_EVTQUEUE; }
