	...
	if (!IsVisible()) {
		...
		for (int ic = 0; ic < gatewaydef->maxcpus; ++ic) {
			...
			if (gatewaydef->cpus.ValidIndex(ic)) {
				...
				EclRegisterButton(gatewayX + gdl->x,
								  gatewayY + gdl->y,
								  SIZE_GATEWAY_CPU_W,
								  SIZE_GATEWAY_CPU_H,
								  " ",
				...
				if (ic < cpus->Size()) {
					...
					EclRegisterButtonCallbacks(
						bname, DrawPopulatedItem, NULL, button_click, button_highlight);

				} else {

					EclGetButton(bname)->SetTooltip("Empty CPU slot");
					EclRegisterButtonCallbacks(
						bname, DrawUnPopulatedItem, NULL, button_click, button_highlight);
				}
			}
		}
		...
		for (int im = 0; im < gatewaydef->maxmemory; ++im) {
			...
			if (gatewaydef->memory.ValidIndex(im)) {
				...
				EclRegisterButton(
					gatewayX + gdl->x, gatewayY + gdl->y, SIZE_GATEWAY_MEM_W, SIZE_GATEWAY_MEM_H, " ", bname);
		...
		for (int is = 0; is < gatewaydef->maxsecurity; ++is) {
			...
			if (gatewaydef->security.ValidIndex(is)) {
				...
				EclRegisterButton(
					gatewayX + gdl->x, gatewayY + gdl->y, SIZE_GATEWAY_SEC_W, SIZE_GATEWAY_SEC_H, " ", bname);
		...
		EclRegisterButton(gatewayX + gatewaydef->modemX,
						  gatewayY + gatewaydef->modemY,
						  SIZE_GATEWAY_MODEM_W,
						  SIZE_GATEWAY_MODEM_H,
						  " ",
						  modemtooltip,
						  "gateway_modem");
		...
		EclRegisterButton(gatewayX + gatewaydef->powerX,
						  gatewayY + gatewaydef->powerY,
						  SIZE_GATEWAY_POWER_W,
						  SIZE_GATEWAY_POWER_H,
						  " ",
						  "Power Supply Unit",
						  "gateway_power");
...
bool GatewayInterface::IsVisible() { return (EclGetButton("gateway_background") != NULL); }

int GatewayInterface::ScreenID() { return SCREEN_GATEWAY; }
