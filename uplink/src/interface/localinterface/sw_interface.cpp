...
void SWInterface::SoftwareClick(Button* button)
{
	...
	for (int i = 0; i < numfiles; ++i) {
		...
		if (db->GetDataFile(i) && strcmp(db->GetDataFile(i)->title, softwarename) == 0
...
void SWInterface::SoftwareDraw(Button* button, bool highlighted, bool clicked)
{
	...
	int areyoutheone;
	sscanf(button->name.c_str(), "%s %d", junk, &areyoutheone);

	if (areyoutheone == currentprogrambutton
		&& strstr(button->name.c_str(), "version")
			== NULL) { // Don't auto-highlight buttons from a version menu
		nhighlighted = true;
	}
	...
	GciDrawText(button->x + button->width - 30,
				(button->y + button->height / 2) + 3,
				button->caption.c_str() + strlen(softwarename));
}
	...
	if (!IsVisibleSubMenu(softwareTYPE)) {
		...
		for (int di = 0; di < db->GetDataSize(); ++di) {
			...
			if (db->GetDataFile(di) && db->GetDataFile(di)->TYPE == DATATYPE_PROGRAM
				...
				if (existingindex != -1 && db->GetDataFile(di)->version > versions.GetData(existingindex)) {
...
void SWInterface::SoftwareHighlight(Button* button)
{
	...
	int index;
	sscanf(button->name.c_str(), "hud_software %d", &index);
...
bool SWInterface::IsVisibleSubMenu(int softwareTYPE)
{
	...
	return (EclGetButton("hud_software 0") != NULL && currentsubmenu == softwareTYPE);
}
...
void SWInterface::StartMenuItemDraw(Button* button, bool highlighted, bool clicked)
{
	...
	int softwareTYPE;
	sscanf(button->name.c_str(), "hud_swmenu %d", &softwareTYPE);
...
void SWInterface::ToggleSoftwareMenu()
{
	...
	if (!IsVisibleSoftwareMenu()) {
		...
		EclRegisterButton(
			3, yPos, 100, 15, "File Utilities", "Eg file copiers, deleters, encrypters etc", "hud_swmenu 1");
		EclRegisterButton(3, yPos, 100, 15, "Hardware Drivers", "Eg gateway cameras", "hud_swmenu 2");
		EclRegisterButton(
			3, yPos, 100, 15, "Security", "Eg log deleters, Trace trackers etc", "hud_swmenu 3");
		EclRegisterButton(
			3, yPos, 100, 15, "Crackers", "Eg password breakers, decrypters etc", "hud_swmenu 4");
		EclRegisterButton(
			3, yPos, 100, 15, "Bypassers", "Eg proxy bypass, firewall bypass etc", "hud_swmenu 5");
		...
		if (HasSoftwareType(1)) {
			EclRegisterButtonCallbacks("hud_swmenu 1", StartMenuItemDraw, NULL, NULL, StartMenuItemHighlight);
		} else {
			EclRegisterButtonCallbacks(
				"hud_swmenu 1", StartMenuItemDrawUnavailable, NULL, NULL, StartMenuItemHighlightUnavailable);
		}

		if (HasSoftwareType(2)) {
			EclRegisterButtonCallbacks(
				"hud_swmenu 2", StartMenuItemDraw, NULL, button_click, StartMenuItemHighlight);
		} else {
			EclRegisterButtonCallbacks(
				"hud_swmenu 2", StartMenuItemDrawUnavailable, NULL, NULL, StartMenuItemHighlightUnavailable);
		}

		if (HasSoftwareType(3)) {
			EclRegisterButtonCallbacks(
				"hud_swmenu 3", StartMenuItemDraw, NULL, button_click, StartMenuItemHighlight);
		} else {
			EclRegisterButtonCallbacks(
				"hud_swmenu 3", StartMenuItemDrawUnavailable, NULL, NULL, StartMenuItemHighlightUnavailable);
		}

		if (HasSoftwareType(4)) {
			EclRegisterButtonCallbacks(
				"hud_swmenu 4", StartMenuItemDraw, NULL, button_click, StartMenuItemHighlight);
		} else {
			EclRegisterButtonCallbacks(
				"hud_swmenu 4", StartMenuItemDrawUnavailable, NULL, NULL, StartMenuItemHighlightUnavailable);
		}

		if (HasSoftwareType(5)) {
			EclRegisterButtonCallbacks(
				"hud_swmenu 5", StartMenuItemDraw, NULL, button_click, StartMenuItemHighlight);
		} else {
			EclRegisterButtonCallbacks(
				"hud_swmenu 5", StartMenuItemDrawUnavailable, NULL, NULL, StartMenuItemHighlightUnavailable);
		}

		if (HasSoftwareType(6)) {
			EclRegisterButtonCallbacks(
				"hud_swmenu 6", StartMenuItemDraw, NULL, button_click, StartMenuItemHighlight);
		} else {
			EclRegisterButtonCallbacks(
				"hud_swmenu 6", StartMenuItemDrawUnavailable, NULL, NULL, StartMenuItemHighlightUnavailable);
		}

		if (HasSoftwareType(10)) {
			EclRegisterButtonCallbacks(
				"hud_swmenu 10", StartMenuItemDraw, NULL, button_click, StartMenuItemHighlight);
		} else {
			EclRegisterButtonCallbacks(
				"hud_swmenu 10", StartMenuItemDrawUnavailable, NULL, NULL, StartMenuItemHighlightUnavailable);
		}
...
void SWInterface::ToggleVersionMenu(char* program, int x, int y)
{
	...
	if (program && !IsVisibleVersionMenu(program)) {
...
bool SWInterface::IsVisibleVersionMenu(char* program)
{
	...
	if (program) {
		return (EclGetButton("hud_version 0") != NULL
				&& strstr(EclGetButton("hud_version 0")->caption.c_str(), program) != NULL);
	}
...
void SWInterface::Update()
{
	...
	if (currentprogrambutton != -1 && timesync != -1 && EclGetAccurateTime() - timesync > 1000) {
		...
		if (currentbutton && strcmp(currentbutton->name.c_str(), correctname) == 0) {
			...
			if (HasVersionMenu(swname) && !IsVisibleVersionMenu(swname)) {
...
bool SWInterface::IsVisibleSoftwareMenu() { return (EclGetButton("hud_swmenu 1") != NULL); }

bool SWInterface::IsVisible() { return IsVisibleSoftwareMenu(); }
