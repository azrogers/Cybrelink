...
void IRCInterface::AddEmoticons(int row, const char* smiley, Image* imgSmiley)
{
	...
	if (buffer.ValidIndex(thisRow)) {
		...
		if (imgSmiley) {
			...
			const char* nextSmiley = strstr(msg->text.c_str(), smiley);

			while (nextSmiley) {
				char textSoFar[2048];
				size_t lenCopy = min((nextSmiley - msg->text.c_str()) + 1, sizeof(textSoFar));
				strncpy(textSoFar, msg->text.c_str(), lenCopy);
...
void IRCInterface::PostClick(Button* button)
{
	...
	if (textButton->caption[1] == '£') {
		...
		UplinkIntSnprintf(newMessage, sizeof(newMessage), "%s", (textButton->caption.c_str() + 2));
		newMessage[sizeof(newMessage) - 1] = '\0';

	} else if (strstr(textButton->caption.c_str(), "/nick")) {

		UplinkIntSnprintf(newMessage,
						  sizeof(newMessage),
						  "nick %s: %s",
						  cIrcSession->GetInfo().sNick.c_str(),
						  (textButton->caption.c_str() + 7));
		newMessage[sizeof(newMessage) - 1] = '\0';
	} else if (strstr(textButton->caption.c_str(), "/action")) {

		UplinkIntSnprintf(newMessage,
						  sizeof(newMessage),
						  "privmsg %s: %s :%cACTION %s%c",
						  cIrcSession->GetInfo().sNick.c_str(),
						  channelName,
						  '\x1',
						  (textButton->caption.c_str() + 9),
						  '\x1');
		newMessage[sizeof(newMessage) - 1] = '\0';
	} else {
		UplinkIntSnprintf(newMessage,
...
void IRCInterface::TextScrollChange(const char* name, int newValue) { EclDirtyButton("irc_maintext"); }

void IRCInterface::UserScrollChange(const char* name, int newValue) { EclDirtyButton("irc_userlist"); }

void IRCInterface::AddText(const char* user, const char* text, float r, float g, float b)
{
	...
	std::list<std::string_view> wrapped = wordwraptext(text, mainWidth);
	...
	if (!wrapped.empty()) {
		...
		for (int i = 0; i < numLines; ++i) {
			std::string_view& theLine = *it;
			if (!theLine.empty()) {
				UplinkIRCMessage* msg = new UplinkIRCMessage();
				std::string thisuser = (i == 0 ? user : "");
				msg->Set(thisuser, std::string(theLine), r, g, b);
	...
	if (scrollBox) {
		...
		if (scrollBox->numItems <= scrollBox->windowSize
			|| (scrollBox->currentIndex == scrollBox->numItems - scrollBox->windowSize)) {
			viewingNewest = true;
		}
...
void IRCInterface::ResetUsers()
{
	...
	ScrollBox* scrollBox = ScrollBox::GetScrollBox("irc_userscroll");
	if (scrollBox) {
		scrollBox->SetNumItems(users.Size());
	}
...
void UplinkIRCMessage::Set(std::string newuser, std::string newtext, float r, float g, float b)
{
	text = newtext;
	user = newuser;
	red = r;
...
void IRCInterface::CreateTalkWindow()
{
	...
	ScrollBox::CreateScrollBox("ircscroller",
							   ...
							   buffer.Size(),
							   numRows,
							   (buffer.Size() < numRows) ? 0 : buffer.Size() - numRows,
							   TextScrollChange);
	...
	EclRegisterButton((30 + mainWidth) - 50,
					  mainHeight + 30,
					  50,
					  15,
					  "Post",
...
void IRCInterface::Create()
{
	...
	if (!IsVisible()) {
		...
		// EclRegisterButton ( screenw - panelwidth - 3, paneltop, panelwidth, 15, "INTERNET RELAY CHAT",
		// "Remove the IRC screen", "irc_title" );
		EclRegisterButton(screenw - panelwidth,
						  paneltop + 3,
						  panelwidth - 7,
						  15,
						  "INTERNET RELAY CHAT",
						  "Remove the IRC screen",
						  "irc_title");
		...
		// EclRegisterButton ( screenw - panelwidth - 3, paneltop + 20, panelwidth - 15, 370, "USERS", " ",
		// "irc_userlist" );
		EclRegisterButton(screenw - panelwidth,
						  (paneltop + 3) + 20,
						  (panelwidth - 7) - 20,
		...
		// EclRegisterButton ( screenw - panelwidth - 3, paneltop + 400, panelwidth, 15, "CONNECT", "Click to
		// connect/disconnect", "irc_connect" );
		EclRegisterButton(screenw - panelwidth,
						  (paneltop + 3) + 20 + boxheight + 5,
						  (panelwidth - 7),
						  15,
						  "CONNECT",
		...
		EclRegisterButton(
			30, 150, 100, 15, "Server", "The location of the server to connect to", "irc_servertitle");
		...
		EclRegisterButton(
			130, 150, 300, 15, "irc.uplinkcorp.net", "Enter the Server name here", "irc_server");
...
bool IRCInterface::IsVisible() { return (EclGetButton("irc_title") != NULL); }

int IRCInterface::ScreenID() { return SCREEN_IRC; }
...
void UplinkIRCMonitor::OnIrcDefault(const CIrcMessage* pmsg)
{
	...
	char command[512];
	UplinkIntSnprintf(
		command, sizeof(command), "%s : %s", (char*)pmsg->sCommand.c_str(), (char*)pmsg->AsString().c_str());
...
bool UplinkIRCMonitor::Received_RPL_WELCOME(const CIrcMessage* pmsg)
{
	...
	UplinkIntSnprintf(joinCommand,
					  sizeof(joinCommand),
					  "join %s: %s",
					  IRCInterface::cIrcSession->GetInfo().sNick.c_str(),
					  IRCInterface::channelName);
...
bool UplinkIRCMonitor::Received_PRIVMSG(const CIrcMessage* pmsg)
{
	...
	if (pmsg->m_bIncoming == 0) {
		...
		if (action) {
			...
			UplinkIntSnprintf(parsedMessage,
							  sizeof(parsedMessage),
							  "%s %s",
							  (char*)pmsg->prefix.sNick.c_str(),
							  (action + 7));
			...
			char reply[512];
			UplinkIntSnprintf(reply,
							  sizeof(reply),
							  "NOTICE %s: %s :%cVERSION UplinkIRC v%s%c",
							  IRCInterface::cIrcSession->GetInfo().sNick.c_str(),
							  (char*)pmsg->prefix.sNick.c_str(),
							  '\x1',
							  app->version,
			...
			UplinkIntSnprintf(reply,
							  sizeof(reply),
							  "NOTICE %s: %s :%cFINGER %s running UplinkIRC v%s, www.introversion.co.uk%c",
							  IRCInterface::cIrcSession->GetInfo().sNick.c_str(),
							  (char*)pmsg->prefix.sNick.c_str(),
							  '\x1',
							  IRCInterface::cIrcSession->GetInfo().sNick.c_str(),
...
bool UplinkIRCMonitor::Received_JOIN(const CIrcMessage* pmsg)
{
	...
	if (strlen(pmsg->prefix.sNick.c_str()) > 0) {
		...
		UplinkIntSnprintf(parsedMessage1,
						  sizeof(parsedMessage1),
						  "%s has joined this channel from %s",
						  pmsg->prefix.sNick.c_str(),
						  pmsg->prefix.sHost.c_str());
...
bool UplinkIRCMonitor::Received_PART(const CIrcMessage* pmsg)
{
	...
	if (strlen(pmsg->prefix.sNick.c_str()) > 0) {
		char parsedMessage1[512];
		UplinkIntSnprintf(
			parsedMessage1, sizeof(parsedMessage1), "%s has left this channel", pmsg->prefix.sNick.c_str());
		parsedMessage1[sizeof(parsedMessage1) - 1] = '\0';
...
bool UplinkIRCMonitor::Received_ERR_BANNEDFROMCHAN(const CIrcMessage* pmsg)
{
	...
	if (strcmp(IRCInterface::cIrcSession->GetInfo().sNick.c_str(), GETIRCPARAM(0)) == 0) {
		...
		char parsedMessage[512];
		UplinkIntSnprintf(parsedMessage, sizeof(parsedMessage), "%s : %s", GETIRCPARAM(1), GETIRCPARAM(2));
		parsedMessage[sizeof(parsedMessage) - 1] = '\0';
...
UplinkIRCMonitor::UplinkIRCMonitor(CIrcSession& session) :
	CIrcDefaultMonitor(session) {
		...
		IRC_MAP_ENTRY(UplinkIRCMonitor, "003", Received_RPL_LUSER) IRC_MAP_ENTRY(
			UplinkIRCMonitor, "250", Received_RPL_LUSER)
			IRC_MAP_ENTRY(UplinkIRCMonitor, "251", Received_RPL_LUSER) IRC_MAP_ENTRY(
				UplinkIRCMonitor, "252", Received_RPL_LUSER) IRC_MAP_ENTRY(UplinkIRCMonitor,
																		   "253",
																		   Received_RPL_LUSER)
				IRC_MAP_ENTRY(UplinkIRCMonitor, "254", Received_RPL_LUSER) IRC_MAP_ENTRY(
					UplinkIRCMonitor, "255", Received_RPL_LUSER) IRC_MAP_ENTRY(UplinkIRCMonitor,
																			   "265",
																			   Received_RPL_LUSER)
					IRC_MAP_ENTRY(UplinkIRCMonitor, "266", Received_RPL_LUSER) IRC_MAP_ENTRY(
						UplinkIRCMonitor, "332", Received_RPL_TOPIC) IRC_MAP_ENTRY(UplinkIRCMonitor,
																				   "331",
																				   Received_RPL_TOPIC)
						IRC_MAP_ENTRY(UplinkIRCMonitor, "353", Received_RPL_NAMREPLY) IRC_MAP_ENTRY(
							UplinkIRCMonitor, "366", Received_RPL_ENDOFNAMES)
							IRC_MAP_ENTRY(UplinkIRCMonitor, "372", Received_MOTD) IRC_MAP_ENTRY(
								UplinkIRCMonitor, "375", Received_MOTD) IRC_MAP_ENTRY(UplinkIRCMonitor,
																					  "376",
																					  Received_MOTD)

								IRC_MAP_ENTRY(UplinkIRCMonitor, "PRIVMSG", Received_PRIVMSG)
									IRC_MAP_ENTRY(UplinkIRCMonitor, "NOTICE", Received_RPL_LUSER)
										IRC_MAP_ENTRY(UplinkIRCMonitor, "JOIN", Received_JOIN) IRC_MAP_ENTRY(
											UplinkIRCMonitor, "PART", Received_PART)
												...
												IRC_MAP_ENTRY(UplinkIRCMonitor, "MODE", Received_MODE)
													IRC_MAP_ENTRY(UplinkIRCMonitor, "NICK", Received_NICK)
														IRC_MAP_ENTRY(UplinkIRCMonitor, "QUIT", Received_QUIT)
															...
															IRC_MAP_ENTRY(
																UplinkIRCMonitor, "ERROR", Received_ERROR)
																IRC_MAP_ENTRY(UplinkIRCMonitor,
																			  "474",
																			  Received_ERR_BANNEDFROMCHAN)
