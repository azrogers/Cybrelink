...
Message* SendMailInterface::sGetMessage() { return m; }
...
void SendMailInterface::FileClick(Button* button) { ToggleFileList(button->x - 130, button->y); }
...
void SendMailInterface::AttachFileClick(Button* button)
{
	...
	for (int i = 0; i < db->GetDataSize(); ++i) {
		...
		if (db->GetDataFile(i) && strcmp(db->GetDataFile(i)->title, button->caption.c_str()) == 0) {
...
bool SendMailInterface::IsVisibleFileList() { return (EclGetButton("sendmail_file 0") != 0); }
		...
		if (numfiles > 0) {
			...
			for (int si = 0; si < numfiles; ++si) {
				...
				EclRegisterButtonCallbacks(
					name, button_draw, AttachFileClick, button_click, button_highlight);
...
void SendMailInterface::ToClick(Button* button) { ToggleAddressBook(button->x - 150, button->y); }

bool SendMailInterface::IsVisibleAddressBook() { return (EclGetButton("sendmail_addressbook 0") != 0); }
...
void SendMailInterface::SendClick(Button* button)
{
	...
	std::string sendmail_to = EclGetButton("sendmail_to")->caption;

	if (sendmail_to.length() >= SIZE_PERSON_NAME) {
		sendmail_to = sendmail_to.substr(0, SIZE_PERSON_NAME);
	}
...
void SendMailInterface::Create()
{
	...
	if (!IsVisible()) {
		...
		// EclRegisterButton ( screenw - panelwidth - 3, paneltop, panelwidth, 15, "SEND MAIL", "Remove the
		// send mail screen", "sendmail_title" );
		EclRegisterButton(screenw - panelwidth,
						  paneltop + 3,
						  panelwidth - 7,
						  15,
						  "SEND MAIL",
						  "Remove the send mail screen",
						  "sendmail_title");
		EclRegisterButtonCallback("sendmail_title", TitleClick);

		// EclRegisterButton ( screenw - panelwidth - 3, paneltop + 20, 50, 15, "To", "Show a list of known
		// recipients", "sendmail_tto" ); EclRegisterButton ( screenw - panelwidth - 3, paneltop + 40, 50, 40,
		// "Subject", "sendmail_tsubject" );
		EclRegisterButton(screenw - panelwidth,
						  (paneltop + 3) + 20,
						  50,
						  15,
						  "To",
						  "Show a list of known recipients",
						  "sendmail_tto");
		...
		// EclRegisterButton ( screenw - panelwidth + 49, paneltop + 20, panelwidth - 52, 15, "", "Enter
		// recipient name here", "sendmail_to" ); EclRegisterButton ( screenw - panelwidth + 49, paneltop +
		// 40, panelwidth - 52, 40, "", "Enter subject here", "sendmail_subject" );
		EclRegisterButton(screenw - panelwidth + 49 + 3,
						  (paneltop + 3) + 20,
						  (panelwidth - 7) - 52,
						  15,
						  "",
		...
		// create_stextbox ( screenw - panelwidth - 3, paneltop + 85, panelwidth, boxheight, "",
		// "sendmail_body" );
		create_stextbox(
			screenw - panelwidth, (paneltop + 3) + 85, (panelwidth - 7), boxheight, "", "sendmail_body");
		EclRegisterButtonCallbacks("sendmail_body box", draw_stextbox, NULL, NULL, button_highlight);

		// EclRegisterButton ( screenw - panelwidth - 3, paneltop + 85 + boxheight + 5, panelwidth, 15, "File:
		// None attached", "Attach a file to this email", "sendmail_file" );
		EclRegisterButton(screenw - panelwidth,
						  (paneltop + 3) + 85 + boxheight + 5,
						  (panelwidth - 7),
						  15,
						  "File: None attached",
...
void SendMailInterface::Update()
{
	...
	if (changed && IsVisible()) {
		...
		char attached[128];
		if (m->GetData()) {
			UplinkSnprintf(attached, sizeof(attached), "File: %s", m->GetData()->title);
		} else {
			UplinkStrncpy(attached, "File: None attached", sizeof(attached));
		}
...
bool SendMailInterface::IsVisible() { return (EclGetButton("sendmail_title") != NULL); }

int SendMailInterface::ScreenID() { return SCREEN_SENDMAIL; }
