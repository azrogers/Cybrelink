...
bool LocalInterface::IsVisible() { return (InScreen() != SCREEN_NONE && screen->IsVisible()); }
...
void LocalInterface::RunScreen(int SCREENCODE, int index)
{
	...
	if (currentscreencode != SCREEN_NONE) {
		screen->Create();
	}
...
bool LocalInterface::Load(FILE* file)
{
	...
	if (!FileReadData(&currentscreencode, sizeof(currentscreencode), 1, file)) {
		return false;
	}
	...
	if (!VerifyScreen(currentscreencode, screenindex)) {
		return false;
	}
...
std::string LocalInterface::GetID() { return "LINT"; }
