...
void StatusInterface::Create()
{
	...
	if (!IsVisible()) {
		...
		EclRegisterButton(
			screenw - panelwidth + 50, paneltop + 40, 80, 20, "Ratings", "", "status_ratingstitle");
		EclRegisterButtonCallbacks("status_ratingstitle", MiniTitleDraw, NULL, NULL, NULL);

		//		EclRegisterButton ( screenw - panelwidth, paneltop + 70, cwidth, 30, "UPLINK", "",
		//"status_tuplinkrating" ); 		EclRegisterButton ( screenw - panelwidth, paneltop + 110, cwidth,
		// 30, "NEUROMANCER", "", "status_tneuromancerrating" );
		...
		EclRegisterButton(
			screenw - panelwidth, paneltop + 70, panelwidth - 5, 30, "", "", "status_uplinkrating");
		EclRegisterButton(
			screenw - panelwidth, paneltop + 120, panelwidth - 5, 30, "", "", "status_neuromancerrating");
		...
		char urating[256];
		char nrating[256];
		UplinkSnprintf(urating,
					   sizeof(urating),
					   "GRADE %d UPLINK AGENT\nRating: %s",
					   NUM_UPLINKRATINGS - game->GetWorld()->GetPlayer()->rating.uplinkrating,
					   game->GetWorld()->GetPlayer()->rating.GetUplinkRating());
		UplinkSnprintf(nrating,
					   sizeof(nrating),
					   "NEUROMANCER (Unofficial)\nRating: %s",
					   game->GetWorld()->GetPlayer()->rating.GetNeuromancerRating());
		...
		EclRegisterButton(
			screenw - panelwidth + 50, paneltop + 190, 80, 20, "Criminal", "", "status_legaltitle");
		EclRegisterButtonCallbacks("status_legaltitle", MiniTitleDraw, NULL, NULL, NULL);

		Record* rec = RecordGenerator::GetCriminal("PLAYER");
		const char* crimrec = (rec ? rec->GetField("Convictions") : "");

		EclRegisterButton(
			screenw - panelwidth + 5, paneltop + 220, panelwidth - 15, 60, "", "", "status_legal");
		...
		for (int i = 0; i < 16; ++i) {
			if (game->GetWorld()->plotgenerator.PlayerCompletedSpecialMission(i)) {
				...
				UplinkSnprintf(tooltip,
							   sizeof(tooltip),
							   "Completed Special Mission '%s'",
							   game->GetWorld()->plotgenerator.SpecialMissionTitle(i));
...
bool StatusInterface::IsVisible() { return (EclGetButton("status_title") != NULL); }

int StatusInterface::ScreenID() { return SCREEN_STATUS; }
