...
Options::Options() { UplinkStrncpy(themeName, "graphics", sizeof(themeName)); }
...
Option* Options::GetOption(const char* name)
{
	...
	const BTree<Option*>* btree = options.LookupTree(name);
...
void Options::SetOptionValue(const char* name, int newvalue)
{
...
void Options::SetOptionValue(const char* name, int newvalue, const char* tooltip, bool yesorno, bool visible)
{
...
LList<Option*>* Options::GetAllOptions(const char* searchstring, bool returnhidden)
{
...
void Options::CreateDefaultOptions()
{
	...
	if (!GetOption("game_version")) {
		SetOptionValue("game_version", (int)currentVersion, "z", false, false);
	}
	...
	if (!GetOption("graphics_screendepth")) {
		SetOptionValue("graphics_screendepth",
					   -1,
					   "Sets the colour depth. -1 Means use desktop colour depth.",
					   false,
					   false);
	}
	if (!GetOption("graphics_screenrefresh")) {
		SetOptionValue("graphics_screenrefresh",
					   -1,
					   "Sets the refresh rate. -1 Means use desktop refresh.",
					   false,
					   false);
	}
	if (!GetOption("graphics_fullscreen")) {
		SetOptionValue(
			"graphics_fullscreen", 1, "Sets the game to run fullscreen or in a window", true, true);
	}
	if (!GetOption("graphics_borderless")) {
		SetOptionValue("graphics_borderless",
					   0,
					   "Sets the game to run in borderless fullscreen (overrides fullscreen option)",
					   true,
					   true);
	}
	...
	if (!GetOption("graphics_softwaremouse")) {
		SetOptionValue("graphics_softwaremouse",
					   0,
					   "Render a software mouse.  Use to correct mouse problems.",
					   true,
					   true);
	}
	if (!GetOption("graphics_fasterbuttonanimations")) {
		SetOptionValue(
			"graphics_fasterbuttonanimations", 0, "Increase the speed of button animations.", true, true);
	}
	if (!GetOption("graphics_defaultworldmap")) {
		SetOptionValue(
			"graphics_defaultworldmap", 1, "Create agents with the default world map.", true, true);
	}

	Option* optionSoftwareRendering = GetOption("graphics_softwarerendering");
	if (!optionSoftwareRendering) {
#ifdef WIN32
		SetOptionValue("graphics_softwarerendering",
					   opengl_isSoftwareRendering(),
...
void Options::SetThemeName(const char* newThemeName)
{
	...
	if (rsFilename) {
		...
		while (!thefile.eof()) {
			...
			BTree<ColourOption*>* exists = const_cast<BTree<ColourOption*>*>(colours.LookupTree(colourName));
			if (!exists) {
...
char* Options::GetThemeName() { return themeName; }

char* Options::GetThemeTitle() { return themeTitle; }

char* Options::GetThemeDescription() { return themeDescription; }

ColourOption* Options::GetColour(const char* colourName)
{
...
void Options::RequestShutdownChange(const char* optionName, int newValue)
{
...
bool Options::Load(FILE* file)
{
	...
	if (RsFileEncryptedNoVerify(filename)) {
		if (RsFileEncrypted(filename)) {
			optionsfile = RsFileOpen(filename, "rb");
			encrypted = true;
		} else {
			printf("failed\n");
			return false;
	...
	if (optionsfile) {
		...
		if (version[0] == '\0' || strcmp(version, SAVEFILE_VERSION_MIN) < 0
			|| strcmp(version, SAVEFILE_VERSION) > 0) {
...
std::string Options::GetID() { return "OPTIONS"; }
...
void Option::SetName(const char* newname)
{
...
void Option::SetTooltip(const char* newtooltip)
{
...
void Option::SetYesOrNo(bool newyesorno) { yesorno = newyesorno; }

void Option::SetVisible(bool newvisible) { visible = newvisible; }

void Option::SetValue(int newvalue) { value = newvalue; }
...
bool Option::Load(FILE* file)
{
	...
	if (!FileReadData(&yesorno, sizeof(yesorno), 1, file)) {
		return false;
	}
	if (!FileReadData(&visible, sizeof(visible), 1, file)) {
		return false;
	}
	if (!FileReadData(&value, sizeof(value), 1, file)) {
		return false;
	}
...
std::string Option::GetID() { return "OPTION"; }

int Option::GetOBJECTID() { return OID_OPTION; }
