...
void GameObituary::SetGameOverReason(const char* newreason)
{
...
char* GameObituary::GameOverReason() { return gameoverreason; }

void GameObituary::SetDemoGameOver(bool newvalue) { demogameover = newvalue; }

void GameObituary::SetWarezGameOver(bool newvalue) { warezgameover = newvalue; }
...
void GameObituary::Evaluate()
{
	...
	score = game->GetWorld()->GetPlayer()->rating.uplinkscore + money / 100 + gatewayvalue / 1000
		+ score_peoplefucked + score_systemsfucked + score_highsecurityhacks + numspecialmissions * 200;
...
bool GameObituary::Load(FILE* file)
{
	...
	if (!FileReadData(&money, sizeof(money), 1, file)) {
		return false;
	}
	...
	if (!FileReadData(&score_peoplefucked, sizeof(score_peoplefucked), 1, file)) {
		return false;
	}
	if (!FileReadData(&score_systemsfucked, sizeof(score_systemsfucked), 1, file)) {
		return false;
	}
	if (!FileReadData(&score_highsecurityhacks, sizeof(score_highsecurityhacks), 1, file)) {
		return false;
	}
	if (!FileReadData(&score, sizeof(score), 1, file)) {
		return false;
	}

	if (!FileReadData(&demogameover, sizeof(demogameover), 1, file)) {
		return false;
	}
	if (!FileReadData(&warezgameover, sizeof(warezgameover), 1, file)) {
		return false;
	}

	if (!LoadDynamicStringPtr(&gameoverreason, file)) {
		return false;
	}
...
std::string GameObituary::GetID() { return "GOBIT"; }
