...
//////////////////////////////////////////////////////////////////////185bpmcopyright TJ 19th feb 2000 init (I
/// love you!)XXX
...
void ScriptLibrary::Script10()
{
	...
	std::string target_ip = EclGetButton("t_ip_value 0 0")->caption;
	std::string source_accno = EclGetButton("s_accno_value -1 -1")->caption;
	std::string target_accno = EclGetButton("t_accno_value 0 0")->caption;
	std::string value = EclGetButton("amount_value 0 0")->caption;

	if (target_ip.length() >= SIZE_VLOCATION_IP) {
		target_ip[SIZE_VLOCATION_IP - 1] = '\0';
	}

	if (source_accno.length() >= 16) {
		source_accno = source_accno.substr(0, 16);
	}

	if (target_accno.length() >= 16) {
		target_accno = target_accno.substr(0, 16);
	}
	...
	if (target_ip == "Fill this in" || target_accno == "Fill this in" || value == "Fill this in") {
		return;
	}

	// Prevent if it's not his account and a proxy is running

	if (!game->GetWorld()->GetPlayer()->IsPlayerAccount(source_ip, source_accno.c_str())
	...
	bool success = BankAccount::TransferMoney(source_ip,
											  source_accno.c_str(),
											  target_ip.c_str(),
											  target_accno.c_str(),
											  i_value,
...
void ScriptLibrary::Script11()
{
	...
	std::string& name = EclGetButton("nametext 0 0")->caption;
	std::string& passwd = EclGetButton("passwordtext 0 0")->caption;
	std::string& passwd2 = EclGetButton("passwordtext2 0 0")->caption;
	...
	for (int i = 0; i < baccounts->Size(); i++) {
		if (baccounts->ValidIndex(i)) {
			if (strcmp(name.c_str(), baccounts->GetData(i)->name) == 0) {
				delete baccounts;
	...
	int accno = game->GetWorld()->GetPlayer()->CreateNewAccount(comp->ip, name.c_str(), passwd.c_str(), 0, 0);
...
void ScriptLibrary::Script12()
{
	...
	record->AddField(RECORDBANK_NAME, name.c_str());
	record->AddField(RECORDBANK_PASSWORD, passwd.c_str());
	...
	char code[128];
	Computer::GenerateAccessCode(name.c_str(), passwd.c_str(), code, sizeof(code));
	...
	game->GetInterface()->GetRemoteInterface()->SetSecurity(name.c_str(), 3);
...
void ScriptLibrary::Script13()
{
	...
	Record* rec =
		bank->recordbank.GetRecordFromName(game->GetInterface()->GetRemoteInterface()->security_name);
	UplinkAssert(rec);

	const char* accno = rec->GetField(RECORDBANK_ACCNO);
	UplinkAssert(accno);
...
void ScriptLibrary::Script15()
{
	...
	game->GetInterface()->GetRemoteInterface()->RunScreen(
		8, game->GetInterface()->GetRemoteInterface()->GetComputerScreen()->GetComputer());

	// Start the search going

	CriminalScreenInterface* csi =
		(CriminalScreenInterface*)game->GetInterface()->GetRemoteInterface()->GetInterfaceScreen();
	UplinkAssert(csi);
...
void ScriptLibrary::Script16()
{
	...
	game->GetInterface()->GetRemoteInterface()->RunScreen(
		6, game->GetInterface()->GetRemoteInterface()->GetComputerScreen()->GetComputer());

	// Start the search going

	AcademicScreenInterface* asi =
		(AcademicScreenInterface*)game->GetInterface()->GetRemoteInterface()->GetInterfaceScreen();
	UplinkAssert(asi);
...
void ScriptLibrary::Script17()
{
	...
	SocialSecurityScreenInterface* sssi =
		(SocialSecurityScreenInterface*)game->GetInterface()->GetRemoteInterface()->GetInterfaceScreen();
	UplinkAssert(sssi);
...
void ScriptLibrary::Script30()
{
	...
	EclRegisterCaptionChange(
		"firsttimeloading_text", "Establishing connection to UPLINK public access server...", 1500);
...
void ScriptLibrary::Script33()
{
	...
	if (strcmp(name, RECORDBANK_ADMIN) == 0 || strcmp(name, RECORDBANK_READWRITE) == 0
...
void ScriptLibrary::Script34()
{
	...
	if (!EclGetButton("start_localhost")) {
		...
		EclRegisterButton(key->x + key->width / 2 + 8,
						  key->y + key->height / 2 + 8,
						  lock->x - key->x - 16,
						  lock->x - key->x - 16,
						  " ",
						  " ",
...
void ScriptLibrary::Script35()
{
	...
	if (EclGetButton("start_target") && EclGetButton("start_target")->x < 640) {
		...
		EclRegisterCaptionChange(
			"connecting 0 0", "Establishing connection to your GATEWAY computer...", 600);
...
void ScriptLibrary::Script40()
{
	...
	if (!EclGetButton("downloadingUOS")) {
		...
		EclRegisterButton(
			180, 180, 300, 20, "Downloading UPLINK Operating System...", "", "downloadingUOS_title");
	...
	if (EclGetButton("syscheckUOS_13")) {
		...
		for (int i = 0; i < 14; ++i) {
			...
			if (!EclGetButton(buttonname)) {
				...
				case 0:
					UplinkStrncpy(
						caption, "Checking gateway HARDWARE : Trinity Ax086 2GQs cpu...", sizeof(caption));
					break;
				case 1:
					UplinkStrncpy(
						caption, "Checking gateway HARDWARE : K256 512KQds modem...", sizeof(caption));
					break;
				case 2:
					UplinkStrncpy(
						caption, "Checking gateway HARDWARE : 24 GQds DR-RAM memory...", sizeof(caption));
					break;
				case 3:
					UplinkStrncpy(
						caption, "Verifying integrity of Uplink Operating System...", sizeof(caption));
					break;
				case 4:
					UplinkStrncpy(
						caption, "Verifying security of Uplink Operating System...", sizeof(caption));
					break;
				case 5:
					UplinkStrncpy(caption, "Installing UPLINK Operating system files...", sizeof(caption));
					break;
				case 6:
					UplinkStrncpy(caption, "Initialising TSR programs in memory...", sizeof(caption));
					break;
				case 7:
					UplinkStrncpy(caption, "Sending RUN signal to core Uplink Kernel...", sizeof(caption));
					break;
				case 8:
					UplinkStrncpy(caption, "Checking Kernel Initilisation...", sizeof(caption));
					break;
				case 9:
					UplinkStrncpy(
						caption, "Establishing Network communications protocols...", sizeof(caption));
					break;
				case 10:
					UplinkStrncpy(caption, "Installing File-Copier...", sizeof(caption));
					break;
				case 11:
					UplinkStrncpy(caption, "Installing File-Deleter...", sizeof(caption));
					break;
				case 12:
					UplinkStrncpy(caption, "Installing Task-Manager you can read fast...", sizeof(caption));
					break;
				case 13:
					UplinkStrncpy(caption, "Running Graphical User Interface...", sizeof(caption));
					break;
				}
	...
	if (EclGetButton("newpatch_3")) {
		...
		for (int i = 0; i < 4; ++i) {
			...
			if (!EclGetButton(buttonname)) {
				...
				switch (i) {
				case 0:
					UplinkStrncpy(caption, "Reading new patch data...", sizeof(caption));
					break;
				case 1:
					UplinkStrncpy(caption, "Modifying Uplink executable...", sizeof(caption));
					break;
				case 2:
					UplinkStrncpy(caption, "Modifying Uplink runtime data...", sizeof(caption));
					break;
				case 3:
					UplinkStrncpy(caption,
								  "Validating patch authorisation with Uplink Corporation...",
								  sizeof(caption));
					break;
				}
...
void ShowRecommendationsClick(Button* button) { ScriptLibrary::RunScript(47); }

void ExitGameClick(Button* button) { app->Close(); }
...
void ScriptLibrary::Script46()
{
	...
	EclRegisterCaptionChange("newpatch_text",
							 "UPLINK VERSION 1.3 PATCH\n"
							 "Release notes\n\n"
...
void ScriptLibrary::Script47()
{
	...
	UplinkSnprintf(messagesetresolution,
				   sizeof(messagesetresolution),
				   "Set the in-game screen resolution to %dx%d",
				   mode->w,
				   mode->h);
	delete mode;

	EclRegisterButton(150,
					  250,
					  300,
					  15,
					  messagesetresolution,
					  "This is now the recommended in-game resolution.",
					  "newpatch_screenres");
	EclRegisterButtonCallbacks("newpatch_screenres", textbutton_draw, NULL, button_click, button_highlight);

	EclRegisterButton(
		450, 250, 15, 15, "1", "click to Enable or Disable this option", "newpatch_screenrestoggle");
	button_assignbitmaps("newpatch_screenrestoggle",
						 "mainmenu/optionno.tif",
						 "mainmenu/optionyes.tif",
						 "mainmenu/optionyes.tif");
	EclRegisterButtonCallbacks(
		"newpatch_screenrestoggle", ToggleBoxDraw, ToggleBoxClick, button_click, button_highlight);
	...
	EclRegisterButton(150,
					  ...
					  300,
					  15,
					  "Start a new Agent profile",
					  "Your old profiles will work but many new features won't be available",
					  "newpatch_newuser");
	EclRegisterButtonCallbacks("newpatch_newuser", textbutton_draw, NULL, button_click, button_highlight);

	EclRegisterButton(
		450, 280, 15, 15, "1", "click to Enable or Disable this option", "newpatch_newusertoggle");
	button_assignbitmaps("newpatch_newusertoggle",
						 "mainmenu/optionno.tif",
						 "mainmenu/optionyes.tif",
						 "mainmenu/optionyes.tif");
	EclRegisterButtonCallbacks(
		"newpatch_newusertoggle", ToggleBoxDraw, ToggleBoxClick, button_click, button_highlight);
...
void ScriptLibrary::Script50()
{
	...
	game->GetWorld()->GetPlayer()->ChangeBalance(COST_UPLINK_NEWGATEWAY * -1,
												 "Uplink Corporation : New gateway");
...
void ScriptLibrary::Script62()
{
	...
	EclRegisterCaptionChange("mailtext 0 0",
							 ...
							 "send me each issue for the next year.",
							 2000);
}
...
void ScriptLibrary::Script71()
{
	...
	const char* description = PlotGenerator::SpecialMissionDescription(SPECIALMISSION_MOLE);

	std::ostrstream details;
	details
		<< "We have recently heard a rumour going around that says the complete Uplink Agent "
		   ...
		   "We have also heard a rumour, totally unsubstantiated of course, that you might be the Agent who "
		   "stole "
		   "the data.  Certainly there are only a handful of agents capable, and you are indeed one.\n\n"
		   "We have set up a file Server at the address below, and supplied you with a login.  If the rumour "
		   "is "
		   "true, we will be willing to pay you "
		<< PAYMENT_SPECIALMISSION_MOLE
		<< " for all of the data files and the Agent List program.  "
		   "Payment will be up to the amount specified - if we "
		   "dont receive all of the data we will reduce payment accordingly.\n\n"
		   "If we have made a mistake and you are not the person with whom we should be talking, then we "
		   "apologise.  "
...
void ScriptLibrary::Script72()
{
	...
	news->SetDetails(
		"Federal Agents today announced they were investigating a series of murders and "
		...
		"suggested any connecting factor other than the fact that they all worked freelance for Uplink "
		"Corporation.\n\n"
		"Murders of Uplink Agents are rare - Uplink Corporation is very careful at making sure the agent "
		"handles "
...
void ScriptLibrary::Script80()
{
	...
	if (lan->systems.ValidIndex(currentIndex)) {
		...
		if (system->TYPE == LANSYSTEM_AUTHENTICATION) {
			...
			if (LanMonitor::IsInConnection(unlockIndex1) || LanMonitor::IsInConnection(unlockIndex2)
...
void ScriptLibrary::Script85()
{
	...
	int TIME[] = { 0, 700, 200, 100, 100, 100, 100, 100, 500, 200, 250, 250, 250, 250 };
	...
	if (status) {
		...
		for (int i = 0; i < NUMCAPTIONS; ++i) {
			if (strcmp(status->caption.c_str(), TEXT[i]) == 0) {
				captionIndex = i;
				break;
			}
		}
		...
		if (captionIndex == NUMCAPTIONS - 1) {
			...
			RadioTransmitterScreenInterface* rsi = (RadioTransmitterScreenInterface*)game->GetInterface()
													   ->GetRemoteInterface()
													   ->GetInterfaceScreen();
			rsi->Connect();
...
void ScriptLibrary::Script92()
{
	...
	EclRegisterCaptionChange(
		"firsttimeloading_text", "Awaiting connection acknowledgment from UPLINK...", 800);
