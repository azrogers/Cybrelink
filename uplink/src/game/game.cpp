...
void Game::ExitGame() { opengl_close(); }

void Game::SetGameSpeed(int newspeed) { gamespeed = newspeed; }

int Game::GameSpeed() { return gamespeed; }

bool Game::IsRunning() { return (gamespeed > GAMESPEED_PAUSED); }

void Game::GameOver(const char* reason)
{
...
bool Game::LoadGame(FILE* file)
{
	...
	if (loadedSavefileVer[0] == '\0' || strcmp(loadedSavefileVer, SAVEFILE_VERSION_MIN) < 0
		|| strcmp(loadedSavefileVer, SAVEFILE_VERSION) > 0) {
...
bool Game::Load(FILE* file)
{
	...
	if (strcmp(game->GetLoadedSavefileVer(), "SAV62") >= 0) {
		...
		if (!FileReadData(&whichWorldMap, sizeof(whichWorldMap), 1, file)) {
			return false;
		}
	...
	if (!FileReadData(&gamespeed, sizeof(gamespeed), 1, file)) {
		return false;
	}
	...
	if (strcmp(game->GetLoadedSavefileVer(), "SAV58") >= 0) {
		...
		if (!LoadDynamicStringPtr(&createdSavefileVer, file)) {
			return false;
		}

		if (!FileReadData(&winningCodeWon, sizeof(winningCodeWon), 1, file)) {
			return false;
		}
		if (!LoadDynamicStringPtr(&winningCodeDesc, file)) {
			return false;
		}
		if (!LoadDynamicStringPtr(&winningCodeExtra, file)) {
			return false;
		}
		if (!FileReadData(&winningCodeRandom, sizeof(winningCodeRandom), 1, file)) {
			return false;
		}
	}
...
std::string Game::GetID() { return "GAME"; }
...
bool Game::IsCodeWon() { return (winningCodeWon != 0); }
...
enum Game::WorldMapType Game::GetWorldMapType() { return whichWorldMap; }
