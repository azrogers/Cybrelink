...
template <class T> LList<T>::~LList() { Empty(); }

template <class T> void LList<T>::PutData(const T& newdata) { PutDataAtEnd(newdata); }
...
template <class T> void LList<T>::PutDataAtIndex(const T& newdata, int index)
{
...
template <class T> int LList<T>::Size() { return numitems; }
...
template <class T> T LList<T>::operator[](int index) { return GetData(index); }

template <class T> bool LList<T>::ValidIndex(int index) { return (index >= 0 && index < numitems); }
