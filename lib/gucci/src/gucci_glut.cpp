					 ...
					 char* argv[])
{
	...
	if (debugging) {
		printf("Gucci is storing the current screen settings...");
	}
	GciStoreScreenSize();
	if (debugging) {
		printf("done\n");
	...
	if (runFullScreen) {
		...
		if (debugging) {
			printf("GUCCI is now setting the screen size...");
		}
	...
	*/
	...
	if (debugging) {
		printf("GLUT is now opening a %dx%d window...", screenWidth, screenHeight);
	}
	...
	if (runFullScreen) {
		if (debugging) {
			printf("GLUT is now changing into fullscreen mode...");
		}
		glutFullScreen();
		if (debugging) {
			printf("done\n");
...
int GciFallbackTextWidth(char* text, int STYLE)
{
	...
	#define GUCCI_GLUT_FUNC(Function)                                                                        \
		void Gci##Function##Func(Gci##Function##FuncT* f) { glut##Function##Func(f); }
...
{
	if (gciMouseHandlerP) {
		(*gciMouseHandlerP)(glutButtonToGucci(button), glutStateToGucci(state), x, y);
	}
}
...
bool GciLayerDamaged() { return glutLayerGet(GLUT_NORMAL_DAMAGED) != 0; }

void GciSwapBuffers() { glutSwapBuffers(); }

void GciPostRedisplay() { glutPostRedisplay(); }
