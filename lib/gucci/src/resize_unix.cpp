...
static int ignoreXErrors(Display* dpy, XErrorEvent* event) { return 0; }
...
void GciResizeGlut(int width, int height)
{
	...
	Window rootWindow = XCreateWindow(__glutDisplay,
									  realRoot,
									  0,
									  0,
									  width,
	...
	XSetWMProperties(dpy, rootWindow, NULL, NULL, 0, 0, &size_hints, 0, 0);

	XMapRaised(dpy, rootWindow);

	XSetInputFocus(dpy, rootWindow, RevertToParent, CurrentTime);
	XGrabPointer(dpy, rootWindow, true, 0, GrabModeAsync, GrabModeAsync, rootWindow, None, CurrentTime);
...
bool GciSetScreenSize(int, int, int, int) { return true; }

void GciRestoreScreenSize() { GciRestoreGlut(); }
