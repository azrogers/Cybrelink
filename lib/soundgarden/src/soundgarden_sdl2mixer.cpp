...
static void SgPrintActualSoundSettings()
{
	...
	if (!numtimesopened) {
		...
		printf("SDL_Mixer audio configuration: opened %d times, %dHz (%d) %d channels\n",
			   numtimesopened,
			   frequency,
			   format,
			   channels);
...
void SgPlaySound(const char* fullfilename, const char* id, bool synchronised)
{
	if (!SgInitialised) {
		return;
	}
	...
	const char* sampleid = id ? id : fullfilename;
	...
	if (cache.LookupTree(sampleid)) {
		...
		if (!sample) {
			SgDebugPrintf(
				"SoundGarden WARNING : Failed to load sound file %s\n (%s)", fullfilename, Mix_GetError());
			return;
		}
	...
	if (Mix_PlayChannel(-1 /* First free unreserved channel */, sample, 0 /* number of loops */) == -1) {
		SgDebugPrintf(
			"SoundGarden WARNING : Failed to play sound file %s\n (%s)\n", fullfilename, Mix_GetError());
	}
}
...
void SgPlayMod(const char* fullfilename)
{
	...
	if (currentmod == NULL) {
		SgDebugPrintf(
			"SoundGarden WARNING : Failed to load music file %s\n (%s)\n", fullfilename, Mix_GetError());
		return;
	}
	...
	if (Mix_PlayMusic(currentmod, -1 /* loops */) == -1) {
		SgDebugPrintf(
			"SoundGarden WARNING : Failed to play music file %s\n (%s)\n", fullfilename, Mix_GetError());
		return;
	}
