...
void SgPlaySound(char* filename, char* id, bool synchronised)
{
	...
	++currentwavchannel;
	if (currentwavchannel > 23) {
		currentwavchannel = 16;
	}
}
...
void SgSetModVolume(int newvolume) { mp_volume = newvolume; }

int SgGetModVolume() { return mp_volume; }

bool SgModFinished() { return !currentmod || MP_Ready(); }
