	...
	glList(0)
{
	...
	for (tesselationIndex = 0; tesselationIndex < mesh->TesselationCount(); ++tesselationIndex) {
		...
		for (unsigned int pointIndex = 0; pointIndex < subMesh->PointCount(); ++pointIndex) {
			...
			glTexCoord2f(point.X() / horizontalTextureScale, point.Y() / verticalTextureScale);
	...
	for (tesselationIndex = 0; tesselationIndex < mesh->TesselationCount(); ++tesselationIndex) {
		...
		for (unsigned int pointIndex = 0; pointIndex < subMesh->PointCount(); ++pointIndex) {
			...
			glVertex3f(
				subMesh->Point(pointIndex).X() / 64.0f, subMesh->Point(pointIndex).Y() / 64.0f, -depth);
		}
		glEnd();
	}
	...
	for (size_t c = 0; c < vectoriser.ContourCount(); ++c) {
		...
		for (unsigned int j = 0; j <= numberOfPoints; ++j) {
			...
			if (contourFlag & ft_outline_reverse_fill) {
				glTexCoord2f(point.X() / horizontalTextureScale, point.X() / verticalTextureScale);
...
FTExtrdGlyph::~FTExtrdGlyph() { glDeleteLists(glList, 1); }
...
FTPoint FTExtrdGlyph::GetNormal(const FTPoint& a, const FTPoint& b)
{
	...
	return FTPoint(-vectorY * length, vectorX * length, 0.0f);
}
