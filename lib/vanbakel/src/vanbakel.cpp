...
static LList<TaskWrapper*> tasklist; // List of all running tasks

// Local interface variables

static int numregisteredbuttons = 0; // Number of buttons currently on screen
static int xpos;
static int ypos;
...
int SvbRegisterTask(const char* name, Task* task)
{
...
int SvbNumTasks() { return tasklist.Size(); }
	...
	if (tw) {
		...
		if (!locked) {
			...
			for (int i = 0; i < tasklist.Size(); ++i) {
				...
				if (i != index) {
					SvbChangePriority(i, tasklist[i]->priority * (double)actualamount * -dP, true);
				}
			}
		}
	}
...
Task* SvbGetTask(const char* name)
{
...
bool SvbIsInterfaceVisible() { return (EclGetButton("svb_scalebar") != NULL); }
...
void SvbDebugPrint()
{
	...
	printf(
		"numregisteredbuttons:%d, xpos:%d, ypos:%d, nextpid:%d\n", numregisteredbuttons, xpos, ypos, nextpid);
