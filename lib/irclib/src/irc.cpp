...
void CIrcMessage::ParseIrcCommand(const char* lpszCmdLine)
{
	...
	*/
	...
	// prefix exists ?
	if (*p1 == ':') { // break prefix into its components (nick!user@host)
		p2 = ++p1;
		while (*p2 && !strchr(" !", *p2)) {
			++p2;
		}
...
CIrcIdentServer::~CIrcIdentServer() { Stop(); }

bool CIrcIdentServer::Start(const char* lpszUserID, unsigned int uiPort, const char* lpszResponseType)
{
	if (m_socket) {
		return false;
	}
...
CIrcMonitor::PfnIrcMessageHandler CIrcMonitor::FindMethod(IrcCommandsMapsListEntry* pMapsList,
														  const char* lpszName)
{
