...
WinsockInit::WinsockInit(WORD wVersionRequested) { m_iStatus = WSAStartup(wVersionRequested, &m_wsd); }
...
InetAddr::InetAddr(LPCTSTR lpszAddress, WORD wPort) { Resolve(lpszAddress, wPort); }
...
bool Socket::Listen() { return listen(m_sock, 5) != SOCKET_ERROR; }
...
int Socket::Send(const unsigned char* buf, int cbBuf) { return send(m_sock, (const char*)buf, cbBuf, 0); }
